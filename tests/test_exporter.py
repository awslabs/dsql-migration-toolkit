...
class _FakeS3Client:
    """Captures ``put_object`` calls instead of contacting S3."""
...
# ---------------------------------------------------------------------------
# CSV S3 sink (injected client)
# ---------------------------------------------------------------------------


def test_csv_row_writer_uploads_to_s3_on_close() -> None:
    client = _FakeS3Client()
    writer = CsvRowWriter.for_s3(client, "my-bucket", "exports/events.csv")
    ...
    assert call["Key"] == "exports/events.csv"
...
def test_table_exporter_to_csv_s3_streams_and_uploads() -> None:
    connection = _FakeConnection(_typed_rows())
    engine = _FakeEngine(connection)
    client = _FakeS3Client()
    exporter = TableExporter(engine_factory=lambda _conn: engine, batch_size=10)

    count = exporter.export_table_to_csv_s3(
        _source_config(), _typed_table(), client, "bucket", "key.csv"
    )
...
# ---------------------------------------------------------------------------
# Streaming multipart S3 staging (large-scale: no whole-table local CSV / RAM)
# ---------------------------------------------------------------------------


class _FakeMultipartS3:
    """Captures multipart-upload and put_object calls instead of contacting S3."""

    def __init__(self) -> None:
        self.parts: list[bytes] = []
        self.completed: Optional[list] = None
        self.aborted = False
        self.put: Optional[bytes] = None
        self._part_no = 0

    def create_multipart_upload(self, **kwargs):  # noqa: ANN003, ANN201
        return {"UploadId": "upload-1"}

    def upload_part(self, **kwargs):  # noqa: ANN003, ANN201
        self._part_no += 1
        self.parts.append(bytes(kwargs["Body"]))
        return {"ETag": f"etag-{kwargs['PartNumber']}"}

    def complete_multipart_upload(self, **kwargs):  # noqa: ANN003
        self.completed = kwargs["MultipartUpload"]["Parts"]

    def abort_multipart_upload(self, **kwargs):  # noqa: ANN003
        self.aborted = True

    def put_object(self, **kwargs):  # noqa: ANN003
        self.put = bytes(kwargs["Body"])


def test_export_table_to_s3_streaming_small_uses_single_put_object() -> None:
    connection = _FakeConnection([{"id": 1, "name": "alice"}, {"id": 2, "name": "bob"}])
    engine = _FakeEngine(connection)
    s3 = _FakeMultipartS3()
    exporter = TableExporter(engine_factory=lambda _conn: engine, batch_size=10)

    count = exporter.export_table_to_s3_streaming(
        _source_config(), _simple_table(), s3_client=s3, bucket="b", key="k.csv"
    )

    assert count == 2
    # Small content takes the single put_object path (no multipart overhead).
    assert s3.put is not None
    assert s3.completed is None and s3.parts == [] and s3.aborted is False
    import csv as _csv
    import io as _io

    parsed = list(_csv.reader(_io.StringIO(s3.put.decode("utf-8"))))
    assert parsed[0] == ["id", "name"]
    assert parsed[1] == ["1", "alice"]


def test_s3_multipart_stream_completes_large_content_in_parts() -> None:
    from dsql_migrator.core.exporter import _S3MultipartCsvStream

    s3 = _FakeMultipartS3()
    stream = _S3MultipartCsvStream(s3, "b", "k", part_size_bytes=5 * 1024 * 1024)
    stream.write("a" * (12 * 1024 * 1024))  # 12 MiB -> 5 + 5 + 2 MiB parts
    stream.close()

    assert [len(p) for p in s3.parts] == [
        5 * 1024 * 1024,
        5 * 1024 * 1024,
        2 * 1024 * 1024,
    ]
    assert s3.completed is not None and len(s3.completed) == 3
    assert s3.put is None  # multipart used, not a single put_object


def test_s3_multipart_stream_abort_cancels_started_upload() -> None:
    from dsql_migrator.core.exporter import _S3MultipartCsvStream

    s3 = _FakeMultipartS3()
    stream = _S3MultipartCsvStream(s3, "b", "k", part_size_bytes=5 * 1024 * 1024)
    stream.write("a" * (6 * 1024 * 1024))  # forces one part -> multipart started
    assert s3.parts  # a part was uploaded
    stream.abort()
    stream.close()  # no-op after abort

    assert s3.aborted is True
    assert s3.completed is None  # never completed -> no partial object committed


def test_export_table_to_s3_streaming_does_not_commit_on_failure() -> None:
    class _RaisingConnection(_FakeConnection):
        def execute(self, statement, parameters=None):  # noqa: ANN001, ANN201
            sql = str(statement).strip().upper()
            if sql.startswith("START TRANSACTION") or sql.startswith("COMMIT"):
                return _FakeResult([])
            raise RuntimeError("read boom")

    engine = _FakeEngine(_RaisingConnection([{"id": 1, "name": "a"}]))
    s3 = _FakeMultipartS3()
    exporter = TableExporter(engine_factory=lambda _conn: engine, batch_size=10)

    with pytest.raises(RuntimeError, match="read boom"):
        exporter.export_table_to_s3_streaming(
            _source_config(), _simple_table(), s3_client=s3, bucket="b", key="k.csv"
        )
    # Nothing partial is committed (no complete, no put_object).
    assert s3.completed is None and s3.put is None
