...
def test_postgres_enrich_flags_serial_and_identity_primary_key() -> None:
    # FIX 3: enrich must set auto_increment_column for a serial (nextval DEFAULT) or an
    # IDENTITY (attidentity 'a'/'d') PRIMARY-KEY column, so the converter's primary-key
    # strategy governs it (widen to bigint + GENERATED AS IDENTITY, or UUID/KEEP). A plain
    # integer PK, and a nextval on a NON-PK column, are left alone (auto_increment unset).
    from dsql_migrator.core.models import ColumnDef, TableDef

    d = dialect_for(SourceType.POSTGRES)

    # (a) serial PK: nextval DEFAULT on the PK column
    conn = _DispatchPgConnection({"pg_attribute": lambda p: [
        {"col": "id", "typ": "integer", "gen": "", "ident": ""},
        {"col": "email", "typ": "text", "gen": "", "ident": ""},
    ]})
    serial = TableDef(
        name="users",
        columns=[
            ColumnDef(name="id", mysql_type="?", nullable=False,
                      default="nextval('users_id_seq'::regclass)"),
            ColumnDef(name="email", mysql_type="?"),
        ],
        primary_key=["id"],
    )
    d.enrich(conn, "shop", [serial])
    assert serial.auto_increment_column == "id"

    # (b) IDENTITY PK: attidentity 'd' (GENERATED BY DEFAULT AS IDENTITY), no default
    conn2 = _DispatchPgConnection({"pg_attribute": lambda p: [
        {"col": "id", "typ": "integer", "gen": "", "ident": "d"},
    ]})
    identity = TableDef(
        name="t", columns=[ColumnDef(name="id", mysql_type="?", nullable=False)],
        primary_key=["id"],
    )
    d.enrich(conn2, "shop", [identity])
    assert identity.auto_increment_column == "id"

    # negatives: a plain integer PK, and a nextval on a NON-PK column -> left unset
    conn3 = _DispatchPgConnection({"pg_attribute": lambda p: [
        {"col": "seq", "typ": "integer", "gen": "", "ident": ""},
        {"col": "id", "typ": "uuid", "gen": "", "ident": ""},
    ]})
    plain = TableDef(
        name="t",
        columns=[
            ColumnDef(name="seq", mysql_type="?", nullable=False,
                      default="nextval('s'::regclass)"),
            ColumnDef(name="id", mysql_type="?", nullable=False),
        ],
        primary_key=["id"],
    )
    d.enrich(conn3, "shop", [plain])
    assert plain.auto_increment_column is None
