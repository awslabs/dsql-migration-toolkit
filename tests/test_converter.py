...
def test_the_generated_key_default_is_per_engine_and_per_table() -> None:
    """A PostgreSQL identity key keeps generating; a MySQL AUTO_INCREMENT key does not.

    The old single default (KEEP_INTEGER everywhere) silently dropped the generator from a
    PostgreSQL source whose column already IS an identity -- so the default output broke
    every application that relied on the database filling the key, while the picker's own
    default tile said "nothing in the application changes". MySQL keeps that default on
    purpose: AUTO_INCREMENT is a different mechanism, so converting it to an identity is
    ...
    from dsql_migrator.core.converter import (
        ...
        identity_strategy_applies,
    )
    from dsql_migrator.core.models import ColumnDef, SourceType, TableDef

    def table(pk: "list[str]", key_type: str = "integer") -> TableDef:
        return TableDef(
            name="t",
            columns=[
                ColumnDef(name="id", mysql_type=key_type, nullable=False),
                ColumnDef(name="ts", mysql_type="timestamp", nullable=False),
            ],
            primary_key=pk,
            auto_increment_column="id",
        )

    def key_line(engine: SourceType, tbl: TableDef) -> str:
        ddl = SchemaConverter(source_type=engine).convert_table(tbl).target_ddl
        return next(line.strip() for line in ddl.splitlines() if '"id"' in line)

    # PostgreSQL: the generator is PRESERVED, which on DSQL means bigint + explicit CACHE
    # (both are DSQL requirements -- an INTEGER identity is rejected outright).
    for key_type in ("integer", "bigint", "smallint"):
        line = key_line(SourceType.POSTGRES, table(["id"], key_type))
        assert "BIGINT" in line, line
        assert "GENERATED BY DEFAULT AS IDENTITY (CACHE 65536)" in line, line

    # MySQL: unchanged -- plain integer, no identity, whatever the width.
    for key_type, expected in (
        ...
        line = key_line(SourceType.MYSQL, table(["id"], key_type))
        assert "IDENTITY" not in line, (key_type, line)
        assert expected in line, (key_type, line)

    # A composite key gets no identity on EITHER engine: a DSQL identity is single-column.
    for engine in (SourceType.MYSQL, SourceType.POSTGRES):
        line = key_line(engine, table(["id", "ts"]))
        assert "IDENTITY" not in line, (engine, line)

    # The predicate and the resolver are the two seams the UI shares, so pin them.
    assert identity_strategy_applies(table(["id"])) is True
    assert identity_strategy_applies(table(["id", "ts"])) is False
    assert (
        default_primary_key_strategy(table(["id"]), is_postgres=True)
        is PrimaryKeyStrategy.IDENTITY_WITH_CACHE
    )
    for kwargs in ({"is_postgres": False}, {}):
        assert (
            default_primary_key_strategy(table(["id"]), **kwargs)
            is PrimaryKeyStrategy.KEEP_INTEGER
        )
    assert (
        default_primary_key_strategy(table(["id", "ts"]), is_postgres=True)
        is PrimaryKeyStrategy.KEEP_INTEGER
    )

    # An EXPLICIT choice still wins over the per-engine default, both ways.
    explicit_keep = SchemaConverter(source_type=SourceType.POSTGRES).convert_table(
        table(["id"]),
        SchemaConvertOptions(primary_key_strategy=PrimaryKeyStrategy.KEEP_INTEGER),
    )
    assert "IDENTITY" not in explicit_keep.target_ddl
    explicit_identity = SchemaConverter(source_type=SourceType.MYSQL).convert_table(
        table(["id"], "int"),
        SchemaConvertOptions(primary_key_strategy=PrimaryKeyStrategy.IDENTITY_WITH_CACHE),
    )
    assert "GENERATED BY DEFAULT AS IDENTITY" in explicit_identity.target_ddl
    # ... but an explicit IDENTITY on a composite key still cannot be honoured, and the
    # fallback says why instead of emitting an identity the picker never offers.
    fallback = SchemaConverter(source_type=SourceType.POSTGRES).convert_table(
        table(["id", "ts"]),
        SchemaConvertOptions(primary_key_strategy=PrimaryKeyStrategy.IDENTITY_WITH_CACHE),
    )
    assert "IDENTITY" not in fallback.target_ddl
    reason = " ".join(w.message for w in fallback.warnings if "plain integer" in w.message)
    assert "cannot be used for this table" in reason, reason


def test_the_applied_ddl_parsers_survive_the_identity_cache_clause() -> None:
    """sqlglot cannot parse the identity CACHE option, so ALL THREE returned nothing.

    The clause is injected as raw text precisely because sqlglot cannot render it -- and the
    same limitation made ``sqlglot.parse_one`` raise when the applied DDL was read back.
    Every table with an identity column therefore reported no column types, no primary key
    and no generated columns. It became the COMMON case the moment a PostgreSQL identity key
    started defaulting to an identity target, and the consequence is the one the code's own
    comment warns about: Full Load fell back to the SOURCE type, so a remodelled column
    (array -> jsonb, money -> numeric) had its value sent in the source type and Aurora DSQL
    rejected it with an opaque driver error.
    """
    from dsql_migrator.core.converter import (
    ...
    ddl = (
        ...
        '  "tags" JSONB,\n'
        '  "price_band" TEXT,\n'
        '  "line_total" NUMERIC(12,2) GENERATED ALWAYS AS ((1)::numeric) STORED,\n'
        '  PRIMARY KEY ("id")\n'
        ")"
    )
    assert parse_target_column_types(ddl) == {
        "id": "bigint",
        "tags": "jsonb",
        "price_band": "text",
        "line_total": "decimal",
    ...
    # The ALWAYS spelling and a CACHE-less clause are stripped too.
    for clause in (
        "GENERATED ALWAYS AS IDENTITY (CACHE 1)",
        "GENERATED BY DEFAULT AS IDENTITY",
        "GENERATED ALWAYS AS IDENTITY",
    ):
        variant = f'CREATE TABLE t ("id" BIGINT NOT NULL {clause}, "c" TEXT, PRIMARY KEY ("id"))'
        assert parse_target_column_types(variant).get("c") == "text", clause
        assert parse_target_primary_key(variant) == ["id"], clause
