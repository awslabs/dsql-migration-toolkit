...
# ---------------------------------------------------------------------------
# Dirty-check coverage of the Schema-Conversion edits
# ---------------------------------------------------------------------------


def test_session_signature_changes_when_an_edited_ddl_is_stored() -> None:
    """The persist dirty-check must see an edit-only change.

    ``capture_session_snapshot`` records ``edited_target_ddls``, but the signature used
    to omit it, so ``_persist_session`` skipped the write and the edit was lost on a task
    replacement. The per-table PK-strategy picker stores its answer ONLY here (and the
    cut-over "Sync identity sequences" gate is derived from it), so choosing a
    server-generated IDENTITY key silently reverted.
    """
    session, eval_state, conv_state, migration_state = _populated_states()

    before = session_signature(session, eval_state, conv_state, migration_state)
    conv_state.set_edited_target_ddl(
        "migration_demo.customers",
        'CREATE TABLE "migration_demo"."customers" ('
...
def test_session_signature_changes_when_an_edited_ddl_is_rewritten() -> None:
    # Same table, different DDL (e.g. switching PK strategy again): the key set is
    # unchanged, so only the content term can catch it.
    session, eval_state, conv_state, migration_state = _populated_states()
    conv_state.set_edited_target_ddl("migration_demo.orders", "CREATE TABLE a (x int)")

    before = session_signature(session, eval_state, conv_state, migration_state)
    conv_state.set_edited_target_ddl("migration_demo.orders", "CREATE TABLE a (y int)")
    after = session_signature(session, eval_state, conv_state, migration_state)

    assert after != before


def test_session_signature_changes_when_preserve_foreign_keys_is_turned_off() -> None:
    # Stripping FKs is a deliberate, hard-to-notice-if-lost choice: restoring it as True
    # would re-offer foreign keys the operator chose to drop.
    session, eval_state, conv_state, migration_state = _populated_states()
...
def test_session_signature_changes_when_ticked_nodes_change() -> None:
    session, eval_state, conv_state, migration_state = _populated_states()

    before = session_signature(session, eval_state, conv_state, migration_state)
    conv_state.ticked_node_ids = ["table:orders"]
    after = session_signature(session, eval_state, conv_state, migration_state)

    assert after != before


def test_session_signature_is_stable_across_repeated_calls() -> None:
    # The new terms must not make every poll look dirty (the signature exists so a large
    # inventory is not re-serialized on every refresh). ``hash`` of a frozenset of the
    # same (key, ddl) pairs is stable within a process.
    session, eval_state, conv_state, migration_state = _populated_states()

    first = session_signature(session, eval_state, conv_state, migration_state)
    second = session_signature(session, eval_state, conv_state, migration_state)

    assert first == second


def test_restored_snapshot_still_gates_the_cutover_actions() -> None:
    """A restore must keep the cut-over FK / identity actions visible.

    The two gates count re-converted schema output, so if a restore dropped the
    inventory or the conversion edits the actions would silently vanish on the screen
    that decides whether referential integrity is enforced. Nothing covered this before,
    which is why such a regression would have shipped green.
    """
    ...
    from dsql_migrator.ui.validation import (
        _cutover_gate_inputs_missing,
        _cutover_pending_foreign_keys,
    )

    session, eval_state, conv_state, migration_state = _populated_states()
    # A real foreign key, so the count under test is NON-zero -- otherwise the
    # comparison below would pass on 0 == 0 and prove nothing.
    inventory = _inventory()
    ...
    baseline = _cutover_pending_foreign_keys(
        session,
        SimpleNamespace(get_or_create=lambda _sid: eval_state),
        SimpleNamespace(get=lambda _sid: conv_state),
        "S1",
    )
    assert baseline > 0, "the fixture must have a foreign key for this to mean anything"

    snapshot = capture_session_snapshot(
        "S1", session, eval_state, conv_state, migration_state
    )

    # Through the real serialization boundary, onto FRESH state objects.
    revived = SessionSnapshot.model_validate_json(snapshot.model_dump_json())
    r_session = SessionConnectionState()
    r_eval = EvaluationState()
    r_conv = SchemaConversionState()
    r_migration = DataMigrationState()
    apply_session_snapshot(revived, r_session, r_eval, r_conv, r_migration)

    stores = (
        SimpleNamespace(get_or_create=lambda _sid: r_eval),
        SimpleNamespace(get=lambda _sid: r_conv),
    )
    assert _cutover_gate_inputs_missing(stores[0], stores[1], "S1") is False
    assert _cutover_pending_foreign_keys(r_session, stores[0], stores[1], "S1") == baseline

    # And with NO restore, the same gate reports indeterminate rather than 0-as-done.
    assert _cutover_gate_inputs_missing(
        SimpleNamespace(get_or_create=lambda _sid: EvaluationState()),
