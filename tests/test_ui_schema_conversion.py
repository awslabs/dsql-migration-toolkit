...
# ---------------------------------------------------------------------------
# The rendered SOURCE DDL must reproduce the source, not a flattened version
# ---------------------------------------------------------------------------


def test_the_source_ddl_shows_the_identity_clause() -> None:
    """Omitting it hid the one change that matters most on these tables.

    An identity column has NO pg_attrdef default, so the DEFAULT branch cannot stand in
    for it: `id integer GENERATED BY DEFAULT AS IDENTITY` rendered as plain
    `"id" integer NOT NULL` — byte-identical to the converted target, so the side-by-side
    diff showed NO difference on the column whose value generation does not survive. The
    MySQL renderer has always emitted AUTO_INCREMENT for exactly this reason.
    """
    from dsql_migrator.core.models import ColumnDef, TableDef
    ...
    table = TableDef(
        ...
        columns=[
            ColumnDef(
                name="id", mysql_type="integer", nullable=False,
                identity=True, identity_generation="BY DEFAULT",
            ),
            ColumnDef(
                name="strict", mysql_type="integer", nullable=False,
                identity=True, identity_generation="ALWAYS",
            ),
            ColumnDef(name="plain", mysql_type="integer", nullable=False),
        ],
    )
    ddl = _render_source_table_ddl_postgres(table)
    assert '"id" integer NOT NULL GENERATED BY DEFAULT AS IDENTITY' in ddl, ddl
    # The VARIANT is user-visible: ALWAYS rejects an explicit INSERT value.
    assert '"strict" integer NOT NULL GENERATED ALWAYS AS IDENTITY' in ddl, ddl
    # An ordinary column gains nothing.
    assert '"plain" integer NOT NULL,' in ddl, ddl


def test_an_identity_column_with_no_recorded_variant_still_shows_the_clause() -> None:
    # Older persisted inventories carry only the boolean; the clause must still appear
    # rather than silently reverting to the flattened rendering.
    from dsql_migrator.core.models import ColumnDef, TableDef
    from dsql_migrator.ui.schema_conversion import _render_source_table_ddl_postgres

    ddl = _render_source_table_ddl_postgres(
        TableDef(
            name="t", primary_key=["id"],
            columns=[ColumnDef(name="id", mysql_type="integer", nullable=False, identity=True)],
        )
    )
    assert "AS IDENTITY" in ddl, ddl


def test_the_source_ddl_shows_the_generated_columns_expression() -> None:
    """Evaluation told the operator to read it from the source; the tool discarded it.

    ``attgenerated`` only says a column IS generated, so the panel printed
    "expression not captured" — leaving the operator with no way to follow the very advice
    the finding gave them.
    """
    from dsql_migrator.core.models import ColumnDef, TableDef
    from dsql_migrator.ui.schema_conversion import _render_source_table_ddl_postgres
    ...
    ddl = _render_source_table_ddl_postgres(
        TableDef(
            ...
            columns=[
                ColumnDef(name="id", mysql_type="bigint", nullable=False),
                ColumnDef(
                    name="line_total", mysql_type="numeric(12,2)", generated=True,
                    generated_expression="((quantity)::numeric * unit_price)",
                ),
            ],
        )
    )
    assert "GENERATED ALWAYS AS (((quantity)::numeric * unit_price)) STORED" in ddl, ddl
    assert "not captured" not in ddl, ddl

    # Without the expression it degrades to the honest placeholder, not a broken clause.
    ddl2 = _render_source_table_ddl_postgres(
        TableDef(
            name="t", primary_key=["id"],
            columns=[
                ColumnDef(name="id", mysql_type="bigint", nullable=False),
                ColumnDef(name="c", mysql_type="numeric(12,2)", generated=True),
            ],
        )
    )
    assert "expression not captured" in ddl2, ddl2
    assert "GENERATED ALWAYS AS ()" not in ddl2, ddl2


def test_the_pg_collector_reads_both_facts_from_the_catalog() -> None:
    """Both were available and discarded -- pin the SQL so they stay read."""
    import inspect

    from dsql_migrator.core.source_dialect import postgres as pg

    src = inspect.getsource(pg)
    # The identity variant, not just the boolean.
    assert 'column.identity_generation = (' in src, "the ALWAYS/BY DEFAULT variant is dropped again"
    assert '"ALWAYS" if resolved[2] == "a" else "BY DEFAULT"' in src
    # The generation expression, via pg_attrdef.
    assert "pg_get_expr(ad.adbin, ad.adrelid)" in src, "the expression is not selected"
    assert "LEFT JOIN pg_attrdef ad" in src, "pg_attrdef is not joined"
    assert "column.generated_expression = str(resolved[5])" in src


def test_pk_picker_renders_the_strategy_the_converter_actually_emits() -> None:
    """With nothing stored, the picker must show the per-engine DEFAULT, not always Keep.

    The picker derives its state from the stored (edited) target DDL and read "nothing
    stored" as Keep. That was right while KEEP_INTEGER was the only default; for a
    PostgreSQL source the converter now defaults to a server-generated identity, so the
    unchanged screen would have shown "Keep source PK" selected beside a DDL panel
    containing ``GENERATED BY DEFAULT AS IDENTITY`` -- the two surfaces disagreeing about
    ...
    # PostgreSQL: identity is the default, so that tile is the selected one.
    ui = _PickerUi()
    _render_pk_strategy_picker(
        ui, table, SchemaConversionState(), lambda: None, source_type=SourceType.POSTGRES
    )
    # Asserted through the notice the picker renders ONLY for the identity state -- the
    # tile chrome (border/radio glyph) is not observable through the double.
    assert "expect gaps and loose ordering" in ui.body()
    assert "becomes a cached identity" in ui.body()

    # MySQL: unchanged -- Keep is the default, so that notice must NOT appear.
    ui = _PickerUi()
    _render_pk_strategy_picker(
        ui, table, SchemaConversionState(), lambda: None, source_type=SourceType.MYSQL
    )
    assert "expect gaps and loose ordering" not in ui.body(), ui.body()


def test_pk_picker_keep_is_stored_when_the_default_would_generate_the_key() -> None:
    """Choosing Keep must PRODUCE Keep, which now means storing an explicit script.

    Keep used to be expressed by CLEARING the override -- correct only while clearing fell
    back to KEEP_INTEGER. On a PostgreSQL source the no-override path is now the identity,
    so clearing would have handed the operator the very strategy they just opted out of.
    """
    from dsql_migrator.ui.schema_conversion import (
        _render_pk_strategy_picker,
        identity_from_ddl,
    ...
    ui = _PickerUi()
    _render_pk_strategy_picker(
        ui, table, state, lambda: None, source_type=SourceType.POSTGRES
    )
    ui.tile_clicks[0]()  # the "Keep source PK" tile
    stored = state.get_edited_target_ddl("orders")
    assert stored is not None, "Keep on a PG identity key must store its own script"
    assert "IDENTITY" not in stored, stored
    assert identity_from_ddl(table, stored) is False

    # MySQL, where the default already keeps the key: clearing is still correct, so no
    # pointless override is written.
    state = SchemaConversionState()
    ui = _PickerUi()
    _render_pk_strategy_picker(
        ui, table, state, lambda: None, source_type=SourceType.MYSQL
    )
    ui.tile_clicks[0]()
    assert state.get_edited_target_ddl("orders") is None


def test_keep_tile_does_not_promise_no_application_change_for_a_generated_key() -> None:
    """The DEFAULT tile claimed "Nothing in the application changes" on every table.

    For a key the SOURCE generates, keeping the column as-is is exactly what drops that
    generator (DSQL can only generate into a bigint identity), so the claim contradicted
    the conversion warning rendered beside it -- and a real operator read the tile,
    concluded the identity option was redundant, and proposed disabling it.
    """
    from dsql_migrator.ui.schema_conversion import _render_pk_strategy_picker

    generated = _int_auto_inc_table()
    ui = _PickerUi()
    _render_pk_strategy_picker(ui, generated, SchemaConversionState(), lambda: None)
    body = ui.body()
    assert "Nothing in the application changes" not in body, body
    assert "DSQL will NOT fill it" in body, body
    assert "must supply the value on every insert" in body, body

    # On a table with no generated key, "nothing changes" is TRUE and must stay.
    plain = TableDef(
        name="regions",
        columns=[
            ColumnDef(name="code", mysql_type="varchar(8)", nullable=False),
            ColumnDef(name="name", mysql_type="varchar(64)", nullable=False),
        ],
        primary_key=["code"],
    )
    ui = _PickerUi()
