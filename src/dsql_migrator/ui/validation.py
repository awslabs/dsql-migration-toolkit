    ...
    def runner() -> None:
        ...
        def work(handle: object) -> None:
            ...
            if ai_post_event is not None:
                ai_post_event(
                    text=(
                        f"Started validation: {inputs.mode.value} comparison of "
                        f"{len(scoped_tables)} table(s) (source vs target)"
                    ),
                    status="started",
                )
            ...
            if ai_post_event is not None:
                ai_post_event(
                    text=(
                        (
                            f"Validation complete: {_summary.matched_tables}/"
                            f"{_summary.total_tables} tables matched ({_summary.mode})"
                        )
                        if _summary.is_match
                        else (
                            f"Validation MISMATCH: {_summary.matched_tables}/"
                            f"{_summary.total_tables} matched, "
                            f"{_summary.mismatched_tables} mismatched ({_summary.mode})"
                        )
                    ),
                    status="success" if _summary.is_match else "error",
                )
    ...
    *,
    ...
    def runner() -> None:
        ...
        if ai_post_event is not None:
            ai_post_event(
                text=(
                    "Cut over acknowledged: live on Aurora DSQL ("
                    + (
                        "clean match"
                        if _release == "clean"
                        else "accepted gap: migrating without permanently-dropped rows"
                        if _accepted_gap
                        else f"release state: {_release}"
                    )
                    + ")"
