...
def _render_source_table_ddl_postgres(table: TableDef) -> str:
    ...
    for column in table.columns:
        ...
        # The IDENTITY clause. Omitting it made this panel misrepresent the source on the
        # one column that matters most: an identity column has NO pg_attrdef default, so
        # the DEFAULT branch above cannot stand in for it, and `id integer GENERATED BY
        # DEFAULT AS IDENTITY PRIMARY KEY` rendered as plain `"id" integer NOT NULL` --
        # identical to the target, so the side-by-side diff showed NO change on the column
        # whose value generation does not survive. (The MySQL renderer has always emitted
        # AUTO_INCREMENT for exactly this reason.)
        if column.identity:
            spelling = column.identity_generation or "BY DEFAULT"
            clause += f" GENERATED {spelling} AS IDENTITY"
        if column.generated:
            # Show the EXPRESSION when introspection captured it. Printing only
            # "expression not captured" while Evaluation told the operator to read that
...
def _identity_eligible(table: TableDef) -> bool:
    ...
    Delegates to :func:`converter.identity_strategy_applies` -- the converter both
    APPLIES the strategy and now DEFAULTS to it, so the rule has to live with the
    converter; the picker owning a private copy is how the two surfaces came to
    disagree about the same table.
    """
    return identity_strategy_applies(table)
...
def _render_pk_strategy_picker(
    ...
    *,
    ...
    ``source_type`` selects the word for the SOURCE mechanism. The converter's warnings and
    Evaluation were made engine-correct earlier, but this PICKER -- where the operator
    actually makes the decision -- still named AUTO_INCREMENT to a PostgreSQL user whose
    database has no such thing, and its "no alternative applies" hint explained the absence
    in MySQL terms, so a PG user with a non-serial key could not tell why IDENTITY was
    withheld.
    ...
    * Composite key -- prepends a high-cardinality column to the key, requiring the
    ...
    if not table.primary_key:
        return
    # The source mechanism's name. MySQL's strings stay byte-identical.
    _key_word = (
        "serial / identity" if source_type is SourceType.POSTGRES else "AUTO_INCREMENT"
    )
    candidates = composite_leading_candidates(table)
    identity_ok = _identity_eligible(table)
    # What this table gets with NOTHING stored. Not the same as identity_ok: a MySQL
    # `bigint unsigned` key is eligible for the tile (an operator may accept the
    # narrowing) but is NOT defaulted to it, because the widening to bigint could drop
    # an existing row. The picker has to render the strategy that is actually emitted.
    identity_by_default = (
        default_primary_key_strategy(
            table, is_postgres=source_type is SourceType.POSTGRES
        )
    ...
    is_identity = not is_composite and (
        # A stored script is the single source of truth for the picker's state; with
        # NOTHING stored the table takes the converter's DEFAULT, which is now
        # IDENTITY_WITH_CACHE wherever it applies (it preserves the source's value
        # generation). Reading "nothing stored" as Keep would show the operator a tile
        # that contradicts the DDL beside it.
        identity_from_ddl(table, stored) if stored is not None else identity_by_default
    )
    ...
    def _select_strategy(choice: str) -> None:
        ...
        elif identity_by_default:
            # Keep has to be STORED now: clearing the override would fall back to the
            # converter's default, which for this table IS the identity -- so "Keep"
            # would silently produce the very thing the operator just opted out of.
            conv_state.set_edited_target_ddl(
                name, render_target_ddl(build_keep_conversion(converter, table))
            )
        else:
            # No generated key, so the default conversion already keeps the source key:
            # drop the override and use the deterministic conversion again.
    ...
    tiles: list[tuple[str, str, str, str]] = [
        (
            ...
            "the source. "
            + (
                # The source GENERATES this key, and keeping the column as-is is
                # precisely what drops that generator (DSQL can only generate into a
                # bigint identity). Saying "nothing changes" here contradicted the
                # conversion warning shown beside it -- on the DEFAULT choice.
                "The column type is unchanged, but DSQL will NOT fill it: the "
                "application must supply the value on every insert."
                if table.auto_increment_column
                else "Nothing in the application changes."
            ),
        )
    ]
    if identity_ok:
        tiles.append(
            (
                ...
                f"{_key_word}: the loader keeps the source ids; gaps/loose order "
                "after.",
            )
        )
    if candidates:
    ...
    with ui.card().classes("w-full !shadow-none border border-gray-200 bg-gray-50 p-3 gap-2"):  # type: ignore[attr-defined]
        ...
        if is_composite:
            ...
            inline_hint(
                ...
                f"single-column {_key_word} key and no NOT NULL non-key column to "
