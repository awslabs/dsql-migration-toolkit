...
def _cdc_recovery_context(source_engine: str = "MySQL") -> str:
    ...
    "- OVERSIZED VALUE (error_code OVERSIZED_VALUE, no SQLSTATE): a column's value "
    "exceeds Aurora DSQL's 1 MiB per-value limit, so the sink refuses the row BEFORE any "
    "write. This row can NEVER be stored as-is and is NEVER retried -- do not suggest "
    "retrying, re-applying or raising a limit. The fix is to stop capturing that column "
    "and keep the value outside the database (e.g. Amazon S3, storing a URI).\n"
    "- NEVER tell the operator to hand-edit connector configuration. THIS TOOL OWNS the "
    "Debezium connector's config: it writes column.exclude.list itself from the "
    "migration's LOB column exclusion, and re-sends it on EVERY Start CDC (so the change "
    "reaches a RUNNING pipeline -- no cdc-stack teardown and no manual connector "
    "restart). Advice like 'add \"column.exclude.list\": \"db.table.col\" to the source "
    "connector and restart it' is both unnecessary and not doable from this UI. Name the "
    "IN-APP control instead: the Exclude-column action on the quarantine card (Data "
    "Migration step), which opens a picker of the columns that can exceed the limit.\n"
    "- The ORDERED in-app runbook for an oversized-value quarantine, and the order "
    "matters: (1) exclude the column -- note this is MIGRATION-WIDE, so the Full Load, "
    "the prerequisite gate and the Validation comparison all stop covering that column "
    "too, and it arrives NULL for every row; (2) Stop CDC -- a reload while the sink is "
    "live is forced into APPEND mode, so the rows already on the target would KEEP the "
    "old column set; (3) Reload the affected table, choosing DROP so it is recreated "
    "without the column; (4) Start CDC again -- it re-sends the new column.exclude.list "
    "and resumes from the new snapshot point.\n"
    "- If the operator cannot drop the column (the data must migrate), the value itself "
    "has to shrink below 1 MiB on the SOURCE before the reload; there is no target-side "
