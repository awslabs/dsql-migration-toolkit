        ...
        for column in table.columns:
            ...
            if resolved:
                ...
                if resolved[1] in ("s", "v"):  # 's'=STORED, 'v'=VIRTUAL (PG18+)
                    ...
                    # Carry the expression, so the source panel can show what the column
                    # computes and the operator can re-implement it. DSQL has no generated
                    # columns, so this value is never emitted into target DDL.
