...
class PrimaryKeyStrategy(str, Enum):
    ...
    - ``KEEP_INTEGER``: keep the integer key column unchanged -- SAME TYPE, NO
      GENERATOR. The default for a MySQL source (``AUTO_INCREMENT`` is not an identity
      column, so converting it to one is the operator's call). Note what it costs on
      either engine: on Aurora DSQL an identity must be ``bigint``, so preserving the
      column's exact TYPE and preserving its value GENERATION are mutually exclusive --
      keeping the type means the APPLICATION has to supply the key on every insert, or
      its inserts collide.
    ...
    - ``IDENTITY_WITH_CACHE``: the default for a PostgreSQL source (whose column
      already IS an identity / ``serial``), opt-in for MySQL. The faithful port of a
      source generated key:
      the key becomes ``bigint ... GENERATED BY DEFAULT AS IDENTITY (CACHE n)`` so the
      TARGET keeps generating it, with a per-node cache so concurrent inserts do not
      contend on one key range. ``bigint`` and an explicit ``CACHE`` are both required
      by Aurora DSQL, and ``BY DEFAULT`` keeps the data path unchanged (Full Load
      inserts the source's own ids, then the sequence is advanced past them). Applies
      only when the generated column is the whole primary key
      (:func:`identity_strategy_applies`); otherwise it falls back to
      ``KEEP_INTEGER``.
...
def identity_strategy_applies(table: TableDef) -> bool:
    """Return whether ``IDENTITY_WITH_CACHE`` can be applied to ``table``'s key.

    Only when the generated column IS the whole primary key: that is the column the
    strategy rewrites, and a DSQL identity attaches to a single column. A composite
    source key (e.g. ``PRIMARY KEY (id, occurred_at)`` over a serial ``id``) is out of
    scope -- ``_apply_pk_strategy`` falls back to ``KEEP_INTEGER`` there and says so,
    and the Schema Conversion picker does not offer the tile.

    Shared deliberately: the picker used to own this rule as a private helper while the
    converter applied the strategy unconditionally, so the two could disagree about the
    same table. One predicate, imported by both.
    """
    column = table.auto_increment_column
    return bool(column) and table.primary_key == [column]


def default_primary_key_strategy(
    table: TableDef, *, is_postgres: bool = False
) -> PrimaryKeyStrategy:
    """Return the strategy to use when the caller did not choose one.

    PostgreSQL: ``IDENTITY_WITH_CACHE`` wherever it applies, because the source column
    already IS an identity (or a ``serial``, the same mechanism spelled differently), so
    an identity on the target is the faithful port -- and keeping the column's exact TYPE
    instead is what DROPS the generation, since an Aurora DSQL identity must be
    ``bigint``. A PG serial / identity key is always int2/int4/int8, so that widening is
    always lossless.

    MySQL: ``KEEP_INTEGER``, unchanged. ``AUTO_INCREMENT`` is not an identity column --
    it is a different mechanism with different semantics -- so converting it to one is a
    deliberate choice the operator makes in Schema Conversion, not something the tool
    does on their behalf. (It is also the engine where the widening can COST something:
    ``bigint unsigned`` maps to ``numeric(20, 0)`` to preserve its range, and an identity
    must be ``bigint``, so a value above 2^63-1 would fail to load.)

    Resolved PER TABLE, so one global option cannot force an identity onto a composite
    key.
    """
    if not is_postgres:
        return PrimaryKeyStrategy.KEEP_INTEGER
    if not identity_strategy_applies(table):
        return PrimaryKeyStrategy.KEEP_INTEGER
    return PrimaryKeyStrategy.IDENTITY_WITH_CACHE


class SchemaConvertOptions(BaseModel):
    """Options controlling schema conversion.

    ``primary_key_strategy`` controls how a monotonic generated primary key
    (``AUTO_INCREMENT`` / ``serial`` / ``GENERATED AS IDENTITY``) is rewritten for DSQL
    (Requirement 3.5). Leave it UNSET to get the per-table, per-engine default from
    :func:`default_primary_key_strategy`: ``IDENTITY_WITH_CACHE`` for a PostgreSQL key
    that already is an identity / ``serial`` (preserving the generation the source has),
    ``KEEP_INTEGER`` otherwise. Setting it explicitly overrides that for every table --
    which is what the Schema Conversion picker does per table.
    ...
    primary_key_strategy: PrimaryKeyStrategy = Field(
        ...
        description=(
            "How to handle a monotonic AUTO_INCREMENT / serial primary key. Leave "
            "UNSET for the per-engine, per-table default (see "
            "default_primary_key_strategy); this value is the fallback for a caller "
            "that reads the field without setting it."
        ),
    )
...
def _apply_pk_strategy(
    ...
    *,
    ...
    if not column_name:
        return []
    # IDENTITY is now the DEFAULT, so it reaches every table with a generated key --
    # including one whose key is composite, where a DSQL identity does not apply. Fall
    # back to KEEP there (the message below explains why the strategy is unavailable)
    # rather than emitting an identity the Schema Conversion picker never offers.
    if strategy is PrimaryKeyStrategy.IDENTITY_WITH_CACHE and not identity_strategy_applies(
        table
    ):
        strategy = PrimaryKeyStrategy.KEEP_INTEGER
    ...
    if strategy is PrimaryKeyStrategy.CONVERT_TO_UUID:
        ...
        if column_def is not None:
            ...
            column_def.set("constraints", constraints)
        message = (
            f"The {key_word} column '{column_name}' KEEPS its value generation on "
            "Aurora DSQL: it becomes a cached identity "
            f"(bigint, GENERATED BY DEFAULT AS IDENTITY, CACHE {_IDENTITY_CACHE_SIZE}), "
            "so the target fills the key exactly as the source did and the application "
            "needs no change. The column is widened to bigint because Aurora DSQL "
            "supports an identity only on bigint -- lossless for an integer key. Two "
            "consequences to expect: each DSQL node draws its own block of values, so "
            "the key is no longer gap-free or strictly increasing (reconsider if the "
            "application treats it as a sequential number, e.g. an invoice number), and "
            "that same cache is what spreads inserts instead of concentrating them on "
            "one partition (DSQL stores rows in primary-key order). Full Load inserts "
            "the source's own ids and the sequence is advanced past them afterwards, so "
            "later application inserts do not collide."
        )
    else:  # PrimaryKeyStrategy.KEEP_INTEGER
        message = (
            f"The integer key from {key_word} column '{column_name}' was kept as a "
            "plain integer, WITHOUT the source's value generation. IMPORTANT: Aurora "
            "DSQL will NOT auto-generate this key after "
            ...
            + (
                ...
                else "The 'Server-generated (IDENTITY)' strategy cannot be used for this "
        ...
        if options.primary_key_strategy is PrimaryKeyStrategy.COMPOSITE_KEY:
            ...
            pk_strategy_warnings.extend(
                _apply_pk_strategy(
                    create,
                    table,
                    # An UNSET strategy resolves per table (identity where it applies
                    # and is lossless, keep otherwise). An explicitly-set one is
                    # honoured as given -- the operator's choice, not a default.
                    (
                        options.primary_key_strategy
                        if "primary_key_strategy" in options.model_fields_set
                        else default_primary_key_strategy(
                            table, is_postgres=is_postgres
                        )
                    ),
