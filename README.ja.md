...
設定は (config ファイルではなく) 環境変数から読み取られます。認証情報の値が設定に永続化されることは
一切ありません。Fargate では、これらは ECS タスク定義のコンテナ `environment` に設定します。Full Load /
Validation の並列度 4 つは、サイドバーフッターの **Performance tuning** コントロールから **実行時に**
再調整することもできます — 再デプロイ不要で、次の Full Load / Validation に反映され、再起動でリセットされます。
