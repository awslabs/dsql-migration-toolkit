...
への変換、次に PostgreSQL → DSQL 固有の制約（楽観的同時実行制御、
...
引き継ぎがギャップなく保たれる仕組みはエンジンごとに異なります。MySQL のバイナリログは
コンシューマーがいなくても履歴を保持するため、CDC を後から接続してもウォーターマークから
再開できます。PostgreSQL は**論理レプリケーションスロット**に対してのみ WAL を保持し、
スロットを過去の位置に作成することはできないため、スロットは読み込みの*前*に存在していな
ければなりません。**Full load + CDC** を選べば、ツールがスナップショット時点でスロットを
作成します。**Full Load のみ**を実行した後はその選択肢が失われ、後から CDC を開始すると
テーブルを再度スナップショットします（依然としてギャップはありませんが、ソースをもう一度
読み込みます）。
...
- **スキーマ変換** — ソーススキーマを DSQL DDL に変換（型マッピング、外部キー保持（ロード後に再作成）、非同期
...
- **DSQL の制約をそのまま継承** — トリガー・ストアドプロシージャ非対応、
