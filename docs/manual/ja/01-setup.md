...
- ネットワーク経由で到達できるソースの **Amazon RDS または Aurora MySQL** データベース。
  ...
  ソースに書き込むことは一切ありません)。
  - **サポートされるソースエンジンとバージョン**(Full Load + CDC + チェックサム検証まで
    エンドツーエンドで検証済み):**RDS for MySQL** 5.7 / 8.0 / 8.4、**Aurora MySQL**
    5.7(v2) / 8.0(v3) / 8.4。MySQL 5.7 は標準サポートが終了しています(RDS/Aurora の
    Extended Support が適用される場合あり)が、ツールは移行ソースとして 5.7 を完全にサポートします。
