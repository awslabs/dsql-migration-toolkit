...
| **1 値あたりのサイズ上限（型ごとに異なる）** | **バイナリ**値は 1 MiB を超えると保存できません（`bytea`、MySQL の `BLOB`、PostgreSQL の `bytea`）— DSQL がその場で拒否します。**テキスト**値に 1 MiB の上限はなく、書き込みトランザクションあたり 10 MiB の上限によって制限されます（9.5 MiB まで正常に保存されることを実測）。 | 大きすぎる**バイナリ**値はエラーログ / DLQ に**隔離**（quarantine）され、同じサイズのテキスト値は**通常どおり転送**されます。約 8 MiB を超える列は Kafka のメッセージ上限のため**キャプチャ時に除外**する必要があります。大きな LOB 列は `OVERSIZED_LOB`（MANUAL）としてフラグ付け。 |
