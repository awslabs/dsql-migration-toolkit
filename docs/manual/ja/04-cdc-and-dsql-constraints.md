...
<p align="center">
  <img src="../../images/architecture-cdc-pipeline.png" alt="CDC パイプライン: ソース変更ログ（MySQL binlog / PostgreSQL の論理レプリケーションスロット経由の WAL）→ Debezium ソースコネクタ → Amazon MSK（PK でキー付けしたテーブルごとのトピック + DLQ）→ カスタム DSQL シンクコネクタ → Aurora DSQL" width="900">
</p>

- **Debezium source connector（MySQL または PostgreSQL）** は、ソースの変更ログ — MySQL はバイナリログ、PostgreSQL は論理レプリケーションスロット経由の write-ahead log (WAL) — を読み込み、変更イベントを出力します。ソースは読み取り専用のままで、唯一認められた例外として、PostgreSQL ソースではツールがマイグレーション対象テーブルだけにスコープした論理スロットと publication を作成（し、解体時に削除）します。
- **Amazon MSK (Kafka)** は耐久性のあるバックボーンです。**テーブルごとに 1 トピック** を持ち、主キーでキーイングし (ある行のすべての変更が 1 つのパーティションで順序を保つ)、加えてデッドレター (DLQ) トピック、Debezium の **スキーマ履歴** トピック (`recovery` が再構築する対象 — **MySQL ソース専用** であり、PostgreSQL/pgoutput コネクタにはスキーマ履歴トピックがありません)、およびアイドル区間でもコミット済みソース位置を前進させ再起動後もギャップなく再開できるようにする **ハートビート** トピック — MySQL は binlog オフセット、PostgreSQL はスロットの確定 LSN (これは WAL の蓄積も止めます) — を備えます。
- **カスタム DSQL sink connector** — このプロジェクトが所有する Java Kafka Connect プラグイン — が変更を DSQL に適用します。両方の connector は **マネージドの MSK Connect** 上で実行され、ツール自身は **sink 用のコンピュートを一切持たず**、コントロールプレーンとして機能します (構成の作成、再開ポイントの確立 — MySQL では Kafka 開始オフセットをシードし、PostgreSQL では論理レプリケーションスロットと publication を作成 — 監視を行います)。
...
### CDC 中のソーススキーマドリフトの検出と対処

ソースの `ALTER` を自分で気づく必要はありません。デッドレターされたレコードがソースの形状変更を示すと、
ツールは SQLSTATE でドリフトを**分類**（列の追加/削除/型変更）し、CDC ステップに **「ソーススキーマ変更を
検出」** バナーを表示します。よくあるケースであるソースの **`ADD COLUMN`** については、正確な
`ALTER TABLE … ADD COLUMN` 文を示して適用する（トランザクションごとに 1 DDL）**「Fix target schema…」**
アクションを提供します。その後 CDC は再開し、列が無かった間に脇へ置かれた行はテーブル単位の **Reload** で
バックフィルします。列削除/型変更のドリフトは手動対応のために表示のみ行います（ツールはターゲットの
データを自動削除しません）。
...
1. Full Load は、スナップショット時点で **ウォーターマーク (watermark)** をキャプチャしました — MySQL は binlog の file:position (可能な場合は GTID も)、PostgreSQL は論理レプリケーションスロットを作成したときに返される WAL LSN で、スロット名・publication 名とともに記録されます ([第 3 章 §3.5](03-full-load.md#35-ウォーターマーク--cdc-への橋渡し))。
2. CDC を開始すると、ツールはまさにそのウォーターマークに再開ポイントを確立します。**MySQL ソース** では connector の **開始オフセットをシード** します (source connector が開始する前に、VPC 内の Lambda がオフセットレコードを書き込みます)。**PostgreSQL ソース** ではオフセットシーダー Lambda はありません。代わりに Full Load の整合点で、ツールは論理レプリケーションスロット (とマイグレーション対象テーブルだけにスコープした publication) を作成し、スロットが返した整合 WAL LSN がそのままウォーターマークになります — CDC はその事前作成済みスロットから直接再開するため、Kafka の connect-offsets には何も書き込まれません。いずれの場合も Debezium は **スナップショット後の最初の変更** からストリーミングを開始します — 「今」からでもなく、データを再読み込みするのでもありません。
3. **MySQL ソース** では、source connector は **`snapshot.mode=recovery`** で実行されます。オフセットがすでにシードされているため、Debezium は (binlog イベントをデコードするために) 内部の **スキーマ履歴** を **現在のソーステーブル** から再構築し、**行データを一切再読み込みせずに** シードされたオフセットから再開します。**PostgreSQL ソース** では、Debezium pgoutput コネクタにスキーマ履歴トピックはありません。ギャップのない経路では **`snapshot.mode=never`** で実行され (論理スロットが既に開始 LSN を保持し、行もロード済み)、スタンドアロン/手動の開始では **`initial`** (スナップショットしてからストリーム) を使います。Debezium PostgreSQL はスロット自身の位置からしか再開できず、任意の WAL LSN にシークすることはできません。

その結果、スナップショットと「今」の間のすべての変更が正確に 1 回だけ適用されます。sink の適用は (PK でキーイングされ) **冪等** であるため、オーバーラップやリトライがあっても重複が生じることはありません。

> **MySQL: ウォーターマーク位置の binlog は、CDC 開始時点でまだ存在している必要があります。** このハンドオフは、ソースがウォーターマーク位置のバイナリログをまだパージしていない場合にのみ機能します。RDS/Aurora はデフォルトで binlog を積極的にパージし (Aurora MySQL は 24 時間保持)、CDC スタックのデプロイには約 15〜20 分かかるため、**開始前に binlog の保持期間を延長してください** — [§1.1](01-setup.md#11-前提条件) を参照してください。該当セグメントがすでに失われている場合、CDC はギャップなく再開できないため、新しいウォーターマークを取得するために Full Load を再実行することになります。
>
> **PostgreSQL: 論理レプリケーションスロットが代わりに WAL を固定します。** スロットが作成された瞬間から必要な WAL を保持するため、足元で何かがパージされて失われることはありません — 前提条件は binlog の保持ではなく `wal_level=logical` (RDS/Aurora は `rds.logical_replication`) です。ただしスロットは健全に保つ必要があります: 遅れが大きくなりすぎて Postgres が `wal_status='lost'` (スロット無効化) を報告すると、ギャップのない再開が壊れるため、新しいウォーターマークを取得するために Full Load を再実行する必要があります。

> **なぜ単純な schema-only 開始ではなく `recovery` なのか。(MySQL ソース)** シードされたオフセットが存在すると、Debezium は「再開」パスをたどり、既存のスキーマ履歴トピックを期待します。`recovery` は、行を再スナップショットせずにその履歴をライブのデータベースから再構築するモードです — まさに「データは自分ですでにロード済みなので、このオフセットから再開するだけでよい」という状況に合致します。**これは PostgreSQL ソースには当てはまりません:** PG にはスキーマ履歴トピックがないため、Debezium は事前作成済みの論理スロットから `snapshot.mode=never` で単純に再開します (スタンドアロン開始の場合は `initial` スナップショット)。
...
## 4.5 値ごとのサイズ上限 (型により異なる)、および DLQ

DSQL の値あたりの上限は **型によって異なります**。**バイナリ値 (`bytea`) は 1 MiB** が上限で、これを超えると `ProgramLimitExceeded` (SQLSTATE 54000) で拒否されます。一方 **`text` 値に 1 MiB の上限はなく**、書き込みトランザクションあたり 10 MiB の上限によって制限されます (9.5 MiB まで正常に保存されることを実測)。パイプラインは特大サイズの値を **3 つの帯域** に分けて処理します。

| 値のサイズ | 処理内容 |
|---|---|
| **≤ 1 MiB** | 通常どおり適用されます。 |
| **1 MiB – 8 MiB** | **バイナリ (`bytea`) 値のみ** が隔離対象です。sink は **書き込み前に** 各値を測定し、1 MiB を超えるバイナリ値を **DLQ** に **隔離 (quarantine)** します (適用されることは決してありません)。一方でそのレコードのテーブルの残りの値はそのまま流れ続けます。**テキスト / JSON 値には 1 MiB の上限がないため、この帯域でも通常どおり適用されます** (境界はトランザクションあたりのバイト予算 8 MiB)。そのようなレコードが Kafka を通過できるように (バイナリ値のデッドレター化、および大きなテキスト値の転送のため)、テーブルごとのトピックとクライアントの上限が引き上げられます (デフォルト 4 MiB、最大 8 MiB)。 |
...
| 値をその型の上限内 (`bytea` なら 1 MiB 未満) に縮める `UPDATE` | Debezium が **after-image 全体** を送り、sink が `INSERT … ON CONFLICT (pk) DO UPDATE` を書きます。既存行が無いため `ON CONFLICT` は発火せず、行は **INSERT** されます。 | **ギャップが自己修復します。** 対応不要。 |
...
| **Quarantined** | **DLQ** に隔離された変更イベントの、テーブルごとの件数 — 恒久的に拒否された行(型の不一致、型ごとの上限を超える特大サイズの値(`bytea` は 1 MiB 超、`text` はトランザクション上限に収まらない 8 MiB 超)、制約 / スキーマドリフト)。シンクがライブ報告。 |
| **Source rows (est.)** | スキャン不要のカタログ **見積り** (MySQL は `information_schema.tables`、PostgreSQL は `pg_class.reltuples`)。**Target rows** — DSQL の正確なカウント。 |
...
送出します。モニターは、最近のウィンドウで観測された **最悪 (最大) の遅延** を表示します (直近の単一値では
ありません)。
...
### パイプラインの健全性と change flow

テーブル別テーブルの上に、**パイプライン健全性カード**が各コネクタの状態と **「change flow」** ステータス行
を表示します — *Streaming*、*No changes flowing — idle*、または **「Sink stalled — changes are NOT reaching
DSQL」** — ソース poll とシンク send の rec/s ゲージ 2 つとともに、アイドルなストリームと詰まったストリームを
一目で見分けられます。**ライブ stream-lag チャート**は最悪のエンドツーエンド遅延を時系列で描き、「caught up」
と確定したシンク stall を区別します。

### DLQ を調べる

インタラクティブな **DLQ インスペクター**が、デッドレターされたレコードを **Time / Table / SQLSTATE /
Reason** で一覧します — ページング・フィルタ対応、テーブル別の内訳と depth バッジ付き — UI を離れずに poison
行をトリアージできます。**「Download CDC error log」** で NDJSON としてエクスポートします。(Reason には SQL
テンプレートが含まれ、行の値は含まれません — §4.5 参照。)
