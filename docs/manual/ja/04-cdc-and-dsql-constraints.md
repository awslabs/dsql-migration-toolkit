...
## 4.7 CDC 進捗のモニタリング (マイグレーションモニター)
...
| **Net rows since Full Load** | Full Load 以降に CDC が適用した**正味(net)**行数 — CDC イベント数では*ありません*: insert は加算、delete は減算、update は変化なし。シンクがライブ報告(スキャン不要)。ストリームが正味削除(insert より delete が多い)した場合は**負**になり、これはエラーではなく想定内です。 |
