...
2. CDC を開始すると、ツールはまさにそのウォーターマークに再開ポイントを確立します。**MySQL ソース** では connector の **開始オフセットをシード** します (source connector が開始する前に、VPC 内の Lambda がオフセットレコードを書き込みます)。**PostgreSQL ソース** ではオフセットシーダー Lambda はありません。代わりに Full Load の整合点で、ツールは論理レプリケーションスロット (とマイグレーション対象テーブルだけにスコープした publication) を作成し、スロットが返した整合 WAL LSN がそのままウォーターマークになります — CDC はその事前作成済みスロットから直接再開するため、Kafka の connect-offsets には何も書き込まれません。いずれの場合も Debezium は **スナップショット後の最初の変更** からストリーミングを開始します — 「今」からでもなく、データを再読み込みするのでもありません。
   ...
   > **これは `Full load + CDC` 実行についての説明です。** 再読み込みのない引き継ぎを可能にするのは、そのロード*中*に作成されたスロットであり、PostgreSQL のスロットは過去の位置に作成できません。**`Full Load のみ`**を実行して後から CDC に戻る場合、ストリーミングは依然として無損失ですが、source connector が自前の publication とスロットを作成し、**選択したすべてのテーブルを先に再スナップショット**します — データはもう一度読み込まれます。選び方は [第 10 章 §10.1](10-conclusion.md) を参照してください。MySQL はコンシューマーがいなくてもバイナリログが履歴を保持するため、後から CDC を開始してもウォーターマークから再読み込みなしで再開できます。
...
| **> 8 MiB** | Kafka にまったく入ることができません。これらは **キャプチャ時に除外** する必要があります。Debezium の `column.exclude.list` が特大サイズの LOB 列をドロップし、パイプラインに到達しないようにします。どの列をドロップするかは Data Migration / CDC ステップの特大 LOB カードで選びます — Evaluation が `OVERSIZED_LOB` としてフラグ付けするのと同じ集合です。 |
