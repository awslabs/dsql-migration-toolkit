...
8 パーティションでシンクは ~1,500 rows/s に到達（DSQL 適用のクロスチェックは 1,484 rows/s）しましたが、
4→8 のスケールは **~1.4 倍（サブリニア）** にとどまりました: 同一テーブルへの同時 upsert が DSQL 内部で
競合するためです。スマートデフォルトが実効並列度を 8 で上限とするのは、まさにこの理由です。
