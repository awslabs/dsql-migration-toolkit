...
### バリエーション：単一の巨大テーブル（1TB を 1 テーブルに）

同じ 16 vCPU ですが、データを 20 テーブルではなく**単一テーブル** `big_events`
（915.7M 行 ≈ 1.07TB、BIGINT AUTO_INCREMENT PK）に格納した場合。エンジンは整数 PK を検出し、
テーブルを**コアごとに 1 つずつ 16 個の PK レンジシャード**へ自動分割します。

| 指標 | 値 |
|---|---|
| **完走** | 915.7M 行すべてロード |
| **総所要時間（wall）** | **~2h10m**（20 テーブルの 2h27m より速い ― 16 均等シャードで tail penalty なし） |
| **スループット** | 最初 ~16K → ランプ後 **~120–150K rows/s**（CPU 飽和） |
| **OCC 40001** | 0 |

- **重要な知見 ― 新規単一テーブルのパーティションウォームアップ。** 作成直後の DSQL テーブルは
  パーティションが 1 つなので、16 シャードが同時に書き込んでも、初期は書き込みがその単一
  パーティションに集中し、ロードは**低速（~16K rows/s）で始まります**。DSQL が負荷に応じて
  **テーブルを複数パーティションへ分割する**につれ、スループットは ~16K → 46K → 97K →
  **120–150K（CPU 飽和）** へ急上昇します。マルチテーブルのロードは **20 テーブル = 初期
  パーティションが 20 倍**のためこのウォームアップがありませんでした ― データをテーブルに分散
  すると、DSQL の書き込み並列性が最初から得られます。
- この経路で、**シャード結果集計のバグ**（存在しない `result.rows_skipped` を参照し、全行
  ロード済みの単一テーブルを誤って `FAILED` としていた）を発見・修正しました（**v0.1.119**；
  `rows_skipped` を `conflicts` からマッピング）。マルチテーブルのロード（テーブルごとに 1
  ワーカー、非シャード）は無影響でした。
