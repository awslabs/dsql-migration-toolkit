...
### 値あたりのサイズ上限 — バイナリ(`bytea`)は 1 MiB を超える単一の値を保存できない

| DSQL 特性 | テストするシナリオ | どう検証するか |
|---|---|---|
| 1 MiB を超えるバイナリ値(`text` に 1 MiB の上限はない) | Full Load 中 **および** CDC 中に、上限を超える大きなバイナリ値(MySQL の `BLOB` ファミリー、または PostgreSQL の `bytea`) | Full Load では行単位の **隔離**(quarantine)(PK と理由を記録し、テーブルの読み込みは継続)。シンクでは書き込み前に型ごとの上限で計測して **DLQ** へ(`DsqlSinkTask` のサイズ超過ガードは `bytea` に 1 MiB、`text`/`json` にはチャンク予算の 8 MiB を適用) |
...
### スキーマの違い — FK は強制される、PK 必須、非対応の型/オブジェクト

Aurora DSQL は外部キーを強制し、プライマリキーを必須とし、トリガー、ストアドプロシージャ、いくつかの型を省略します。
...
- **意図的に失敗させる行**: 約 1.5 MiB の**バイナリ** LOB 値(**1 MiB の隔離/DLQ** シナリオを強制 — 1 MiB の上限は `bytea`(`BLOB` 系)にのみ適用され**テキスト**値には適用されないため、このシナリオはバイナリ LOB 列でのみ再現できます)、および分離したテーブル内の `TINYINT(1)` = `2`(**明示的でテーブル単位で致命的な** シナリオを強制)。
...
- **意図的な失敗は、失われずに捕捉されました。** 約 1.5 MiB のサイズ超過 LOB 行(この実行の時点では DSQL は `text` にも値あたり 1 MiB の上限を適用していました。現在この上限は `bytea` のみです)は、正しく **隔離** され(設計上ターゲットには存在せず、プライマリキーで記録)、範囲外の `TINYINT(1)` 行は **明示的でテーブル単位で致命的な** 停止を引き起こしました — まさに安全な動作です。何も静かに破棄されず、静かに破損することもありませんでした。
...
> 同じ PK ごとの突合はシェルからも利用できます: `scripts/cdc_consistency_check.py`(ターゲットに **欠落** /
> **余分** な正確な PK を列挙)と `scripts/compare_rows.py -t <schema.table> --watch N`(行数 / ライブ収束の
> チェック) — [`scripts/README.md`](../../../scripts/README.md) を参照。権威ある go/no-go は組み込みの
> **Validation** ステップです。
