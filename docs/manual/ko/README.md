...
운영(DB Operation) 담당자**를 대상으로, DSQL은 분산 데이터베이스 설계로 인해 PostgreSQL과도
상당히 많은 부분이 다르며, 그 차이를 이 도구가 *어떻게* 변환을 도와주는지를 설명합니다.
...
**이종(heterogeneous) 마이그레이션을 결정론 우선으로** 수행하는 **웹 도구**(및 임포트 가능한
엔진)입니다: MySQL → PostgreSQL 방언 → DSQL 제약. **소스는 항상 읽기 전용**입니다.
마이그레이션은 **Connect**를 사전 단계로 한 6단계 안내 흐름입니다:
...
| # | 장 | 배우는 내용 |
|---|---|---|
| 0 | [시작하기 전에](00-before-you-begin.md) | 사전 점검 체크리스트 — 첫 단계부터 계획을 좌우하는 반드시 알 것(동일 리전 필수, 읽기 전용 소스, DSQL이 제외한 기능, CDC는 선택·과금). **여기서 시작하세요.** |
...
| 7 | [성능과 튜닝](07-performance-and-tuning.md) | 데이터 경로를 이렇게 설계한 이유(AWS 근거: OCC 재시도, 핫 파티션 PK, 트랜잭션 한도, 비동기 인덱스, IAM 토큰)와 Full Load / Validation / CDC 병렬수 튜닝 — 로컬 및 Fargate. |
| 8 | [테스트 — DSQL 특성이 강제하는 시나리오](08-testing-and-verification.md) | 각 Aurora DSQL 특성이 *강제하는* 마이그레이션 테스트 시나리오(트랜잭션 한도, OCC, 1 MiB 값, IAM 토큰, 비동기 인덱스, FK 없음, 무손실 핸드오프, 드리프트)와 도구가 각각을 어떻게 행사하는지 — 오프라인 및 실제 AWS. |
| 9 | [결론](09-conclusion.md) | 어떤 경로를 언제 쓸지, 권장 end-to-end 흐름, 다음 단계. |
| 10 | [고객 FAQ](10-customer-faq.md) | 고객이 가장 많이 묻는 질문 — Full Load, CDC, 제약, 타입 매핑, 검증, 컷오버/롤백, 운영 — 을 도구의 실제 동작에 근거해 답하고 상세 장으로 연결. |
...
| 아이디/비밀번호로 접속 | **단기 IAM 토큰**으로 접속(비밀번호 없음) | 고정 비밀번호 대신, 짧게 만료되는 토큰을 계속 발급받아 접속합니다(도구가 자동 처리). |
...
이런 제약은 DSQL이 **수평 확장(대규모 분산)**을 위해 의도적으로 둔 설계입니다. 걱정하지 않아도
