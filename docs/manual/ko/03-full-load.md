...
> **머릿속 모델:** 로더는 소스를 PK 순서로 한 페이지씩 읽어, 작고 **여러 번 적용해도 중복이 생기지 않으며**
> 재시도 가능한 배치로 DSQL에 씁니다 — 테이블 크기와 무관하게 메모리가 한정되고, 중단되면 재개됩니다.
...
- 라이브 소스가 변해도 읽기는 **단일 일관 스냅샷**.

**기본 키가 필요합니다.** PK 없는 테이블은 keyset 페이지네이션이 불가능해 거부됩니다(Evaluation에서도
`UNSUPPORTED`로 플래그됨 — DSQL도 PK를 요구). 단일·복합 PK 모두 지원합니다.

---

## 3.3 행을 읽어 들이면서 타입 변환

행을 한 건씩 읽어 들이는 동안 각 값이 DSQL 저장 형태로 변환됩니다. 이는 Schema Conversion 매핑과
일치합니다(컬럼
타입과 값이 일치하도록). MySQL 사용자가 알아둘 예:
...
| 바인드 파라미터 한도 | 맞도록 배치 클램프 | `MAX_STATEMENT_PARAMETERS = 65535` (`65535 // 컬럼수`) |
