...
규칙이 다른 다른 엔진이므로, 몇 가지 사실이 첫 단계부터 계획을 좌우합니다. 아래 각 항목은 한 줄 "반드시
알 것"과 이를 깊이 다루는 장으로의 포인터입니다.

> 이 페이지는 **사전 점검 체크리스트**입니다. 상세 장을 대체하지 않으며 — 어떤 접근을 *이미 정한 뒤*
...
- [ ] **(AWS 배포 시) 단일 태스크 컨트롤 플레인이며 인증은 선택입니다.** ECS Fargate 태스크 하나; 진행
      중 작업 상태는 임시 스토리지에 있음; 앱은 ALB의 **선택적 Cognito** 게이트에 의존(인터넷 노출
      배포에서는 반드시 켜기). → [§6.3](06-limitations.md#63-배포-한계-aws-호스팅-형태)
