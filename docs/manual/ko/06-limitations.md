...
| **열당 1 MiB 한도** | 열 하나의 값이 **1 MiB**를 넘으면 저장할 수 없습니다 — 타입과 무관합니다(MySQL `TEXT`/`BLOB`, PostgreSQL `text`/`bytea`): `54000 ERROR: maximum column size exceeded`. **행** 전체는 **2 MiB**가 한도이고(`maximum row size exceeded`), 쓰기 트랜잭션 하나는 10 MiB입니다. ([Aurora DSQL 쿼터](https://docs.aws.amazon.com/aurora-dsql/latest/userguide/CHAP_quotas.html)) | 한도를 넘는 값은 행 단위로 에러 로그(Full Load) / **DLQ**(CDC)로 **격리(quarantine)**됩니다. Kafka 메시지 상한을 넘길 수 있는 컬럼은 **캡처 단계에서 제외**해야 합니다. 초과 값을 담을 수 있는 컬럼은 `OVERSIZED_LOB`로 표시됩니다: 바이너리/문서 컬럼(MySQL `MEDIUMBLOB`/`LONGBLOB`/`MEDIUMTEXT`/`LONGTEXT`, PostgreSQL `bytea`/`json`/`jsonb`)은 LOSS — 그 타입을 고른 것이 큰 값을 담겠다는 의도이기 때문입니다. 길이 제한 없는 PostgreSQL `text`/`varchar`는 RECOMMENDATION — 그것이 평범한 짧은 문자열의 관용적 표기이기도 하기 때문입니다. CHECK로 리터럴 집합에 제한된 컬럼은 아예 표시되지 않습니다. |
