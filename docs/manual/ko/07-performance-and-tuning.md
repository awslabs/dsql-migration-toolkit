...
세 단계 모두 합리적인 한정 기본값으로 실행되며, 큰 하드웨어에서 처리량을 위해 올리거나 바쁜 소스를 보호하기
...
실행되며, 그 스케일링 노브는 **캡처 대상 테이블 수**(그리고 편향 워크로드의 경우 테이블별 추정 크기 —
아래 참고)로부터 계산됩니다. 각 테이블은 자체 Kafka 토픽이고, 싱크는 토픽 **파티션**을 병렬로
소비하므로(파티션당 싱크 태스크 1개) 총 싱크 병렬수 = `토픽당 파티션 수 × 테이블 수`입니다. 툴은 이 곱이
싱크 병렬수 상한에 도달하는 가장 작은 파티션 수를 고르고 거기서 멈춥니다:
...
**편향(skewed) 워크로드 — 크기비례 파티션.** 위 표는 부하가 테이블에 **균등** 분산된다고 가정합니다.
실제로는 그렇지 않은 경우가 많습니다: 소수의 "hot" 테이블이 대부분의 쓰기를 담당할 수 있습니다. 테이블이
많으면 균등 규칙은 **토픽당 1파티션**을 주는데, 파티션 1개짜리 토픽은 **최대 1개 싱크 태스크**만 소비할 수
있어 hot 테이블이 단일 태스크로만 스트리밍되고 나머지는 유휴 상태가 됩니다(DSQL이 거의 유휴여도 순수
throughput 손실). 이를 피하기 위해 툴은 **스캔 없는 테이블별 row-count 추정치**(Full Load 워터마크의 값,
또는 Full Load 전에 CDC 인프라를 배포한 경우 `information_schema`에서 즉석 추정)를 읽어 큰 테이블에 Debezium
`topic.creation` 그룹으로 **파티션을 더 — 2개 또는 4개 —** 배분합니다. 그러면 hot 테이블이 여러 싱크
태스크로 병렬 스트리밍됩니다. 4가 테이블당 상한입니다(단일 테이블에 대한 동시 DSQL upsert 경합으로 ~4개를
넘으면 이득이 평탄해짐). 이 동작은 **자동**입니다: 실제로 편향된 다중 테이블 캡처에서만 작동하고, 부하가
균등하면 **아무것도 하지 않으며(no-op)**, 크기 신호가 없거나(또는 `DSQL_MIGRATOR_CDC_TOPIC_PARTITIONS`를
설정하면) 균등 기본값으로 폴백합니다. 순서는 영향받지 않습니다 — Debezium이 각 레코드를 기본 키로 키잉하므로
한 행의 모든 변경은 여전히 같은 파티션에 도착합니다. 토픽의 파티션 수는 생성 시 고정되므로 이 결정은 CDC 인프라
배포 시점에 이뤄지며, 변경하려면 CDC를 새로 배포해야 합니다.
...
상한이 존재하는 이유는 싱크가 **DSQL 쓰기 지연에 묶이기** 때문입니다: 측정된 처리량은 파티션 수에
**선형 미만(sublinear)으로** 증가합니다(4 → 8 파티션에서 2배가 아니라 약 1.4배) — 동일 테이블에 대한
동시 upsert가 DSQL 내부에서 경합하기 시작하기 때문입니다. 상한을 넘으면 파티션을 더 늘려도 처리량은
그대로인 채 MCU 비용만 늘어납니다. 소스 측은 MySQL 서버당 단일 태스크(단일 binlog 스트림)이지만
**병목이 아닙니다** — 기본 제공되는 producer 튜닝으로 초당 수만 레코드를 소화합니다. 언제나 그렇듯 부하가
높을 때 실제 상한은 핫 기본 키에서의 OCC이므로 **PK 전략이 가장 중요합니다**(§7.1 참고).
...
> 재시도하고 커밋된 binlog offset부터 gap 없이 재개 — 운영자 개입 불필요. 재부팅 후 CDC를
> Stop/Start할 **필요 없습니다**.
...
> 사이드바 푸터의 **Settings**를 열고 **Full Load / Validation** 탭으로 가세요. 로더와 검증기가 매 실행마다
...
`OFFSET` 재스캔 없음, 테이블당 한 번에 ~5000행 페이지 하나만 in-flight, 전역 락 없음, `COUNT(*)` 대신
...
아니면 그대로 두세요. (소스 읽기 **페이지 크기는 5000행으로 고정**되어 튜닝 불가; 소스 읽기의 **주** 레버는
테이블 병렬수이며, 선택형 `Threads_running` 상한이 두 번째 안전밸브입니다 — 아래 노트 참조.)

> **Settings → Full Load**에서 실행 사이에 실험하거나, 영속화하려면 환경 변수로 설정하세요(§7.2).
...
> [!note] 소스 부하 안전밸브
> **QPS 제한기는 없지만**, 선택형 동시성 스로틀이 **있습니다**:
> `DSQL_MIGRATOR_FULL_LOAD_MAX_SOURCE_THREADS_RUNNING`(기본 `0` = 꺼짐, **Settings → Full Load**에서도 설정)이
> 소스의 전역 `Threads_running`이 상한을 넘는 동안 새 소스 읽기를 일시 중지합니다 — gh-ost의 `--max-load`류 가드.
> 그 외에는 테이블 병렬수를 낮추는 것이 조절 수단입니다. 벌크 로드는 **오프피크**에 스케줄하고 인스턴스의
> 백업/유지보수 창과 겹치지 않게 하세요(스냅샷 + 풀 스캔 동시 실행은 gp2 `BurstBalance`에 최악의 경우).
...
선택적 **Query Converter**는 MySQL 쿼리를 변환하고 `EXPLAIN` / `EXPLAIN ANALYZE`로 읽기 전용
프로브한 뒤, AI 보조를 켜면 **AI DBA**가 DSQL에 맞게 재작성하고 재테스트로 DPU 개선을 증명합니다.

> 전체 흐름은 [9장 — Query Converter와 AI DBA](09-query-validation.md)를 참고하세요.
