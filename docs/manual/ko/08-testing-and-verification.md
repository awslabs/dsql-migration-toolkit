...
| **DSQL이 지원하지 않는 객체·타입** (트리거, 프로시저, 공간형, 정밀도 38 초과 `DECIMAL`, 컬럼 255 초과 등) | 그냥 옮기면 적재 도중에야 하나씩 터짐 | **Evaluation** 단계에서 미리 전수 분류(AUTO / MANUAL / UNSUPPORTED)하고 항목마다 사유·권장 조치·작업량을 제시 — 로드 전에 결정 | **Evaluation** 리포트 |
...
| **외래 키가 많은 스키마** | DSQL엔 외래 키가 없어 참조 무결성을 DB가 강제하지 않음 | FK를 DDL에서 떼되 `MANUAL` 노트로 보존하고, 선택적 **고아 검사**로 앱 측 무결성이 지켜졌는지 확인 | Validation **고아 검사** |
...
- parent → child → lob로 이어지는 **외래 키 체인** (FK 없음 + PK 필수 + 고아 검사를 강제).
