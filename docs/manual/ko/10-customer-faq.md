...
_언어: [English](../en/10-customer-faq.md) | **한국어**_

> **이전:** [9. 결론](09-conclusion.md)
...
| 대규모 / 지속적; **거의 무중단** 컷오버 필요 | **Full Load + CDC** — gapless 핸드오프로 MySQL이 살아 있는 동안 DSQL이 수렴. |

CDC는 실제 구성요소(MSK, MSK Connect, 싱크 커넥터)를 추가하며 **배포된 동안 비용이 발생**합니다.
지속 복제가 정말 필요할 때만 쓰세요; 아니면 Full Load만으로 더 단순하고 저렴합니다.
[9장 §9.1](09-conclusion.md#91-which-path-do-i-need) 참조.
...
([9장 §9.3](09-conclusion.md#93-the-cut-over-switching-your-application-to-dsql) 참조).
...
1. 쓰기 충돌을 배치 전체 재제출이 아니라 **문(statement) 단위**로 재시도(충돌한 행만 다시 실행)해,
   핫 키에서의 라이브락을 피합니다.
...
1. **MSK Serverless 클러스터 프로비저닝** — 관리형 Kafka 클러스터가 준비 상태가 될 때까지 기다려야
   합니다(가장 큰 비중).
2. **MSK Connect 커넥터 2개 생성** — 소스(Debezium) 커넥터를 먼저 만들고(토픽 생성 후), 그다음
   싱크(DSQL) 커넥터를 만듭니다. 각 커넥터가 `CREATING → RUNNING`이 되는 데 각각 수 분이 걸리고,
   순서대로 진행되므로 합산됩니다.
3. 그 사이 in-VPC **오프셋 시더 Lambda**가 gapless 핸드오프용 오프셋을 seed합니다.

**철거(제거)가 오래 걸리는 이유** — MSK 삭제 자체는 비교적 빠르지만, 마지막에 남는 **in-VPC
...
- **Full Load**: ~1 MiB 값당 한도를 넘는 값은 **행 단위로 격리**되고(기본 키 + 사유를 **에러 로그(error
...
[9장 §9.3](09-conclusion.md#93-the-cut-over-switching-your-application-to-dsql)에 있습니다.
...
- **경로 선택과 컷오버:** [9장 — 결론](09-conclusion.md).
