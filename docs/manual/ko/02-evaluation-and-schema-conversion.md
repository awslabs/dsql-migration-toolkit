...
Aurora MySQL 사용자에게 **이 단계는 DSQL이 받아들이지 않을 것을 — 단 한 행도 옮기기 전에 — 알게 되는
지점**입니다. 건너뛰지 마세요.
...
**Evaluation**을 실행하면 도구가 **소스**(테이블, 컬럼, 타입, 기본 키, 인덱스, 외래 키, 뷰, 트리거,
루틴, `AUTO_INCREMENT`, charset/collation)와 **타깃**을 살펴본(introspect) 뒤 **호환성 평가 리포트**를
만듭니다.
...
- **갭** — 옮기지 못했거나 의미가 바뀐 것(제거된 외래 키, 유실된 collation 등). 어떻게 처리할지
  결정해야 합니다.
- **권장사항**(`RECOMMENDED`, info-blue) — 변환은 완전하고 정확합니다. DSQL에서 *잘 돌리기* 위한
  조언이며, 무시하면 성능을 잃지만 정확성은 잃지 않습니다.

대표적인 권장사항은 `AUTO_INCREMENT`입니다. 이 키는 깨끗하게 변환되고 정상 동작합니다. UUID/랜덤
...
권장사항은 선택이므로 **객체의 예상 작업량(effort)에 반영되지 않습니다** — 그러지 않으면 거의 모든
MySQL 테이블이 `AUTO_INCREMENT` 키 하나 때문에 `MEDIUM`으로 부풀려집니다. 조언을 채택할 때의
...
- **위험 설명** — *왜* 플래그됐는지(예: "Aurora DSQL은 외래 키를 지원하지 않음"),
- **권장 조치** — *무엇을 해야 하는지*(예: "이 관계를 애플리케이션 계층에서 강제"),
...
- **결정론적 경로를 대체하지 않음.** `sqlglot` MySQL→DSQL 변환이 항상 먼저 돌고, AI는 그것을 *보강*만
  합니다. AI를 끄면(기본값) 워크플로는 동일하게 동작합니다.
...
2. **모든 MANUAL 항목 결정.** 제거된 외래 키를 앱에서 어떻게 강제할지, 파티셔닝 제거, 기본 collation
   수용, `ENUM`/`SET` 처리 등.
3. **AUTO 항목**은 할 일 없음.

> **DSQL에서 특히 중요한 이유:** DSQL은 외래 키, 트리거, 저장 프로시저, 일부 타입을 의도적으로
...
변환(`sqlglot` 기반, MySQL → PostgreSQL 방언 → DSQL 제약)은 방언과 제약의 간극을 자동으로 메워 줍니다:
...
반하는 비관적 락), 외래 키 의존, `AUTO_INCREMENT` 가정, 트리거/저장 프로시저 호출, 미지원 함수 등입니다.
