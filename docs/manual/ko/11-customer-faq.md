...
제외합니다 — **트리거/저장 프로시저 없음, 트랜잭션당 행 수 제한, 바이너리 값당 1 MiB 제한** 등.
...
3. DSQL의 **트랜잭션당 3000행 한도와 타입별 값 크기 한도**(`bytea` 1 MiB)를 강제로 지키고, 처리 불가한(poison) 행은 DLQ로 보냅니다.
...
**외래 키**는 DSQL이 지원·강제하지만, 트리거와 저장 프로시저는 DSQL에 없습니다 — 각각 다음과 같이
처리됩니다.

- **외래 키**: 변환된 DDL에서 **제거하지 않고 보존**됩니다. 각 FK를 적재 후 실행할
  `ALTER TABLE … ADD CONSTRAINT … FOREIGN KEY` 문으로 렌더링(`CREATE TABLE` 밖)해 데이터 적재가 끝난 뒤
  DSQL에 다시 생성하며, CDC 마이그레이션에서는 **cut over 시점**에 적용합니다(순서가 뒤섞인 CDC 스트림
  중에는 FK가 있으면 위반이 dead-letter로 빠지기 때문, SQLSTATE 23503). 원하면 FK를 떼어 내고
  (`preserve_foreign_keys=False`) 참조 무결성을 애플리케이션에서 강제할 수도 있습니다. 런타임 주의점:
...
**Q24. DSQL이 저장할 수 없는 행(예: 1 MiB를 초과하는 **바이너리**(`bytea`) 값)은 어떻게 되나요?**

**절대 조용히 버려지지 않습니다.** 어느 경로든 저장 못 하는 행은 격리(**quarantine**)되고, 무엇이
빠졌는지 정확히 확인할 수 있습니다. 다만 **격리된 행이 기록되는 위치는 경로에 따라 다릅니다.**

- **Full Load**: **바이너리**(`bytea`) 값이 1 MiB 한도를 넘으면 **행 단위로 격리**되고(기본 키 + 사유를 **에러 로그(error
