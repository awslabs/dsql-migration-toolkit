...
| **Per-value size limit (differs by type)** | A **binary** value over 1 MiB can't be stored (`bytea`; MySQL `BLOB`, PostgreSQL `bytea`) — DSQL rejects it outright. A **text** value has no 1 MiB cap: it is bounded by the 10 MiB per-write-transaction limit (measured intact at 9.5 MiB). | An oversized **binary** value is **quarantined** to the error log / DLQ; a text value that large is **replicated normally**. Columns over ~8 MiB must be **excluded at capture** (Kafka's message cap). Large LOB columns flagged `OVERSIZED_LOB` (MANUAL). |
