...
| **Source `CHECK` constraints** | A source `CHECK` constraint is not translated to the target (e.g. a MySQL `CHECK`, 8.0.16+). | Dropped from the DDL and flagged **MANUAL** — re-add a DSQL-compatible `CHECK` by hand or enforce it in the app. (The `CHECK … IN (...)` the tool *generates* for a MySQL `ENUM` is unaffected. For a PostgreSQL source an `ENUM` is a user-defined TYPE, which DSQL does not have: the column is flagged **UNSUPPORTED** and you must remodel it yourself — the tool emits neither a `text` substitution nor a `CHECK`.) |
...
| **1 MiB per-column limit** | A single column value over **1 MiB** can't be stored — ANY type (MySQL `TEXT`/`BLOB`, PostgreSQL `text`/`bytea`): `54000 ERROR: maximum column size exceeded`. A whole **row** is capped at **2 MiB** (`maximum row size exceeded`), and one write transaction at 10 MiB. ([Aurora DSQL quotas](https://docs.aws.amazon.com/aurora-dsql/latest/userguide/CHAP_quotas.html)) | Oversized values are **quarantined** per row to the error log (Full Load) / **DLQ** (CDC); columns that can exceed the Kafka message cap must be **excluded at capture**. Columns that can hold an oversized value are flagged `OVERSIZED_LOB`: a binary/document column (MySQL `MEDIUMBLOB`/`LONGBLOB`/`MEDIUMTEXT`/`LONGTEXT`, PostgreSQL `bytea`/`json`/`jsonb`) as a LOSS, since choosing that type is the intent to store something large; an unbounded PostgreSQL `text`/`varchar` as a RECOMMENDATION, since that is also the idiomatic spelling for an ordinary short string. A column a CHECK limits to a finite set of literals is not flagged at all. |
| **`DECIMAL` precision > 1000** | Higher precision unsupported (DSQL documents maximum precision 1000, scale -1000..1000). | Evaluation flags it **UNSUPPORTED** (`NUMERIC_PRECISION`); if the converted DDL is applied anyway, Schema Conversion **clamps to the maximum** (lossy) with a warning. MySQL caps `DECIMAL` at 65,30, so this is a PostgreSQL-source edge only. |
