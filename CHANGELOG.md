...
- **The "Deploy CDC infrastructure" dialog went from seven notices to three.** Measured on the reported case (PostgreSQL after a `Full load only` run, NAT to create, a routed VPC, six enforced foreign keys): **7 boxes / 532 words → 3 boxes / 335 words**, with nothing actionable lost. What changed: (1) the four conditions that grey out Deploy are hoisted to the TOP and merged into one `Deploy is blocked` box — they were in source order, so the two that fire last sat *below* an info-toned cost estimate, i.e. the explanation for a disabled button was the last thing on a scrolling dialog. Each keeps the consequence sentence the raw blocker string does not carry ("would create a billable MSK Serverless cluster that Start CDC could never use"). (2) The hourly estimate and the re-snapshot disclosure merged into one **Cost and how long it runs** box: they are the same decision, and the second silently qualified the first. (3) The two items that are the operator's **own** work — dropping the target's foreign keys, opening the source's inbound rule — merged into one **Before the connectors can run** box; both fail the same way if skipped (connectors created, billed, never RUNNING). (4) Network, subnet overlap, the VPC mismatch and the MSK seed note merged into one **What this deploy creates in your VPC** box; none of them asks the operator to do anything. Two mechanism paragraphs are dropped from the dialog only, because both are verbatim on surfaces the operator has just passed: the foreign-key rationale (out-of-order child rows, SQLSTATE 23503, silent dead-lettering) still renders where CDC is actually *refused*, and the replication-slot explanation still renders on the start-point card and in the prerequisite row. A test now caps the dialog at three boxes (four with a blocker) so the next addition has to merge rather than append — the aggregate creep a review had warned about and nothing enforced.
- **PostgreSQL no longer offers a CDC start-point choice, because it never had one.** "Manual — re-snapshot from scratch (initial)" was a trap rather than an option: executed across the whole state space it changes the outcome in exactly one state family. With no watermark, and with a `Full load only` watermark, **both** modes yield `snapshot.mode=initial`; a `Full load + CDC` job that did not reach `DONE` has its slot suppressed, so both yield `initial` there too. Only a finished load that recorded a slot differs — and there Manual discards the gapless resume to re-copy every row through the **streaming** pipeline (this repo measures ~19–26K rows/s against the bulk loader's ~103K) while being unable to remove rows deleted since the load, because a snapshot only reports rows that still exist. Re-running as `Full load + CDC` beats it on every axis, which is what the disclosure says. MySQL keeps its radio untouched: there Manual supplies a real GTID or binlog coordinate, and it is the documented cure for a GTID-only watermark. The re-snapshot **capability** is unchanged — the publication-missing escape buttons still record it.

### Fixed

- **A false block that the removed radio had been papering over.** Start CDC's readiness test read `is_pg and mode == "manual"`, so a PostgreSQL CDC-only start — or any PostgreSQL session whose job record was lost to an app restart — was DISABLED under Automatic with "Set the CDC start point above first." and enabled under Manual, while both modes produce the identical connector config. The cure was an undiscoverable radio click, and the start-point card one screen up already badged that same state "Ready". Readiness now keys on the effective snapshot mode, which makes PostgreSQL ready by construction.
- **The start-point card could claim a gapless start while the connector was configured to re-snapshot.** Its disclosure was gated on `not wm_usable and mode == "auto"` and its confirmation line on `mode == "manual"`, so the one state where a slot IS recorded and an escape button switched the start to a re-snapshot — a state session restore reloads — showed the gapless line. Both now key on the same cause the connector config derives from, so they cannot disagree.
...
- **A connector failure nobody had written a needle for reported only its state, after the whole billable wait.** `_connector_failure_message` fetched the worker-log tail, matched it against the known-signature table, and when nothing matched **discarded the tail** and returned the bare `<connector> entered FAILED state.` — so an operator who had just waited out a connector create (bounded by `connector_timeout_seconds`, 45 min per connector) still had to open the MSK Connect log group to learn anything at all. The last log line is now repeated verbatim instead, which is strictly more general than any needle: it cannot assert a wrong cause, and it covers every failure mode that has no needle yet. It also survives the TIMEOUT path, which is where it matters most — a connector whose only task dies at startup sits in `CREATING` until the budget expires, so the state itself says nothing. The excerpt is length-bounded (a Java stack trace is not) and passed through a credential scrub before it reaches a notice or the downloadable activity log (Property 7); the connector's password is a separate `database.password` config that Kafka Connect masks, so this is defence in depth rather than a known leak.
- **Nothing ever mentioned the one firewall rule the tool cannot add for you.** The cdc-stack can only add rules to its **own** connector security group — its source-facing rule is EGRESS, and the template's own comment concedes that "the route table + the source SG inbound rules still gate what is actually reachable". The reciprocal INBOUND rule on the operator's database is theirs to set, and when it is missing both connectors are created, billed, and then simply never reach RUNNING: the most expensive possible way to learn about a security group. The Deploy dialog now states the rule with the source's **own** security-group ids and port filled in, taken from the `DescribeDBInstances` response the dialog already makes (no extra API call, no extra IAM — that response was fetched and everything but the VPC id thrown away). It is `info` and does **not** gate Deploy: the rule is frequently already satisfied by a VPC-wide rule, the tool cannot prove otherwise without reading a customer resource, and a false block is the worse defect.
- **A "consistent" badge could sit in the same row as "12m behind".** `replication_lag_ms` documents itself as "the accurate Stream lag signal, **preferred over** the MAX(pk) leading-edge fallback", and `consistency`'s docstring already claimed to combine "the stream high-water (PK) / time-based lag signals" — but `stream_caught_up` read `pk_gap` alone. So a measured twelve-minute lag rendered green, and a table with no single integer PK had no leading-edge signal at all. The verdict now reads the time-based lag first and the PK gap only as a fallback, with a 30s threshold (a live stream is always slightly behind; this matches the `lag_warn_seconds` the connector chip already uses, so the per-table verdict and the pipeline chip agree). Equal row counts no longer override it either: an UPDATE-only workload changes no count and an insert/delete pair nets to zero, so the target can be equal in size and stale in content — precisely the state a cut-over decision must not read as green. `None` lag still means idle/caught-up, never "behind".
- **The monitoring legend turned a live metric into a go/no-go.** "Stream lag" explained `“caught up” = the target is current (safe to cut over)` — the only place the tool did that — and it is wrong in one reachable state: a table an initial snapshot has not **reached** yet has no lag datapoint, so it reads "caught up" while none of its changes have arrived. The Consistency legend had the matching gap, explaining only that non-green means investigate and never that green does not prove the snapshot got here. Both now say so and point at the authority that actually gates cut over (Validation's exact comparison). Held as module constants so a test pins the safety claim without driving the whole render.
- **Validation asserted a single cause for extra target rows, and it was the wrong one on a re-snapshotting start.** The notice's header said "a delete did not apply" and its body sent the operator to `REPLICA IDENTITY` and the dead-letter queue. That cause is real, but the same signature has a benign one that is likelier here: a CDC start with no replication slot to resume from re-snapshots, and a fresh snapshot only reports rows that still exist — so rows deleted on the source between the Full Load and the CDC start were never streamed as deletes. v0.1.544 discloses that at start time; Step 4 is where it surfaces, and an operator reading only this notice would hunt a `REPLICA IDENTITY` bug that was not there. Both causes are now named, with the activity log as the way to tell which applies. The remedy is unchanged and correct for both.

### Not changed (investigated and deliberately left alone)

- **Kafka topic retention stays at the MSK Serverless defaults.** A review claimed the tool's unset `retention.ms`/`retention.bytes` could silently lose rows on a large re-snapshot. Verified against the MSK docs and rejected as stated: `retention.bytes` trims the **oldest** bytes per partition, so volume alone loses nothing, and the documented per-partition caps (5 MBps ingress vs 10 MBps egress) mean the sink is not structurally outrun — loss would need the sink to fall more than seven days or 250 GiB/partition behind, and Validation's exact comparison plus the cut-over gate already block a release on the result. Pinning `retention.ms=-1` was designed and then dropped because its own failure mode is worse: MSK Serverless documents that value only in an `--alter` example, so acceptance at `CreateTopics` is inferred, and a rejection would make the topic-seeding custom resource fail and hard-block Start CDC for everyone (the shape of v0.1.438's `lc_numeric` regression). It would also need a `PLUGIN_VERSION` bump, so every existing cdc-stack would need Delete + Deploy, and a live MSK run to gate it. Recorded here so the analysis is not repeated rather than shipped on inference.

## v0.1.544

### Fixed

- **A PostgreSQL operator continuing a `Full load only` run into CDC was never told the tool is about to re-read the entire source.** `Full load only` records no replication slot, and a PostgreSQL logical slot cannot be created at a past position — so a later CDC start runs `snapshot.mode=initial` + `publication.autocreate.mode=filtered`: the connector creates its own publication and slot and re-snapshots every selected table before streaming. That is lossless and legitimate, but the source is read a second time, every row crosses MSK into DSQL again, and rows deleted on the source since the load stay on the target. The tool HAD a good explanation of this; one re-grade switched off every copy of it at once. Because `cdc_start_resnapshots` is derived (not chosen), an absent publication was re-graded to `INFO, required=False` so a red FAIL would not sit beside an enabled button — and that same re-grade emptied the remediation string, made `resnapshot_resolvable_failures` return `[]` (so the "Both ways forward re-read the source tables" fork never rendered), and left the Start dialog's "The cost is reading the source tables again" branch untaken. What survived was one INFO row, collapsed under a green badge, worded as if the operator had chosen the re-snapshot. Now: that prerequisite row is a **WARN** (still `required=False`, so `can_proceed` stays true and nothing is blocked — WARN is also what auto-expands the section so the row is read) carrying the full cost and the remedy; the same disclosure appears in the **Deploy CDC infrastructure dialog directly under the hourly cost estimate**, which is the last point at which cancelling is still cheap; and the Start dialog's "Ready to start CDC" notice no longer says the connectors "begin streaming" in "a few minutes" without mentioning the snapshot phase that precedes it. All of it is derived from one predicate pinned against `pg_snapshot_mode` itself, so the disclosure cannot drift from the connector config it describes.
- **Four surfaces asserted a gapless handoff on the one route that re-snapshots.** The start-point card's hover help said "Automatic resumes exactly where the Full Load snapshot ended (no gap, no overlap)" unconditionally — the only explanatory text attached to Automatic anywhere on the card, so it taught the wrong model at the moment the operator was forming one. The table-set glyph said the set "is what makes the handoff gapless", gated on `full_load_ran` with no engine test. The permanent activity-log record appended "(gapless from the Full Load watermark)" whenever a watermark existed, never consulting `slot_name` — worse than silence, because Step 4 will show rows the source no longer has and that line is the only durable artifact explaining them (it now records the deleted-row residue instead). And the no-report Deploy block that a PostgreSQL operator on this route is *guaranteed* to hit first told them to verify "the source binary log", MySQL's mechanism, in the same function whose next branch was already engine-split.
- **The start-point card contradicted the button beneath it.** For PostgreSQL Automatic with no slot the card showed an amber "Action needed" badge directly above an ENABLED blue "Start CDC" (the button's own readiness test passes on the WAL LSN), labelled the PRE-SELECTED default "Automatic — needs a Full Load slot (unavailable)", and offered the re-snapshot as what *Manual* would do — so leaving the default alone read as "I did not choose this" when it is exactly what Automatic does. The card's own code comment already said "picking Automatic without a provisioned slot is not a dead end". That start point is now treated as RESOLVED, the label names what Automatic will do, and the notice states the cost with a cause-specific remedy. The connector-config preview — whose params file prints `PgSnapshotMode: initial` / `PgPublicationAutocreateMode: filtered`, the re-snapshot in the tool's own machine-readable words — no longer early-returns on this route; MySQL always showed it, so this also closes a place where the PostgreSQL path looked like a different app.
- **One message blamed a missing slot for three different causes, and two of its remedies were wrong.** `_cdc_watermark` deliberately suppresses `slot_name` for a load that did not reach `DONE`, so a stripped watermark cannot distinguish "never provisioned" from "not honoured yet" — and the operator whose load merely FAILED was told no slot was recorded and sent to re-run the whole load, when retrying the failed tables is enough (the slot self-heals once the job is `DONE`). An operator who chose Manual over an available slot was likewise told a slot was missing. The cause is now read off the RAW job and carries its own remedy per case: `no_slot` → start over as "Full load + CDC"; `unfinished_load` → retry the failed tables; `manual` → switch back to Automatic; `no_watermark` → no claim that anything is read "again", because nothing was read here. No wording gives a wall-clock equivalence to the Full Load either — the re-snapshot runs through the streaming pipeline, not the bulk loader, and this repo's own measurements put those an order of magnitude apart.
- **The target-foreign-key precondition was checked only AFTER the MSK spend was authorised.** A `Full load only` run applies and validates the foreign keys by design, which is exactly how a session that later switches to CDC arrives with enforced FKs on the target — and the gate for that lived solely in the Start CDC dialog and a worker backstop. So the operator paid for the MSK Serverless cluster, then Start refused with "Remove the target's foreign keys before streaming", leaving the cluster billing while idle. The same probe now also runs in the Deploy dialog as a sequencing **warning** (Deploy stays enabled — it is never a dead end: this migration's own FKs have a one-click Remove in the Start dialog and cut over re-creates them).
- **The AI assistant's CDC diagnostic was dead for every session, on every invocation.** `get_cdc_pipeline_diagnostics` built its payload with `_dm.cdc_start_mode` instead of `_dm.cdc_start_mode()`, so `json.dumps` raised `Object of type method is not JSON serializable` and the tool's catch-all turned it into `{"status": "error", "message": "tool lookup failed"}`. Nothing caught it because every test double is an attribute bag on which the missing name resolves to `None`; the regression test now drives the REAL `DataMigrationState` and asserts the tool answers `ok`, so any future unserialisable field fails the build instead of silently disabling the tool.

### Documentation

- The manual's §4.3 handoff walkthrough ("Debezium begins streaming the first change after the snapshot — not from 'now,' and not by re-reading the data") is true only of a `Full load + CDC` run; it now says so and points at the `Full load only` → CDC behaviour. §10.1's engine-blind "CDC only … with no fresh Full Load" row gains the PostgreSQL caveat. `README.ko.md` and `README.ja.md` were missing the engine-split paragraph `README.md` carries, so the Korean and Japanese "At a glance" claimed only that a watermark bridges the paths gaplessly. All three languages updated.
...
- **A PostgreSQL enum column could not be migrated at all, and the finding could not say why.** `format_type` returns only a user-defined type's NAME, and `pg_type.typtype` was never read — so `ecommerce.order_status` was indistinguishable from a composite or a domain. The message therefore hedged across all three ("a PostgreSQL enum -> text; a composite type -> separate columns or jsonb"), never stated the real reason, and never showed the allowed values, which are the one thing needed to remodel the column. Meanwhile Schema Conversion emitted the enum type verbatim and Aurora DSQL has no `CREATE TYPE` (live-verified: `CREATE TYPE ... AS ENUM` → `CREATE TYPE not supported`), so the whole `CREATE TABLE` was rejected at apply. The type's KIND and an enum's LABELS are now introspected (two columns on a join that was already there), and an enum column takes the same faithful port MySQL's `ENUM` has always had: `text` with a `CHECK` over the same values, with its `DEFAULT` carried across as a plain text literal instead of being dropped. Verified end to end on a real Aurora DSQL cluster: the converted DDL applies, the `DEFAULT` fills, a valid label inserts, an invalid one is rejected by the `CHECK`. The column is no longer graded UNSUPPORTED, because nothing prevents the migration any more.
- **"enum" is a KIND, not a type name, and the wording now respects that.** PostgreSQL has no type spelled `enum` — `CREATE TABLE t (c enum)` fails with `type "enum" does not exist` and `pg_type` holds no such row — so a message may say "a user-defined ENUMERATED type (`CREATE TYPE ... AS ENUM`)" but must never present `enum` as a type. Each kind now gets its own message naming the real reason: an enum / composite / range cannot exist on the target because there is no `CREATE TYPE`; and the risk line adds "Aurora DSQL has no CREATE TYPE, so a user-defined type cannot be created there" only when a user-defined type is actually involved, so a built-in unsupported type (`inet`, `money`, …) is unaffected.
- **A DOMAIN column was reported UNSUPPORTED, which is wrong — Aurora DSQL supports `CREATE DOMAIN`.** Live-verified: the domain is created, used as a column type, and an out-of-range value is rejected by its `CHECK`. The finding now says so and states the actual gap (the converter does not emit the domain's DDL yet, so the `CREATE TABLE` still needs the domain created on the target first, or the column changed to the domain's base type with its `CHECK` re-added on the column).
- **Both engines' enum conversion notes now lead with what HAPPENED rather than with "not supported",** and say the same things, since the two paths now do the same thing: the value domain is preserved by the `CHECK`, and the two real consequences are stated — ordering and comparison become TEXT (an enum sorted in DECLARATION order, which is not alphabetical, so an `ORDER BY` or range predicate on the column changes), and adding a value later is an `ALTER TABLE ... DROP/ADD CONSTRAINT` rather than `ALTER TYPE ... ADD VALUE`.

## v0.1.536

### Fixed

- **A PostgreSQL `STORED` generated column was destroyed on the claim that "Aurora DSQL has no equivalent". DSQL has one.** Live-verified on a real cluster: `GENERATED ALWAYS AS (expr) STORED` is accepted, the value is computed on `INSERT` (3 × 2.50 → 7.50) and RECOMPUTED on `UPDATE` (→ 10.00), `pg_attribute.attgenerated` is `'s'`, and an `INSERT` that supplies a value is rejected — exactly as on PostgreSQL. `VIRTUAL` is a syntax error (PostgreSQL 17 has none either). The tool nonetheless dropped the clause, created an ordinary column, and told the operator in Evaluation, twice in Schema Conversion, in both AI grounding blocks and in all three manuals to "compute the value in the application" — unnecessary work, and **irreversible after apply**, because DSQL can `DROP` an expression from a column but never `ADD` one. The expression is now re-emitted, so the target keeps computing the value: no drift, nothing to re-implement. `VIRTUAL`, a column whose expression was not captured (every MySQL source — introspection never reads `GENERATION_EXPRESSION`), and a generated column inside the PRIMARY KEY are still created ordinary, each with its own stated reason.
- **The clause is injected as RAW TEXT, because sqlglot mangles real `pg_get_expr` output.** Emitting it through the re-parsed/re-rendered DDL builder looks correct and silently corrupts what PostgreSQL actually stores. Measured: `(tags #>> '{a,b}'::text[])` → `CAST(tags #>> '{a,b}' AS TEXT[])`; `((tags -> 'a') ->> 'b'::text)` → `operator does not exist: text ->> unknown`; `date_part('year'::text, d)` → `EXTRACT(CAST('year' AS TEXT) FROM d)`; and `((name IS NOT NULL))::integer` → a `ParseError` that aborts the WHOLE table and takes every other column with it. All four now survive byte-for-byte (same technique as the identity `CACHE` clause).
- **Full Load now omits a target-computed column, which is what makes preservation safe.** DSQL rejects a write that supplies a value for a generated column (SQLSTATE `428C9`) and the failure is permanent, so with the clause preserved every batch would fail and every row be quarantined — reproduced: 64 rows became 127 statements, 0 loaded, 64 permanently dropped. The column is filtered out of the keyset `SELECT`, the `INSERT` column list and the `ON CONFLICT DO UPDATE SET` list by one `TableDef` filter. Which columns those are is resolved by TWO SIGNALS, exactly as the primary key already is: the applied DDL when this run recreates the target, the **live catalog** when appending onto an existing one — because a target made by an older build (which stripped the clause), by the customer's own DDL, or by an edited-then-reverted script can have an ORDINARY column where this run's conversion says GENERATED, and trusting the DDL there would write NULL into every row silently. An unreadable catalog is UNKNOWN and refuses the load rather than guessing, and a table with nothing generated anywhere takes the ordinary path with no extra target round-trip.
- **CDC excludes a preserved generated column from the stream** (`column.exclude.list`), for the same reason: the sink's write would be rejected and dead-lettered. PostgreSQL 17 and earlier do not publish generated columns at all, so this is belt-and-braces there and load-bearing on PostgreSQL 18, whose publications can opt into publishing them.
- **`pg_attribute.attgenerated` was read and then discarded into a boolean, so the SUPPORTED kind looked like the unsupported one.** `ColumnDef.generated_kind` ('STORED' / 'VIRTUAL') now carries it — the same fix `identity_generation` got — and the source-DDL panel renders the real kind instead of hardcoding `STORED`, so a PG18 `VIRTUAL` column is no longer mislabelled as the one DSQL accepts.
- **MySQL's generated-column finding asked a question it can now answer.** "Recreate as a PostgreSQL GENERATED column **if supported on the target**" becomes: DSQL does support `STORED`, but the tool cannot carry a MySQL expression over because introspection does not read `information_schema.COLUMNS.GENERATION_EXPRESSION` — so either re-add it yourself in the DDL editor before applying (translating to PostgreSQL syntax), or compute it in the application, and decide before apply because the expression cannot be added later.

## v0.1.535

### Fixed

- **A PostgreSQL `varchar`/`char` declared longer than Aurora DSQL allows made the whole `CREATE TABLE` fail at apply, with nothing having warned.** DSQL caps a declared character length at 65535 bytes for `varchar` and 4096 for `char` (live-verified: `VARCHAR(65536)` → `Datatype limit greater than 65535 bytes not supported for varchar`), while PostgreSQL allows ~1 GB — and `dsql_lint` returns zero diagnostics on it, so the linter did not catch it either. Such a column is now converted to `text`, which keeps MORE of the source's range than clamping to the ceiling would (clamping would REJECT values the source legitimately holds), with a warning naming what is lost: the length limit is no longer enforced, and a fixed-length column also loses its blank-padding semantics. A new Evaluation finding (`PG_CHARACTER_LENGTH`) states the same thing, so the two steps agree instead of the operator meeting this at apply time.
- **The oversized-value limit was reported as a flat 1 MiB for every type, overstating an unbounded `varchar`'s headroom 16-fold.** Each figure is now per type, from the Aurora DSQL "Supported data types" page and re-verified live with incompressible values: `text` / `bytea` / `json` / `jsonb` 1 MiB, `varchar` 65,535 bytes, `char` 4,096 bytes. The compression carve-out is also scoped correctly — DSQL compresses the character and json types but **not** `bytea`, and only outside a key — so a `bytea` finding no longer implies headroom that does not exist.
- **The value-size probe could not measure a `json`/`jsonb` column, and reported the unmeasured column inside a green PASS.** `octet_length` has no json/jsonb overload (`function octet_length(jsonb) does not exist`), so that column's statement failed — and on PostgreSQL a failed statement leaves the transaction aborted, so **every column after it failed too**, contradicting the probe's own claim that "a failure on one column does not lose the others". Those unmeasured columns were then treated as "did not exceed" and listed in a PASS. Now: json/jsonb are measured as `octet_length(col::text)`, each failure is rolled back (re-applying the statement timeout, which `SET LOCAL` scoped to the rolled-back transaction) so the remaining columns are still probed, and a column that could not be measured is reported as not-determined — one such column downgrades the row from PASS to INFO.
- **Evaluation and Schema Conversion gave different answers about a partitioned table.** Evaluation said "Manual partitioning is not used by Aurora DSQL" / "Remove the manual partitioning", graded MANUAL with MEDIUM effort; the converter graded the identical fact a RECOMMENDATION with zero losses and a clean target DDL. The Evaluation text was also wrong twice for a PostgreSQL source: it used MySQL vocabulary, and "remove the manual partitioning" is work the operator must **not** do — this tool only reads the source — and does not need to do on the target either (the converter emits no `PARTITION BY` and collapses the partitions into one table). The finding now takes the converter's own engine-correct message, so the two cannot drift apart again.
- **The collation findings prescribed three remedies Aurora DSQL does not have.** All live-verified on a real cluster: DSQL accepts only the `C` / `POSIX` / `default` collations (`unicode` and `ucs_basic` are listed in `pg_collation` but rejected in use — `provided collation is not supported`), so "sort explicitly with a `COLLATE` clause the target supports" is impossible; there is no `unaccent()`; and because DSQL runs `LC_CTYPE=C`, `lower()`/`upper()` fold ASCII only — `lower('JOSÉ')` is `'josÉ'` — so the prescribed `lower(col)` normalisation does not work on accented data. The findings now say what is actually available (have the application store a normalised sort/match key) and disclose the half that was missing entirely: `ILIKE` and `~*` stop matching case-insensitively on non-ASCII text, which changes **which rows** a query returns, not just their order.
- **MySQL's oversized-LOB finding still set homework the tool can do.** "Confirm no value exceeds 1 MiB" now points at the Data Migration prerequisite check that probes it, matching the PostgreSQL sibling (v0.1.531).

## v0.1.534

### Fixed

- **A PostgreSQL identity / `serial` primary key silently lost its value generation, and the default tile said nothing changed.** The source column is `id integer GENERATED BY DEFAULT AS IDENTITY`; the default conversion emitted `"id" INT NOT NULL`. On Aurora DSQL an identity column must be `bigint` with an explicit `CACHE` (documented, and `dsql_lint` rejects `integer GENERATED ... AS IDENTITY` outright), so preserving the column's exact TYPE and preserving its GENERATION are mutually exclusive — and the tool defaulted to the type, which is the option that breaks the application. Worse, the default "Keep source PK" tile described itself as "Target key stays (id), exactly as on the source. **Nothing in the application changes**" while the conversion warning beside it said the application "must supply the value on every insert — an app that relied on the database generating it will fail or collide". A PostgreSQL source now defaults to `Server-generated (IDENTITY)` wherever it applies, so the target keeps generating the key; the Keep tile no longer promises no application change on a table whose key the source generates; and the Evaluation finding states the default rather than assigning work that is not needed. **A MySQL source is deliberately unchanged** — `AUTO_INCREMENT` is a different mechanism from an identity column, so converting it to one stays an explicit choice in Schema Conversion (it is also the engine where the widening can cost something: `bigint unsigned` maps to `numeric(20,0)`, and an identity must be `bigint`, so a value above 2^63-1 would fail to load).
- **The picker could show a strategy other than the one being emitted.** It derives its state from the stored (edited) target DDL and read "nothing stored" as Keep — correct only while Keep was the single default. It now resolves the same per-table, per-engine default the converter uses, so the selected tile matches the DDL panel beside it. For the same reason "Keep source PK" now STORES its own script instead of clearing the override: on a source whose default is the identity, clearing would have handed the operator the very strategy they just opted out of.
- **`IDENTITY_WITH_CACHE` on a composite primary key emitted an identity the picker never offers.** The eligibility rule (single-column primary key that IS the generated column) lived in the UI as a private helper while the converter applied the strategy unconditionally, so the two could disagree about the same table. One shared predicate now, and an explicit request on a composite key falls back to a plain integer with a reason instead.
...
- **The rendered source DDL hid the identity clause, so the side-by-side diff showed no change on the column that changes most.** A PostgreSQL identity column has NO `pg_attrdef` default, so the renderer's `DEFAULT` branch could not stand in for it and `id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` rendered as plain `"id" integer NOT NULL` — byte-identical to the converted target. An operator comparing source against target therefore saw nothing changed on the one column whose value generation does **not** survive the migration, which is exactly what the `AUTO_INCREMENT` finding warns about. (The MySQL renderer has always emitted `AUTO_INCREMENT` for this reason.) The clause is now shown, including which spelling: `pg_attribute.attidentity` was already read and then collapsed to a boolean, discarding whether the column is `GENERATED ALWAYS` (which REJECTS an explicit `INSERT` value without `OVERRIDING SYSTEM VALUE`) or `GENERATED BY DEFAULT` (which accepts it).
- **A generated column's expression was read from the catalog only as a flag, so the tool could not show the very thing it told the operator to go and read.** The panel printed `/* GENERATED column - expression not captured */` while the `GENERATED_COLUMN` finding said to "read the generating expression from the source, since it is not carried over" — impossible from inside the tool. `attgenerated` says only THAT a column is generated; the expression lives in `pg_attrdef` and needs no extra privilege, so it is now selected and rendered: `line_total numeric(12,2) GENERATED ALWAYS AS (((quantity)::numeric * unit_price)) STORED`. It is never emitted into target DDL — Aurora DSQL has no generated columns — it exists so the operator can see what to re-implement. Verified end to end against a live PostgreSQL 17.

## v0.1.531

### Fixed

- **CDC replicated NOTHING for a PostgreSQL partitioned table — silently.** The tool collapses a partitioned parent into one target table, so Debezium's `table.include.list` names the PARENT. But PostgreSQL's publication default is `publish_via_partition_root = false`: a publication naming a partitioned parent expands to its LEAVES and publishes every change under the **leaf's** relation name. Verified on a live PostgreSQL 17 — `pubviaroot` was `f` and `pg_publication_tables` returned `order_events_2026q3`/`_2026q4`, not `order_events`. Every change was therefore filtered out by the include-list and discarded: the connector ran, the slot advanced, and no row ever arrived. The publication is now created `WITH (publish_via_partition_root = true)`, which also fixes a second symptom — the reuse reconcile compares against `pg_publication_tables`, so it could never match the requested parent name and refused every re-run over a partitioned source.
- **`numeric` precision was clamped at 38, a limit Aurora DSQL no longer has — so digits were silently dropped.** `_DSQL_NUMERIC_MAX_PRECISION = 38` / `_MAX_SCALE = 37` was the limit the service enforced when the code was written (the comment quoted its "precision 39 must be between 1 and 38" error). DSQL now documents **maximum precision 1000, scale −1000..1000, default `numeric(18,6)`** — re-verified live: `numeric(1000,1000)` is accepted, `numeric(1001,0)` is rejected with "NUMERIC precision 1001 must be between 1 and 1000", and a 500-integer-digit + 500-decimal-digit value round-tripped **EXACT**. While the constant said 38, a `numeric(40,10)` was narrowed to `numeric(38,10)` and lost digits DSQL would have stored, the warning asserted a maximum that no longer exists, and the `NUMERIC_PRECISION` Evaluation rule graded such a column **UNSUPPORTED** ("a redesign is required before the table can migrate at all"). A MySQL `DECIMAL(65,30)` — the widest MySQL allows — now converts verbatim too, so the clamp no longer fires for a MySQL source at all. The limit lives in three constants and the tests are anchored on them, so the next service change has one place to update.
- **The partitioned-table conversion note told a PostgreSQL operator about MySQL.** It said "the source table uses MySQL native partitioning" and asked them to review `PARTITION (p1)` and `DROP/TRUNCATE PARTITION` — syntax PostgreSQL does not have. PostgreSQL partitioning is declarative and each partition is a separate table you `ATTACH`/`DETACH` or drop, which is how range archiving is normally done there. The note is now engine-aware and additionally states the PostgreSQL-only consequence the MySQL wording could not express: an index, `CHECK` or default that exists only on a **leaf** is not carried, because the collapsed table takes the parent's definition.
- **The oversized-LOB note still left homework on the PostgreSQL path.** v0.1.528 pointed the Evaluation finding and the MySQL conversion note at the new prerequisite check, but the PostgreSQL note — the one in use for a PG source — still said *"Check the largest values now"*. Its string is split across source lines, which is how the original search missed it. Both notes now name the check that answers the question.
- **Three user-manual passages contradicted the tool.** `06-limitations.md` claimed a PostgreSQL source's `ENUM` types "are converted to `text`" — they are not: an `ENUM` is a user-defined TYPE, the column is flagged UNSUPPORTED and the operator must remodel it (the same file says so two rows later). `02-evaluation-and-schema-conversion.md` and `11-customer-faq.md` claimed a PostgreSQL source emits **no** column DEFAULTs — carried since v0.1.433, and this run emits six kinds. The stale `38` was also copied into five passages across two chapters.
...
- **A MySQL ENUM column was reported as losing its allowed values, which this tool's own manual says it does not.** The finding said *"the allowed-value constraint is lost when the column is mapped to text"* and told the operator to *"Re-enforce the allowed values in the application layer"* at SIMPLE effort. But the converter emits `"status" TEXT CHECK ("status" IN ('new','paid','shipped'))` unconditionally, and the user manual documents exactly that mapping in all three languages while naming **ORDERING** as the loss (`06-limitations.md` goes out of its way to say the generated ENUM `CHECK` is a real, kept constraint). So Evaluation budgeted application work Schema Conversion had already done, never mentioned the loss that is real, and described SET — which genuinely loses its domain, because a multi-value comma-joined text gets no `CHECK` — as the same problem under the same sentence. ENUM and SET are now split: the ENUM clause states that the values are kept and that ordering, plus MySQL's empty-string invalid-value sentinel, is what breaks; the SET clause states the missing `CHECK` and the multi-value semantics, and only it asks for application-side enforcement. The hedge *"(or with a CHECK constraint if supported)"* is gone — DSQL supports `CHECK` and the tool already relies on it.
- **The CHECK-constraint finding asked the operator to judge an expression it refused to show them.** It listed only names, and MySQL auto-names an unnamed `CHECK` `<table>_chk_N` — so a trivially portable `amount > 0` and an unportable `json_valid(payload)` were indistinguishable, while the recommendation asked them to decide "if its expression is Aurora DSQL-compatible" and to size the work. `CheckConstraintDef.expression` is populated by both introspectors, its own docstring says it is kept for display, and Schema Conversion already prints it. Each constraint now renders as `name: CHECK (expression)`, bounded at six with the remainder named and counted, and the advice gives a portable and a non-portable example instead of a bare instruction.
- **The view-linting finding printed internal enum tokens and pointed at a report that does not exist.** The risk named `PESSIMISTIC_LOCK` / `UNSUPPORTED_FUNCTION` — strings that appear nowhere in the operator's view text and cannot be searched for — and the recommendation said *"see the application anti-pattern report for the exact locations"*, but the linter's only caller is this rule and no page, export, CLI command or documented step produces such a report. The finding now names the operator's own matched text with its line number (`SLEEP (line 1)`, `FOR UPDATE (line 1)`) and carries the linter's per-match recommendations, which were being computed and discarded.

## v0.1.526

### Fixed

- **A PostgreSQL non-key sequence column was reported as a data-quality item to clean up later when it is an application outage at cut over.** The finding said *"an INSERT that omits it writes NULL instead of the next number"* for every flagged column, and reassured that already-loaded rows are unaffected. But `serial` expands to `NOT NULL DEFAULT nextval(...)` and `GENERATED AS IDENTITY` implies NOT NULL — confirmed against a live PostgreSQL catalog AND this tool's own introspector — so for the dominant case the target column is NOT NULL with no default and an omitting INSERT is **REJECTED** (not-null violation, SQLSTATE 23502). Executing the tool's own emitted DDL and an omitting INSERT wrote **zero rows**. Schema Conversion, which does branch on nullability, already said "REJECTED on Aurora DSQL" about the same column. The finding now splits the columns by nullability, names each one's mechanism (`serial` / `GENERATED AS IDENTITY`), states the outcome each actually gets, and says the NOT NULL case must be handled BEFORE cut over. The risk text is copied verbatim into the AI strategist's prompt, so the wrong outcome had been propagating into the generated migration plan. **The existing test asserted the wrong outcome** (it built NOT NULL columns and then required `"writes NULL"` in the text) — the same shape as the v0.1.438 `lc_numeric` regression.
- **The index-limit finding predicted a failure this tool's own pipeline makes impossible.** It said the excess index *"fails with error 54000"* after Full Load had written every row — but since v0.1.433 Schema Conversion truncates the index list at DSQL's budget and names what it dropped, so the load SUCCEEDS and the target is simply left missing those indexes. The two steps stated opposite outcomes and the quieter one is the real one, so an operator prepared for a post-load failure and a re-run decision instead of choosing which indexes to keep. It now states the omission, the success, and the consequence (queries falling back to scans).
- **A PostgreSQL secondary index dropped because of an unsupported column type vanished with no record anywhere.** The skip is deliberate and tested — emitting a doomed `CREATE INDEX ASYNC` would just fail at apply — but nothing said it happened, on either step, so an operator remodelled `inet` to `text` as instructed and had no way to learn that the index on it went too. The adjacent `bytea` skip in the same function is reported by name; this now is too, naming each index with its offending column and type, and stating that remodelling the column is not sufficient on its own.
- **PostgreSQL triggers with the same name on different tables were indistinguishable rows.** A trigger name is unique only per table (`pg_trigger` is keyed on `(tgrelid, tgname)`), but the collector selected `tgname` alone — so a conventional `set_updated_at` on twelve tables produced twelve identical findings, none of which said which table to reimplement the logic for, and because the findings bucket is keyed by object name each duplicate row also repeated every sibling's concerns. The collected name now carries its table, exactly as the routine collector already carries identity arguments for overloads.
...
- **The activity log could say the migration was safe to cut over when it was not.** An audit of one full workshop run found six places where the durable log asserted something stronger than the code guarantees — the class of defect that matters most, because the log is the artifact an operator attaches to a change ticket and every one of these read as reassurance. (1) **The verdict overstated its own evidence.** `CHECKSUM verdict: MATCH — 11/11 table(s) matched … ready for cut-over` was emitted verbatim when a fast sweep had skipped the checksum for every count-matching table (10 of 11 compared by `COUNT(*)` alone) and when the `0 missing / 0 extra` clause covered a single reconcilable table out of eleven. (2) **A column the operator excluded from the migration was invisible on the verdict** — v0.1.491 disclosed it in the text/JSON report and the readiness panel but never in the log, so a `MATCH` stood over a column holding no data on the target. Both now append a bounded `NOT covered:` clause, on the validation verdict **and** on the cut-over acknowledgement that closes the journey. (3) **A PostgreSQL CDC teardown that left a WAL-pinning replication slot on the source logged `delete CDC infrastructure (success)`** — the cleanup is deliberately non-fatal, so the surviving slot (the code's own "production hazard": it pins source WAL until the disk fills) was reported only into the in-memory deploy log that the next lifecycle action wipes. The lifecycle line is now a WARNING naming what was left behind, and the teardown's source writes are audited as durably as their creation already was. (4) **Every audited source write was logged BEFORE it ran**, so a publication that was never created, a `REPLICA IDENTITY` never widened or a slot never dropped were recorded as facts; the line now follows the statement and a failure gets its own `FAILED:` line — including the swallowed compensating `DROP PUBLICATION`, which claimed an orphaned publication had been cleaned up while it was still on the source. (5) **`foreign keys: none in scope`** was printed both when the operator had **stripped every foreign key** (`preserve_foreign_keys=False`, the documented opt-out that makes referential integrity the application's job) and when the schema state the count reads was simply unavailable; those two zeros now say what they are. (6) **The foreign-key pass reported SUCCESS over constraints that are not enforced** — an orphan-skipped FK, or the constraints a stop request never reached (a 15-FK schema logged `3 applied, 0 skipped, 0 failed.` as a clean pass) — and logged nothing at all until it finished, so a pass killed part-way left no evidence while the target already carried enforced constraints. It now opens with a STARTED line and closes as a WARNING naming the unenforced count.
