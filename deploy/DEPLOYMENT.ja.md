...
**us-east-1**（2025）基準の**桁レベル**の概算であり — **見積もりではありません**。実際のコストは
リージョン・使用量で変動するため、[AWS Pricing Calculator](https://calculator.aws/) でアカウント基準
に算定してください。性質の異なる 2 つの部分:

**1. コントロールプレーンのアプリ**（デプロイ中は常時稼働）— 少額:

| リソース | 概算コスト | 備考 |
|---|---|---|
| ECS Fargate タスク | ~$0.02–0.15/hr | 評価用 0.5 vCPU/1 GB → 大規模 Full Load 時 4 vCPU/8 GB; 選んだ CPU/メモリに比例 |
| Application Load Balancer | ~$0.0225/hr + LCU | Fargate モードのみ（EC2/SSM モードにはなし） |
| S3（ジョブ/セッション状態・プラグインバケット）+ CloudWatch Logs | セント単位 | ごく僅か |
| Cognito（任意） | 管理者数名は無料枠内 | OIDC ログイン有効時のみ |

Aurora **DSQL**（ターゲット）とリージョン間**データ転送**は**使用量ベース**（書き込み・ストレージ・
egress）で事前算定不可 — 計算機を参照。

**2. 任意の CDC パイプライン**（CDC 移行の実行中のみ）— **ここが支配的**:

| リソース | 概算コスト |
|---|---|
| **Amazon MSK Serverless**（1 クラスタ） | ~$0.75/hr 基本 **+ パーティション時間/ストレージ** |
| **MSK Connect**（ソース + シンク） | ~6 MCU × $0.11 ≈ $0.66/hr |
| NAT ゲートウェイ（スタックが自前で作る場合のみ） | ~$0.045/hr + GB あたりデータ処理 |

→ **≈ デプロイ中は 1 時間あたり $1.4–$2.0**（アプリが CDC デプロイダイアログで
`estimate_cdc_hourly_cost` により表示する範囲と同じ）。CDC パイプラインは**カットオーバーの期間だけ**
プロビジョニングされ、**削除するまで課金が続く**ため — **移行完了後すぐに「CDC インフラの削除」を
実行**してください。CDC なしの Full Load はこれらを**一切**作りません。（アプリもデプロイ時に同じ
概算を文脈内で表示し、後片付けの段階で残存課金を警告します。）
