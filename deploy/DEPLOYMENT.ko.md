...
**us-east-1** (2025) 기준 대략적인 **자릿수 수준** 추정치이며 — **견적이 아닙니다**. 실제 비용은
리전·사용량에 따라 달라지므로 [AWS Pricing Calculator](https://calculator.aws/)로 계정 기준으로
산정하세요. 성격이 매우 다른 두 부분:

**1. 컨트롤 플레인 앱** (배포 중 상시 실행) — 소액:

| 리소스 | 대략 비용 | 비고 |
|---|---|---|
| ECS Fargate 태스크 | ~$0.02–0.15/hr | 평가용 0.5 vCPU/1 GB → 대형 Full Load 시 4 vCPU/8 GB; 선택한 CPU/메모리에 비례 |
| Application Load Balancer | ~$0.0225/hr + LCU | Fargate 모드만(EC2/SSM 모드는 없음) |
| S3(잡/세션 상태·플러그인 버킷) + CloudWatch Logs | 센트 수준 | 미미 |
| Cognito (선택) | 관리자 소수는 프리티어 | OIDC 로그인 활성화 시에만 |

Aurora **DSQL**(타깃)과 리전 간 **데이터 전송**은 **사용량 기반**(쓰기·스토리지·egress)이라 사전
산정 불가 — 계산기 참고.

**2. 선택적 CDC 파이프라인** (CDC 마이그레이션 실행 중에만) — **여기가 지배적**:

| 리소스 | 대략 비용 |
|---|---|
| **Amazon MSK Serverless** (1 클러스터) | ~$0.75/hr 기본 **+ 파티션-시간/스토리지** |
| **MSK Connect** (소스 + 싱크) | ~6 MCU × $0.11 ≈ $0.66/hr |
| NAT 게이트웨이 (스택이 자체 생성할 때만) | ~$0.045/hr + GB당 데이터 처리 |

→ **≈ 배포 중 시간당 $1.4–$2.0** (앱이 CDC 배포 다이얼로그에서 `estimate_cdc_hourly_cost`로
보여주는 범위와 동일). CDC 파이프라인은 **컷오버 기간에만** 프로비저닝되고 **삭제 전까지 계속
과금**되므로 — **마이그레이션 완료 즉시 "CDC 인프라 삭제"를 실행**하세요. CDC 없는 Full Load는 이
중 **아무것도** 만들지 않습니다. (앱도 배포 시점에 같은 추정치를 문맥에 맞춰 보여주고, 정리 단계에서
잔여 과금을 경고합니다.)
