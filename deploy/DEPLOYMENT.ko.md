...
| `ContainerCpu` | no | `512` | Fargate 태스크 CPU 단위. **Full Load는 CPU-bound**(소스 리더가 행마다 Python 타입 변환 수행)이므로 대규모 마이그레이션에서는 올려야 함 — 동일 데이터의 payments+orders 로드 실측에서 **512(기본)보다 4096(4 vCPU)에서 약 3.8배 빨랐음**. 평가용은 기본 `512`로 충분, 실제 TB급 Full Load에는 **2048–4096** 권장. [매뉴얼 §7.2](../docs/manual/ko/07-performance-and-tuning.md#72-병렬수-튜닝) 참고. |
