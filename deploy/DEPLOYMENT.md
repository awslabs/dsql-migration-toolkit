...
| `ContainerCpu` | no | `512` | Fargate task CPU units. **Full Load is CPU-bound** (the source reader does per-row type conversion in Python), so raise this for a large migration — a measured payments+orders load ran **~3.8× faster at 4096 (4 vCPU) than at the 512 default** on the same data. Default `512` is fine for evaluation; use **2048–4096** for a real TB-scale Full Load. See [manual §7.2](../docs/manual/en/07-performance-and-tuning.md#72-tuning-parallelism). |
