...
이어서 PostgreSQL → DSQL 제약(낙관적 동시성, 트랜잭션당 행/시간 한도,
...
- **스키마 변환** — 소스 스키마를 DSQL DDL로 변환(타입 매핑, 외래 키 보존(적재 후 재생성), 비동기 인덱스, PK 전략)하고,
  객체 트리에서 검토한 뒤 적용.
...
- **DSQL의 제약을 그대로 상속** — 트리거·저장 프로시저 없음, 트랜잭션당 행 한도,
  바이너리(`bytea`) 값의 1 MiB 한도 등.
