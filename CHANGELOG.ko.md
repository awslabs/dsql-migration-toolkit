...
- **외래 키 패스의 orphan 사전 게이트를 동시 실행(상한 8)하도록 바꿨습니다 — 더 이상 FK마다 순차 처리하지 않습니다.** v0.1.459의 27배 쿼리 개선 후에도 FK 15개·자식 15.5M행 스키마에서 약 6분이 걸렸고, 그게 패스 비용의 전부입니다 — `ADD CONSTRAINT ... NOT VALID` DDL 자체는 0.18초로 측정됐습니다. DDL은 **직렬 유지**(DSQL은 트랜잭션당 DDL 하나)이고, 읽기만 팬아웃하며 각 워커가 자기 단기 연결을 씁니다.
...
- **외래 키 패스의 orphan 사전 게이트가 약 27배 빨라졌습니다 — 결과는 완전히 동일합니다.** 페이징된 orphan 카운트가 `COUNT(*) FILTER (WHERE ... NOT EXISTS ...)`를 쓰고 있었는데, 이 형태는 상관 서브쿼리를 **행마다 스칼라 식**으로 평가하게 만듭니다. 그래서 두 테이블을 통째로 대조하는 대신 자식 행마다 부모 테이블을 한 번씩 조회했습니다. 이제 페이지를 `MATERIALIZED` CTE에 고정해 두 번 읽습니다 — 한 번은 `LEFT JOIN` anti-join으로 카운트, 한 번은 keyset 경계용입니다. 윈도우는 여전히 **필터되지 않으며**, 그것이 `FILTER`가 지키려던 정확성 성질입니다(필터된 윈도우는 PK 구간을 건너뛰어 과소집계합니다).
  - 실제 ap-northeast-2 클러스터에서 3M행 자식의 10만행 윈도우로 측정: **행당 0.59ms → 0.022ms**. FK 15개인 8.5M행 스키마(자식 행 합계 15.5M행 스캔)에서는 **약 153분 → 약 6분**입니다. 세 후보 형태 모두 기존 쿼리와 `(orphan_count, last_pk, row_count)`가 정확히 일치했습니다.
