...
- **드롭된 행 갭을 한 번 수락하면, 이후 모든 Full Load가 자기 갭을 조용히 수락했습니다.** "Accept quarantined rows & continue"가 세운 플래그를 **실행 시점**에 읽었기 때문에, 3행 갭을 수락한 뒤 **Re-run Full Load**를 누른 사용자는 — 본 적도 없고 동의한 적도 없는 갭에 대해 — "Full Load complete — with an accepted gap … you accepted that gap, so the next step is unblocked"라는 초록 결과를 받았습니다. 플래그는 갭 **크기**로만 스코핑되어 있었고(수락한 것보다 크지 않은 갭만 자동 수락), 그것으로는 부족합니다: **다른** 3행이든 **다른 테이블**의 3행이든 개수는 같고, 동의의 대상은 숫자가 아니라 특정 행이었습니다. 재실행하는 가장 흔한 이유에도 정반대입니다 — 문제 소스 값을 고친 뒤 사용자는 갭이 사라졌는지 **봐야** 하는데, 이어받은 수락이 바로 그것을 가립니다. 이제 새 실행은 수락을 지운 상태로 시작합니다. 이전 외래 키 적용 결과를 폐기하는 것과 같은 "새 적재는 이전 결과를 무효화한다" 블록에서 처리하므로, 사용자는 눈앞의 실행에 대해 다시 질문받습니다. **재시도**는 그대로 유지합니다: 갭이 이미 수락된 같은 실행의 남은 작업을 마무리하는 것이므로, 거기서 다시 묻는 건 의미 없는 클릭입니다.

## v0.1.545

### 수정

- **needle이 없는 커넥터 실패는, 과금 대기를 전부 치른 뒤 상태만 알려줬습니다.** `_connector_failure_message`는 워커 로그 tail을 가져와 알려진 시그니처 표와 맞춰 보고, **아무것도 안 맞으면 tail을 버리고** `<connector> entered FAILED state.`만 반환했습니다 — 커넥터 생성 대기(커넥터당 `connector_timeout_seconds` = 45분)를 방금 견뎌낸 사용자가 아무것도 알기 위해 MSK Connect 로그 그룹을 직접 열어야 했습니다. 이제 마지막 로그 줄을 원문 그대로 반복합니다. 이는 어떤 needle보다도 일반적입니다: 틀린 원인을 단정할 수 없고, 아직 needle이 없는 모든 실패 모드를 덮습니다. TIMEOUT 경로에서도 유지됩니다 — 유일한 태스크가 시작 중에 죽은 커넥터는 예산이 소진될 때까지 `CREATING`에 머물러 상태 자체가 아무 말도 하지 않으니, 바로 그 경로에서 가장 중요합니다. 발췌문은 길이가 제한되고(자바 스택 트레이스는 무한합니다), 알림과 다운로드 가능한 활동 로그에 닿기 전에 자격증명 마스킹을 통과합니다(Property 7). 커넥터 비밀번호는 Kafka Connect가 마스킹하는 별도 `database.password` 설정이므로, 알려진 누출이 아니라 다층 방어입니다.
- **도구가 대신 추가해 줄 수 없는 유일한 방화벽 규칙을 아무도 언급하지 않았습니다.** cdc-stack은 **자기** 커넥터 보안 그룹에만 규칙을 추가할 수 있습니다 — 소스를 향한 규칙은 EGRESS이고, 템플릿 자체 주석도 "라우트 테이블 + 소스 SG 인바운드 규칙이 실제 도달 가능성을 결정한다"고 인정합니다. 사용자 데이터베이스의 반대 방향 INBOUND 규칙은 사용자 몫이며, 그게 없으면 두 커넥터가 생성되고 과금되고 그저 RUNNING에 도달하지 않습니다 — 보안 그룹을 배우는 가장 비싼 방법입니다. 이제 Deploy 다이얼로그가 소스 **자신의** 보안 그룹 id와 포트를 채워서 그 규칙을 명시합니다. 다이얼로그가 이미 호출하는 `DescribeDBInstances` 응답에서 가져오므로 추가 API 호출도, 추가 IAM도 없습니다(그 응답을 받아 VPC id만 빼고 버리고 있었습니다). `info` 톤이고 Deploy를 **막지 않습니다**: 그 규칙은 VPC 전체 규칙으로 이미 충족된 경우가 많고, 도구는 사용자 리소스를 읽지 않고는 증명할 수 없으며, 거짓 차단이 더 나쁜 결함입니다.
- **"consistent" 배지가 같은 행의 "12m behind" 옆에 붙을 수 있었습니다.** `replication_lag_ms`는 자신을 "MAX(pk) 선단 폴백보다 **우선되는** 정확한 Stream lag 신호"라고 문서화하고, `consistency`의 docstring도 이미 "스트림 high-water(PK) / 시간 기반 lag 신호를 결합한다"고 주장했는데, `stream_caught_up`은 `pk_gap`만 읽었습니다. 그래서 측정된 12분 지연이 초록으로 표시되고, 단일 정수 PK가 없는 테이블은 선단 신호가 아예 없었습니다. 이제 시간 기반 lag을 먼저 읽고 PK 갭을 폴백으로만 씁니다. 임계값은 30초입니다(라이브 스트림은 늘 약간 뒤처지며, 커넥터 칩이 이미 쓰는 `lag_warn_seconds`와 일치시켜 테이블별 판정과 파이프라인 칩이 모순되지 않게 했습니다). 동일한 행 수도 더는 이를 덮어쓰지 못합니다: UPDATE만 있는 워크로드는 카운트를 바꾸지 않고 insert/delete 쌍은 0으로 상쇄되므로, 타깃은 크기가 같고 내용이 낡은 상태일 수 있습니다 — cut over 판단이 절대 초록으로 읽어선 안 되는 바로 그 상태입니다. lag이 `None`이면 여전히 유휴/따라잡음이며, 절대 "뒤처짐"이 아닙니다.
- **모니터링 범례가 라이브 지표를 go/no-go로 바꿨습니다.** "Stream lag"이 `"caught up" = 타깃이 최신(cut over 안전)`이라고 설명했고 — 도구가 그렇게 한 유일한 곳입니다 — 이는 도달 가능한 한 상태에서 틀립니다: 초기 스냅샷이 아직 **도달하지 않은** 테이블은 lag 데이터포인트가 없어서 "caught up"으로 읽히는데, 그 변경은 하나도 도착하지 않았습니다. Consistency 범례도 짝이 되는 공백이 있었습니다 — 초록이 아니면 조사하라고만 설명하고, 초록이 스냅샷 도달을 증명하지 않는다는 말은 없었습니다. 이제 둘 다 그 점을 말하고, 실제로 cut over를 게이트하는 권위(Validation의 정확 비교)를 가리킵니다. 전체 렌더를 구동하지 않고도 안전성 주장을 테스트로 고정할 수 있게 모듈 상수로 보관합니다.
- **Validation이 타깃 잉여 행의 원인을 하나로 단정했고, 재스냅샷 시작에서는 그게 틀린 원인이었습니다.** 안내 헤더는 "적용되지 않은 delete"라고 했고 본문은 `REPLICA IDENTITY`와 DLQ로 보냈습니다. 그 원인은 실재하지만, 같은 징후에 여기서 더 개연성 있는 온건한 원인이 있습니다: 재개할 복제 슬롯이 없는 CDC 시작은 재스냅샷하고, 새 스냅샷은 여전히 존재하는 행만 보고하므로 — Full Load와 CDC 시작 사이에 소스에서 삭제된 행은 delete로 스트리밍된 적이 없습니다. v0.1.544가 시작 시점에 이를 고지하고, Step 4에서 드러나는데, 이 안내만 읽은 사용자는 존재하지 않는 `REPLICA IDENTITY` 버그를 찾아다니게 됩니다. 이제 두 원인을 모두 명시하고, 어느 쪽인지 가리는 방법으로 활동 로그를 안내합니다. 해결책은 그대로이며 두 원인 모두에 유효합니다.

### 변경하지 않음 (조사 후 의도적으로 그대로 둔 것)

- **Kafka 토픽 retention은 MSK Serverless 기본값을 유지합니다.** 리뷰에서 설정되지 않은 `retention.ms`/`retention.bytes`가 큰 재스냅샷에서 행을 조용히 잃을 수 있다고 주장했습니다. MSK 문서로 검증한 결과 주장대로는 기각됐습니다: `retention.bytes`는 파티션당 **가장 오래된** 바이트를 자르므로 용량만으로는 아무것도 잃지 않고, 문서화된 파티션당 상한(ingress 5 MBps vs egress 10 MBps)상 싱크가 구조적으로 뒤처지지 않습니다 — 손실은 싱크가 7일 또는 파티션당 250 GiB 넘게 뒤처져야 하고, 그 결과는 이미 Validation의 정확 비교와 cut-over 게이트가 릴리스를 막습니다. `retention.ms=-1` 고정을 설계했다가 폐기한 이유는 그 수정의 실패 모드가 더 나쁘기 때문입니다: MSK Serverless는 그 값을 `--alter` 예시에서만 문서화하므로 `CreateTopics` 시점의 수용은 추론이고, 거부되면 토픽 시딩 커스텀 리소스가 실패해 **모든 사용자의 Start CDC를 하드 차단**합니다(v0.1.438 `lc_numeric` 회귀와 같은 모양). 또 `PLUGIN_VERSION` 범프가 필요해 기존 cdc-stack 전부가 Delete + Deploy를 해야 하고, 라이브 MSK 실행으로 게이트해야 합니다. 추론으로 배포하는 대신 분석이 반복되지 않게 여기에 기록합니다.

## v0.1.544

### 수정

- **`Full Load만` 실행한 뒤 CDC로 이어가는 PostgreSQL 사용자에게, 도구가 소스 전체를 다시 읽으려 한다는 사실을 아무도 알려주지 않았습니다.** `Full Load만` 실행은 복제 슬롯을 기록하지 않고, PostgreSQL 논리 슬롯은 과거 위치에 만들 수 없습니다 — 그래서 나중에 CDC를 시작하면 `snapshot.mode=initial` + `publication.autocreate.mode=filtered`로 동작합니다: 커넥터가 자체 publication과 슬롯을 만들고 선택된 모든 테이블을 재스냅샷한 뒤 스트리밍을 시작합니다. 무손실이고 정당한 경로지만, 소스를 한 번 더 읽고 모든 행이 다시 MSK를 통과해 DSQL로 들어가며, 적재 후 소스에서 삭제된 행은 타깃에 남습니다. 도구에는 이미 좋은 설명이 있었는데, 단 하나의 재등급 처리가 그 설명 전부를 동시에 껐습니다. `cdc_start_resnapshots`는 (선택이 아니라) 파생되는 값이라, 활성 버튼 옆에 빨간 FAIL이 놓이지 않도록 publication 부재를 `INFO, required=False`로 재등급했는데 — 같은 재등급이 remediation 문자열을 비우고, `resnapshot_resolvable_failures`를 `[]`로 만들어("두 경로 모두 소스를 다시 읽습니다" 분기가 아예 렌더되지 않음), Start 다이얼로그의 "소스 테이블을 다시 읽는 비용" 분기도 타지 않게 만들었습니다. 남은 것은 초록 배지 아래 접혀 있는 INFO 한 줄, 그것도 사용자가 그걸 선택한 것처럼 쓰인 문구뿐이었습니다. 이제: 그 사전점검 행은 **WARN**이고(여전히 `required=False`이므로 `can_proceed`는 true, 아무것도 차단하지 않음 — WARN이어야 섹션이 자동 확장되어 실제로 읽힙니다) 비용과 해결책을 모두 담습니다. 같은 고지가 **Deploy CDC infrastructure 다이얼로그의 시간당 비용 추정치 바로 아래**, 즉 취소가 아직 저렴한 마지막 지점에 나타납니다. 그리고 Start 다이얼로그의 "Ready to start CDC" 안내는 앞서 일어나는 스냅샷 단계를 빼놓고 커넥터가 "몇 분 안에 스트리밍을 시작한다"고 말하지 않습니다. 이 모든 것이 `pg_snapshot_mode` 자체에 고정된 하나의 술어에서 파생되므로, 고지가 설명 대상인 커넥터 설정과 어긋날 수 없습니다.
- **재스냅샷하는 그 경로에서 네 군데가 "gapless"를 단언했습니다.** start-point 카드의 호버 도움말은 조건 없이 "Automatic은 Full Load 스냅샷이 끝난 지점에서 정확히 재개합니다(갭 없음, 중복 없음)"라고 했습니다 — 카드에서 Automatic에 붙은 유일한 설명 텍스트라, 사용자가 모델을 형성하는 바로 그 순간에 틀린 모델을 가르쳤습니다. 테이블 집합 안내 글리프는 그 집합이 "handoff를 gapless하게 만드는 것"이라고 했고, 엔진 검사 없이 `full_load_ran`만으로 게이트됐습니다. 영구 활동 로그 기록은 워터마크가 있으면 언제나 "(gapless from the Full Load watermark)"를 붙였고 `slot_name`은 보지 않았습니다 — 침묵보다 나쁩니다. Step 4에서 소스에 없는 행이 보일 텐데 그걸 설명할 유일한 durable 기록이 바로 그 줄이기 때문입니다(이제 삭제 행 잔여를 기록합니다). 그리고 이 경로의 PostgreSQL 사용자가 *반드시* 가장 먼저 만나는 no-report Deploy 차단 메시지는 MySQL의 메커니즘인 "소스 바이너리 로그"를 확인하라고 했습니다 — 바로 다음 분기는 이미 엔진별로 분리돼 있던 같은 함수 안에서요.
- **start-point 카드가 그 아래 버튼과 모순됐습니다.** 슬롯 없는 PostgreSQL Automatic에서 카드는 활성화된 파란 "Start CDC" 바로 위에 호박색 "Action needed" 배지를 띄웠고(버튼 자신의 준비 판정은 WAL LSN으로 통과), **미리 선택된** 기본값을 "Automatic — needs a Full Load slot (unavailable)"로 표시했으며, 재스냅샷을 *Manual*이 하는 일로 제시했습니다 — 그래서 기본값을 그대로 두는 것이 "나는 그걸 선택하지 않았다"로 읽혔는데, 그게 정확히 Automatic이 하는 일입니다. 카드의 코드 주석은 이미 "슬롯 없이 Automatic을 고르는 건 dead end가 아니다"라고 적고 있었습니다. 이제 그 시작점은 **해결된 상태**로 취급되고, 라벨은 Automatic이 할 일을 명시하며, 안내는 원인별 해결책과 함께 비용을 말합니다. 커넥터 설정 프리뷰 — 그 안의 params 파일이 `PgSnapshotMode: initial` / `PgPublicationAutocreateMode: filtered`, 즉 도구 자신의 기계 판독 문구로 재스냅샷을 출력합니다 — 도 이 경로에서 early-return으로 사라지지 않습니다. MySQL은 늘 보여줬으니, PostgreSQL 경로가 다른 앱처럼 보이던 지점 하나가 함께 닫혔습니다.
- **한 메시지가 서로 다른 세 가지 원인을 모두 "슬롯 없음"으로 돌렸고, 그중 둘은 해결책까지 틀렸습니다.** `_cdc_watermark`는 `DONE`에 도달하지 못한 적재의 `slot_name`을 의도적으로 지우므로, 지워진 워터마크로는 "애초에 만들지 않음"과 "아직 인정하지 않음"을 구분할 수 없습니다 — 그래서 단지 실패한 적재의 사용자는 슬롯이 기록되지 않았다는 말을 듣고 전체 적재를 다시 돌리라는 안내를 받았습니다. 실패한 테이블만 재시도하면 충분한데도요(작업이 `DONE`이 되면 슬롯은 스스로 회복됩니다). 사용 가능한 슬롯을 두고 Manual을 고른 사용자도 슬롯이 없다는 말을 들었습니다. 이제 원인은 **원본 job**에서 읽고 경우마다 자기 해결책을 갖습니다: `no_slot` → "Full load + CDC"로 새로 시작, `unfinished_load` → 실패한 테이블 재시도, `manual` → Automatic으로 되돌리기, `no_watermark` → 여기서 읽은 것이 없으므로 "다시" 읽는다는 주장 자체를 하지 않음. 어떤 문구도 Full Load와의 소요 시간 등가를 말하지 않습니다 — 재스냅샷은 벌크 로더가 아니라 스트리밍 파이프라인을 통과하고, 이 저장소 자신의 측정값이 둘을 한 자릿수 차이로 두고 있습니다.
- **타깃 외래 키 사전조건이 MSK 지출이 승인된 *뒤에야* 확인됐습니다.** `Full Load만` 실행은 설계상 외래 키를 적용하고 검증하는데, 그게 바로 나중에 CDC로 전환한 세션이 강제된 FK를 타깃에 가진 채 도착하는 경로입니다 — 그런데 그 게이트는 Start CDC 다이얼로그와 워커 백스톱에만 있었습니다. 그래서 사용자는 MSK Serverless 클러스터 비용을 지불한 뒤 "Remove the target's foreign keys before streaming"으로 거부당하고, 클러스터는 유휴 상태로 과금됐습니다. 이제 같은 프로브가 Deploy 다이얼로그에서도 **순서 경고**로 실행됩니다(Deploy는 계속 활성 — 막다른 길이 아니기 때문입니다: 이 마이그레이션이 만든 FK는 Start 다이얼로그에 원클릭 제거가 있고 cut over가 다시 만들어 줍니다).
- **AI 어시스턴트의 CDC 진단이 모든 세션에서, 매 호출마다 죽어 있었습니다.** `get_cdc_pipeline_diagnostics`가 페이로드를 `_dm.cdc_start_mode()`가 아니라 `_dm.cdc_start_mode`로 만들어서, `json.dumps`가 `Object of type method is not JSON serializable`을 던지고 도구의 catch-all이 그걸 `{"status": "error", "message": "tool lookup failed"}`로 바꿨습니다. 모든 테스트 더블이 속성 가방이라 없는 이름이 `None`으로 풀리는 탓에 아무도 잡지 못했습니다. 회귀 테스트는 이제 **실제** `DataMigrationState`로 구동하고 도구가 `ok`로 답하는지 확인하므로, 앞으로 직렬화 불가한 필드가 추가되면 도구가 조용히 죽는 대신 빌드가 깨집니다.

### 문서

- 매뉴얼 §4.3의 핸드오프 설명("Debezium은 스냅샷 이후 첫 변경부터 스트리밍을 시작합니다 — '지금'부터가 아니며, 데이터를 다시 읽지도 않습니다")은 `Full load + CDC` 실행에만 참입니다. 이제 그 범위를 밝히고 `Full Load만` → CDC 동작을 가리킵니다. §10.1의 엔진 구분 없는 "CDC only … 새 Full Load 없이" 항목에 PostgreSQL 단서를 추가했습니다. `README.ko.md`와 `README.ja.md`에는 `README.md`가 가진 엔진 분기 문단이 빠져 있어서, 한국어·일본어 "한눈에 보기"는 워터마크가 두 경로를 무손실로 잇는다고만 주장했습니다. 3개 언어 모두 갱신했습니다.

## v0.1.543

### 수정

- **CDC VpcId 자동 채움이 관리형 배포에서는 애초에 동작할 수 없었고, 그 이유도 말하지 않았습니다.** 이 기능은 **소스** 데이터베이스의 `DBSubnetGroup`에서 VPC를 도출하는데, 앱 스택의 태스크 역할에 `rds:Describe*`가 **전혀** 없었습니다(IAM simulate: `rds:DescribeDBInstances` → `implicitDeny`, 두 배포 템플릿 모두 언급 없음). 읽기가 AccessDenied로 실패하고 예외는 버려져 필드가 그냥 비어 있었고, 이는 "도구가 시도조차 하지 않은 것"과 구분되지 않았습니다. RDS 권한이 있는 노트북 자격증명으로는 **동작하므로**, 권한 누락이 기능 퇴행처럼 보였습니다. 이제 두 템플릿이 `rds:DescribeDBInstances`와 `rds:DescribeDBClusters`를 부여합니다 — 두 번째가 중요한 이유는 Aurora 소스가 **클러스터** 엔드포인트로 주어지고 writer 멤버를 클러스터에서 해석하기 때문입니다 — 읽기 전용이며, 소스가 이 스택이 소유한 리소스가 아니라 운영자가 입력하는 임의의 엔드포인트이므로 `Resource: "*"`입니다. **이것은 템플릿 변경입니다: image-only 스택 업데이트로는 자동 채움이 계속 깨진 채로 남습니다**(v0.1.503의 PG-CDC 권한과 같은 함정) — 앱 스택에 full-template 업데이트가 필요합니다.
- **그리고 실패가 더 이상 조용하지 않습니다.** 도출이 실패한 **이유**를 기록해 VpcId 필드에 호박색으로 표시합니다: AccessDenied면 빠진 두 액션을 이름으로 대고 앱 스택을 업데이트하라고(또는 VPC를 직접 입력하라고) 안내하며, RDS가 아닌 소스(EC2의 자체 관리 PostgreSQL, 또는 교차 계정 엔드포인트)면 권한 탓이 아니라 도출할 `DBSubnetGroup`이 없다고 말합니다. 자동 채움은 어떤 경우에도 진행을 막지 않고, 필드는 항상 직접 수정할 수 있습니다.
...
- **아무것도 변환하지 못한 Generate가 버튼을 "Reset all" 뒤로 잠가서, 체크 실수 하나에 세션 전체를 리셋해야 했습니다.** 오브젝트 브라우저에서 스키마 행만 체크하면 테이블/뷰 미리보기가 나오지 않는데도 체크된 범위가 그대로 "생성된 범위"로 커밋됐습니다 — 그러면 화면이 생성 완료 모드로 바뀌고 "Generate DDL for selected"가 비활성화되면서 "새 선택으로 생성하려면 Reset all을 쓰라"는 안내가 떴습니다. DDL이 하나도 나오지 않은 시도인데도 말입니다. 이제 변환 결과가 없는 Generate는 **실패**로 취급해 범위를 그대로 두므로, 객체를 체크한 뒤 Generate를 다시 누르면 됩니다.
- **그 사실을 알리는 메시지가 평범한 회색 텍스트라 눈에 띄지 않았습니다.** "No tables or views were selected"가 `inline_hint(tone="neutral")`이어서, 아무 일도 하지 않은 Generate가 "눈에 보이는 일을 하지 않은" 것처럼 보였습니다. 진행을 막는 조치 필요 사항이므로 프로젝트 디자인 시스템 기준 `error`이고, 이제 빨간 알림으로 표시되며 가장 흔한 원인(스키마 행 체크는 그 안의 객체를 선택하지 않음)까지 설명합니다.
- **"Start over"가 RESET을 입력해도 한 글자를 더 눌러야 활성화됐습니다.** 확인 게이트가 raw DOM `input`/`keyup` 이벤트에서 돌면서 필드 값을 읽었는데, Quasar가 아직 동기화하지 않아 R-E-S-E-T를 다 입력해도 서버는 여전히 "RESE"를 들고 있었고 버튼은 비활성 상태였습니다. 스페이스를 누르면 이벤트가 한 번 더 발생해 그때 "RESET"이 도착하고 `.strip()`으로 통과 — 버튼이 무작위로 풀리는 것처럼 보인 이유입니다. 이제 게이트는 값을 페이로드로 갖는 모델 동기화 핸들러에서 돌고, 클릭 시점에 다시 검증합니다(RESET을 넣어 활성화한 뒤 다른 값으로 고쳤다면 리셋되지 않습니다).
- **"Start over" 후 시크릿 ARN과 DSQL 엔드포인트가 다시 나타나 "리셋이 안 됐다"처럼 보였습니다.** 리셋은 세션 자신의 시크릿 id와 타깃 설정을 실제로 지웁니다. 다시 보이는 값은 앱 스택의 `SourceSecretArn` / `DsqlClusterArn`이 attestation 환경변수를 통해 **마지막 fallback**으로 채워진 것이며, 사실 이 배포에서 사용할 수 있는 유일한 값입니다. 이제 각 필드가 출처를 밝히고 다른 값을 넣으면 왜 실패하는지 설명합니다(다른 시크릿은 태스크 역할이 읽을 수 없고, Aurora DSQL의 IAM 토큰은 한 클러스터용으로 발급됨). 운영자가 다른 값을 직접 입력한 경우에는 침묵합니다 — 그때는 기존의 잘못된 클러스터 경고가 올바른 메시지입니다.

## v0.1.538

### 수정

- **복합 키 전략을 선택하면 호박색 알림 2개가 떠서, 사용자가 직접 고른 선택이 테이블의 문제처럼 읽혔습니다.** 나란히 있는 "Server-generated (IDENTITY)" 타일의 같은 성격의 알림은 이미 `info`였으므로 선택기 내부도 일관되지 않았습니다. 프로젝트의 심각도 규칙(예상되거나 선택적인 것은 `info`, 호박색은 실제 문제)에 따라 둘을 분리했습니다. "복합 키로 애플리케이션에서 달라지는 것"은 `info`입니다 — 운영자가 방금 선택한 것의 결과이기 때문입니다(쿼리·조인·upsert가 전체 복합 키를 사용, 원래 키의 유일성은 도구가 만드는 UNIQUE 인덱스로 유지, CDC는 모든 키 컬럼을 캡처에 유지). 선행 컬럼의 불변성은 성격이 다르므로 **호박색을 유지**합니다: 도구가 검증할 수 없는 **전제 조건**이고, 어기면 행이 조용히 그리고 되돌릴 수 없게 사라집니다 — 이제 문제 보고가 아니라 확인 요청으로 읽힙니다("Confirm that 'user_id' is never UPDATEd on the source … The tool cannot verify this, and getting it wrong loses rows").
...
- **PostgreSQL identity / `serial` 기본 키가 값 생성 기능을 조용히 잃었고, 기본 타일은 바뀐 게 없다고 말했습니다.** 소스 컬럼은 `id integer GENERATED BY DEFAULT AS IDENTITY`인데 기본 변환은 `"id" INT NOT NULL`을 내보냈습니다. Aurora DSQL의 identity 컬럼은 `bigint`여야 하고 `CACHE`를 명시해야 하므로(문서에 기재되어 있고 `dsql_lint`도 `integer GENERATED ... AS IDENTITY`를 거부), 컬럼의 정확한 타입 보존과 값 생성 보존은 동시에 불가능합니다 — 그런데 도구는 애플리케이션을 깨뜨리는 쪽(타입)을 기본으로 택했습니다. 게다가 기본 타일 "Keep source PK"는 "Target key stays (id), exactly as on the source. **Nothing in the application changes**"라고 설명하면서, 바로 옆 변환 경고는 애플리케이션이 "매 insert마다 값을 공급해야 하며, DB가 생성해 주는 데 의존한 앱은 실패하거나 충돌한다"고 말했습니다. 이제 PostgreSQL 소스는 적용 가능한 곳에서 `Server-generated (IDENTITY)`를 기본값으로 사용해 대상이 키를 계속 생성하고, Keep 타일은 소스가 키를 생성하는 테이블에서 "앱 변경 없음"을 약속하지 않으며, Evaluation 결과도 불필요한 작업을 지시하는 대신 기본 동작을 설명합니다. **MySQL 소스는 의도적으로 그대로 둡니다** — `AUTO_INCREMENT`는 identity 컬럼과 다른 메커니즘이므로 identity로의 전환은 Schema Conversion에서의 명시적 선택으로 남습니다(확장 비용이 실제로 발생하는 엔진이기도 합니다: `bigint unsigned`는 `numeric(20,0)`으로 매핑되고 identity는 `bigint`여야 하므로 2^63-1을 넘는 값은 적재에 실패합니다).
- **선택기가 실제로 생성되는 전략과 다른 전략을 표시할 수 있었습니다.** 선택기는 저장된(편집된) 대상 DDL에서 상태를 유도하며 "저장된 것 없음"을 Keep으로 읽었는데, 이는 Keep이 유일한 기본값일 때만 옳았습니다. 이제 컨버터와 동일한 테이블별·엔진별 기본값을 해석하므로 선택된 타일이 옆 DDL 패널과 일치합니다. 같은 이유로 "Keep source PK"는 오버라이드를 지우는 대신 자기 스크립트를 저장합니다 — 기본값이 identity인 소스에서 지우기는 운영자가 방금 거부한 그 전략을 다시 돌려주는 동작이었습니다.
- **복합 기본 키에 `IDENTITY_WITH_CACHE`를 적용하면 선택기가 제공하지도 않는 identity가 생성됐습니다.** 적용 가능 규칙(생성 컬럼과 동일한 단일 컬럼 기본 키)이 UI의 비공개 헬퍼로 있었고 컨버터는 전략을 무조건 적용했기 때문에, 같은 테이블에 대해 두 곳이 다른 판단을 할 수 있었습니다. 이제 하나의 공유 술어를 쓰고, 복합 키에 대한 명시적 요청은 사유와 함께 평문 정수로 대체됩니다.
...
- **렌더된 소스 DDL이 identity 절을 숨겨서, 가장 크게 바뀌는 컬럼이 "변화 없음"으로 보였습니다.** PostgreSQL identity 컬럼은 `pg_attrdef` 기본값이 **없으므로** 렌더러의 `DEFAULT` 분기가 대신할 수 없고, `id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` 가 그냥 `"id" integer NOT NULL` 로 렌더됐습니다 — 변환된 타깃과 바이트 단위로 동일합니다. 소스와 타깃을 나란히 비교하는 운영자는 **값 생성이 마이그레이션에서 살아남지 못하는** 바로 그 컬럼에서 아무 차이도 못 봤습니다. `AUTO_INCREMENT` finding이 경고하는 내용이 정확히 그것입니다. (MySQL 렌더러는 같은 이유로 늘 `AUTO_INCREMENT` 를 출력해 왔습니다.) 이제 절을 표시하며, 어느 스펠링인지도 함께 밝힙니다: `pg_attribute.attidentity` 를 이미 읽고 있었는데 boolean으로 좁혀서, `GENERATED ALWAYS`(`OVERRIDING SYSTEM VALUE` 없는 명시적 `INSERT` 값을 **거부**)인지 `GENERATED BY DEFAULT`(수락)인지를 버리고 있었습니다.
- **생성 컬럼의 식을 카탈로그에서 플래그로만 읽어서, 도구가 운영자에게 "가서 읽으라"고 한 바로 그것을 보여줄 수 없었습니다.** 패널은 `/* GENERATED column - expression not captured */` 를 출력하는데 `GENERATED_COLUMN` finding은 "read the generating expression from the source, since it is not carried over" 라고 했습니다 — 도구 안에서는 불가능했습니다. `attgenerated` 는 컬럼이 생성 컬럼**이라는 사실**만 말합니다. 식은 `pg_attrdef` 에 있고 추가 권한도 필요 없으므로, 이제 함께 선택해 렌더합니다: `line_total numeric(12,2) GENERATED ALWAYS AS (((quantity)::numeric * unit_price)) STORED`. 타깃 DDL로는 절대 내보내지 않습니다 — Aurora DSQL에는 생성 컬럼이 없습니다 — 운영자가 무엇을 재구현해야 하는지 보기 위한 것입니다. 라이브 PostgreSQL 17으로 종단 확인했습니다.
...
- **재렌더가 Connect 화면의 운영자 선택 4개를 되돌리고, 복원된 세션이 결정 2개를 잃었습니다.** NiceGUI는 렌더마다 화면을 다시 만들기 때문에, 리터럴로 초기값을 주는 위젯은 운영자가 고른 값을 조용히 버립니다 — Connect를 떠났다 돌아오는 것만으로 충분했습니다.
  - **소스 엔진(MySQL / PostgreSQL)이 되돌아갔습니다.** `on_engine_change` 가 아무도 보지 않는 빌더 로컬 dict만 바꾸고, 타일은 `source_config.source_type` 에서 다시 시드했습니다. **다른** 엔진으로 이미 검증된 config가 세션에 있으면 새로 고른 PostgreSQL이 다음 재렌더에서 사라지고 그 엔진의 host/port/database가 함께 되살아났으며, 워크플로 래치는 단방향이라 Schema Conversion이 잘못된 dialect 상대로 계속 열렸습니다. 이 프로젝트는 과거에 정확히 그 사고를 겪었습니다("PostgreSQL 소스를 MySQL dialect로 변환하고 적용했다"). 이제 선택을 세션에 기록하고 **라이브 config보다 먼저** 참조하며, 테스트 성공 시 해제합니다(그때는 config가 진실이므로).
  - **Authentication 선택이 username/password로 되돌아갔습니다** — 고른 시크릿은 세션에 남아 있는데도요. 라디오 초기값이 하드코딩 리터럴이었습니다. 이제 기록된 시크릿 참조에서 도출합니다.
  - **Secrets Manager ARN 입력란이 비어서 돌아왔습니다.** 이걸 함께 고치지 않으면 라디오 수정이 오히려 해로웠습니다(다음 테스트가 빈 시크릿 id를 resolve해 실패). 이제 세션의 참조를 우선하고, 배포가 부여한 ARN은 최초 기본값으로 유지합니다.
  - **비밀번호 입력란이 세션 비밀번호보다 배포 `.env` 기본값을 우선했습니다.** 다른 모든 소스 필드는 이미 세션을 우선하는데 이것만 아니어서, 다른 DB로 재지정한 뒤 재렌더된 폼이 새 host·username 옆에 **기본** 비밀번호를 보여 줬습니다 — 어긋난 자격증명 조합이고, 다음 테스트가 그걸로 동작 중인 인메모리 비밀번호를 덮어썼습니다.
  - **Secrets Manager 참조를 영속화합니다.** 복원된 세션이 참조와 함께 인증 **방식**도 유지합니다. 자격증명이 아니며(비밀번호는 영속화하지 않음), CDC 배포/제거가 이 값으로 "도구가 삭제해도 되는 시크릿인지"를 판단합니다.
  - **"격리 행을 수용하고 계속" 결정을 영속화합니다**(승인한 갭 크기 포함). 이전엔 메모리에만 있어서 복원된 세션이 그 결정이 없던 것처럼 돌아왔습니다: 패널이 "complete — with an accepted gap"에서 amber 경고로 역행하고, 재실행 시 `FullLoadIncompleteError` 로 Data Migration이 FAILED가 되며 **수용 경로가 수행하는 identity 시퀀스 동기화를 건너뛰었습니다**.
...
- **MySQL ENUM 컬럼이 허용값을 잃는다고 보고했는데, 이 도구의 매뉴얼이 그렇지 않다고 적고 있습니다.** finding이 *"the allowed-value constraint is lost when the column is mapped to text"* 라고 하면서 *"Re-enforce the allowed values in the application layer"* 를 SIMPLE 난이도로 지시했습니다. 그런데 변환기는 `"status" TEXT CHECK ("status" IN ('new','paid','shipped'))` 를 **무조건** 내보내고, 사용자 매뉴얼은 3개 언어 모두 정확히 그 매핑을 문서화하면서 손실은 **ordering**이라고 지목합니다(`06-limitations.md` 는 생성된 ENUM `CHECK` 가 실제로 유지되는 제약이라고 따로 못 박습니다). 즉 Evaluation은 Schema Conversion이 이미 한 앱 작업을 예산에 넣게 하고, 실제 손실은 언급하지 않았으며, 진짜로 도메인을 잃는 SET(다중값이 콤마로 이어진 text가 되어 `CHECK` 가 없음)을 같은 문장 아래 같은 문제로 서술했습니다. 이제 ENUM과 SET을 분리합니다: ENUM 절은 값이 유지되며 깨지는 것은 ordering과 MySQL의 빈 문자열 무효값 센티널임을 밝히고, SET 절은 `CHECK` 부재와 다중값 의미 손실을 밝히며 앱 측 강제는 SET에만 요청합니다. *"(or with a CHECK constraint if supported)"* 라는 유보 표현도 제거했습니다 — DSQL은 `CHECK` 를 지원하고 도구가 이미 그것에 의존합니다.
- **CHECK 제약 finding이 보여주지도 않는 식을 판단하라고 요구했습니다.** 이름만 나열했는데 MySQL은 이름 없는 `CHECK` 를 `<table>_chk_N` 으로 자동 명명하므로, 쉽게 이식되는 `amount > 0` 과 이식 불가한 `json_valid(payload)` 가 구별되지 않았습니다 — 그런데 권고문은 "식이 Aurora DSQL과 호환되는지" 판단하고 작업량을 산정하라고 했습니다. `CheckConstraintDef.expression` 은 두 introspector 모두 채우고, 그 docstring이 표시용으로 보관한다고 적고 있으며, Schema Conversion은 이미 출력합니다. 이제 각 제약을 `name: CHECK (expression)` 으로 렌더하고, 6개까지 상세히 쓰고 나머지는 이름과 개수로 요약하며, 조언에 이식 가능/불가 예시를 함께 제시합니다.
- **뷰 린팅 finding이 내부 enum 토큰을 출력하고 존재하지 않는 리포트로 안내했습니다.** risk가 `PESSIMISTIC_LOCK` / `UNSUPPORTED_FUNCTION` 을 적었는데 — 운영자의 뷰 텍스트에는 없고 검색조차 불가능한 문자열입니다 — 권고문은 *"see the application anti-pattern report for the exact locations"* 라고 했습니다. 그러나 린터의 유일한 호출자가 이 규칙이고, 그런 리포트를 만드는 페이지·내보내기·CLI 명령·문서화된 절차가 **없습니다**. 이제 운영자 자신의 매칭 텍스트와 줄 번호를 명시하고(`SLEEP (line 1)`, `FOR UPDATE (line 1)`), 계산해 놓고 버리던 린터의 매치별 권고를 함께 전달합니다.
...
- **타깃 DSQL 클러스터를 바꾸면 모든 화면이 이전 클러스터의 카탈로그를 계속 보여주고, 그걸로 판단까지 했습니다.** 타깃 카탈로그는 Evaluation이 한 번 읽어 그 결과에 저장하고, Connect 화면의 타깃 무효화는 Next 버튼만 다시 잠그므로, 클러스터를 바꾼 뒤에도 Schema Conversion 오브젝트 브라우저가 클러스터 A의 스키마를 그대로 렌더했습니다 — 어느 클러스터를 보여주는지 화면 어디에도 표시가 없었습니다. 표시를 넘어선 결과가 둘 있었습니다. **Data Migration은 그 카탈로그로 로드 범위를 결정합니다**(타깃에 이미 있는 테이블을 마이그레이션 대상으로 간주하며, 이게 CDC 캡처 목록과 Validation 범위까지 정합니다). 그래서 클러스터 A의 카탈로그가 **연결된 클러스터에 존재하지 않는 테이블에 GO를 냈고** — 이후 로드가 아무도 만들지 않은 릴레이션에 INSERT — 반대 방향이면 선택을 A의 부분집합으로 **조용히 축소**했습니다. 그리고 Evaluation의 **"No conflicts on the target" 초록 전체 통과**가 거짓이었습니다: 충돌은 A 기준으로 계산되므로, 연결된 클러스터가 지금 만들려는 그 테이블을 이미 갖고 있을 수 있습니다. 이제 평가 결과가 타깃 카탈로그를 **어느 엔드포인트에서 읽었는지** 기록하고, 모든 소비자가 **사용 시점에** 그 표식을 확인합니다 — Connect에서는 할 수 없습니다(`set_target`이 연결 테스트 *전에* 실행되고, 키 입력마다 도는 무효화가 정상 카탈로그를 파괴해선 안 되며, 복원된 세션 스냅샷은 Connect 이벤트 없이 불일치 쌍을 다시 만들어 냅니다). Data Migration은 **fail-closed** 로 이번 세션에 DDL을 생성한 테이블만 제시하고, Schema Conversion은 카탈로그를 보류하며 **파생된 존재 확인기를 폐기**하고(이전에는 옛 클러스터 기준으로 "exists on target"을 계속 답했습니다) 두 엔드포인트를 이름으로 적은 notice와 라벨 있는 새로고침 버튼을 보여줍니다. Evaluation은 카운트와 거짓 전체 통과 대신 "out of date" notice로 대체합니다. 두 "Refresh" 동작은 표식을 다시 찍으므로 새로고침이 실제로 상태를 해소합니다. 표식은 영속화되어 복원된 세션도 불일치를 감지합니다. 표식이 없는 결과(이 릴리스 이전에 기록된 것)는 신뢰하므로 기존 세션 동작은 그대로입니다.

### 알려진 문제

- **타깃 클러스터를 바꿔도 단계 완료 상태와 이전 Validation 판정은 초기화되지 않습니다.** 클러스터 A에 스키마를 적용했으면 Schema Conversion이 `DONE`, 프리체크 리포트가 all-PASS로 남아 빈 클러스터 B 상대로 "Start migration"이 활성 상태입니다(Full Load 확인 대화상자는 여전히 라이브 타깃을 조회해 테이블 없음을 경고합니다). 더 심각하게, 클러스터 A에서 얻은 `ValidationReport`가 여전히 컷오버 런북을 해제합니다 — `ValidationReport`에 타깃 식별자가 없어 현재로선 오염을 감지할 수 없습니다. 수정 전까지는 타깃 클러스터를 바꾼 뒤 Validation을 다시 실행하세요.
...
- **CDC 마이그레이션에서 활동 로그에 스트림이 실제로 옮긴 내역이 없었습니다.** `start CDC connectors` 와 `validation started` 사이에 데이터 경로 줄이 0건이었습니다: 테이블별 Inserts/Updates/Deletes는 sink의 CloudWatch 메트릭에서 프로세스 메모리로 읽혀 화면에만 렌더됐고 — 영속화도, 로깅도 없었습니다 — 컷오버 승인 근거의 절반이 브라우저 탭과 함께 사라졌습니다. 이제 **스로틀된 roll-up** 이 누적 테이블별 카운트를 기록하고(변경 시에만, 최대 10분당 1줄, 활동량 많은 테이블 우선 + 명시적 `+N more` 로 500자 detail 상한에서 조용히 잘리지 않게), **Stop 시 최종 누적 1줄**을 추가합니다 — Stop이 커넥터와 그 CloudWatch 디멘션을 삭제하므로 그 메트릭이 존재하는 마지막 순간입니다. 연속 진행률을 로그 파일에서 제외한 원래 설계 결정은 유지됩니다: 복제 지연은 화면에만 남고, 스로틀 마커는 마이그레이션 상태에 두어 Start over가 함께 지웁니다.
- **격리 레코드 수가 조용히 불완전할 수 있었고, 이 숫자가 바로 컷오버 판단이 의존하는 값입니다.** 보이지 않는 경계가 둘 있었습니다. 데드레터 읽기 위치가 프로세스 메모리에만 있어서 Fargate 태스크 교체나 앱 재시작 뒤 재구성된 컨트롤러가 커서 없이 시작했고, 커서 없는 첫 읽기는 블라인드 6시간 창보다 더 과거를 볼 수 없습니다 — 그보다 오래된 것은 해당 모듈의 표현대로 "절대 집계될 수 없는" 이벤트입니다. 이제 커서를 **세션 스냅샷에 영속화**합니다. 또 CloudWatch 읽기가 100건 상한의 단일 페이지만 읽고 `nextToken` 을 따르지 않아 나머지를 조용히 버렸습니다 — 이제 **페이지네이션을 따릅니다**(페이지 상한 있음). 어느 경계든 적용됐다면 카운트를 총계가 아니라 하한으로 보고합니다: DLQ 배지가 `at least N quarantined` 로 바뀌고, 어떤 경계였는지와 완전한 기록이 어디 있는지 알리는 warning notice가 붙고, **영속 로그 1줄**이 남아 활동 로그만 읽는 검토자도 그 수가 최소값임을 압니다.

## v0.1.521

### 수정

- **활동 로그가 컷오버해도 안전하다고 말할 수 있었는데, 실제로는 아니었습니다.** 워크샵 1회 완주 로그를 감사해 **코드가 보장하지 않는 것을 단언하는 6곳**을 찾았습니다 — 로그는 운영자가 변경 티켓에 첨부하는 산출물이고 이 6건 전부가 "안심"으로 읽히므로 가장 중요한 결함 유형입니다. (1) **판정이 자기 증거를 과장했습니다.** Fast sweep이 카운트 일치 테이블의 checksum을 건너뛰었을 때(11개 중 10개가 `COUNT(*)`만으로 비교)도, `0 missing / 0 extra` 가 11개 중 reconcile 가능한 1개만 커버했을 때도 `CHECKSUM verdict: MATCH — 11/11 table(s) matched … ready for cut-over` 가 그대로 나왔습니다. (2) **운영자가 마이그레이션에서 제외한 컬럼이 판정 줄에 보이지 않았습니다** — v0.1.491은 텍스트/JSON 리포트와 준비 패널까지만 공개했고 로그에는 없어서, 타깃에 데이터가 없는 컬럼 위에 `MATCH` 가 성립했습니다. 이제 둘 다 길이가 제한된 `NOT covered:` 절을 붙이고, **Validation 판정과 여정을 닫는 컷오버 승인 줄 양쪽**에 기록합니다. (3) **PostgreSQL CDC 인프라 제거가 소스에 WAL 고정 복제 슬롯을 남겨도 `delete CDC infrastructure (success)` 로 기록됐습니다** — 정리는 의도적으로 비치명적이라, 살아남은 슬롯(코드 주석이 직접 "production hazard"라 부르는 것: 소스 디스크가 찰 때까지 WAL을 고정)이 다음 라이프사이클 동작이 지우는 인메모리 배포 로그에만 남았습니다. 이제 라이프사이클 줄이 남은 것을 이름으로 적은 WARNING이 되고, teardown의 소스 쓰기도 생성과 같은 수준으로 영속 감사됩니다. (4) **감사되는 모든 소스 쓰기가 실행 *전에* 기록됐습니다** — 만들어지지 않은 퍼블리케이션, 넓혀지지 않은 `REPLICA IDENTITY`, 드롭되지 않은 슬롯이 사실로 남았습니다. 이제 구문 실행 뒤에 기록하고 실패는 `FAILED:` 줄을 따로 남깁니다 — 삼켜지던 보상 `DROP PUBLICATION` 포함(고아 퍼블리케이션이 소스에 그대로 있는데 정리됐다고 말했습니다). (5) **`foreign keys: none in scope`** 가 운영자가 **FK를 전부 제거한 경우**(`preserve_foreign_keys=False`, 참조 정합성을 앱의 책임으로 넘기는 문서화된 옵트아웃)에도, 카운트가 읽는 스키마 상태가 그냥 없는 경우에도 출력됐습니다. 이제 두 0을 각각 무엇인지 말합니다. (6) **FK 패스가 강제되지 않은 제약 위에 SUCCESS를 기록했습니다** — orphan으로 건너뛴 FK, 그리고 stop 요청이 도달하지 못한 제약들(15개 FK 스키마가 `3 applied, 0 skipped, 0 failed.` 를 깨끗한 패스로 기록) — 게다가 끝날 때까지 아무것도 기록하지 않아서, 중단된 패스는 타깃에 이미 제약이 생겼는데도 증거를 남기지 않았습니다. 이제 STARTED 줄로 시작하고 강제되지 않은 개수를 적은 WARNING으로 닫습니다.

## v0.1.520

### 수정

- **"Access denied reading the secret"가 운영자를 편집할 수 없는 IAM 역할로 보냈습니다.** 관리형 배포에서 Secrets Manager 인증을 선택하면 *"The AWS identity needs secretsmanager:GetSecretValue"* 로 실패했습니다 — 사실이지만 막다른 길입니다: 태스크 역할의 읽기 권한은 **앱 스택의 `SourceSecretArn` 파라미터에서 생성**되므로 배포 시점에 고정된 **시크릿 단 하나**만 커버하는데, Connect 화면은 입력된 아무 ARN이나 받습니다. 그 파라미터를 비워 두면(기본값) **읽을 수 있는 시크릿이 아예 없고**, 앱은 자기 역할을 읽을 수 없어 그 사실을 말할 수도 없었습니다. 이제 배포 템플릿이 부여된 ARN을 `DSQL_MIGRATOR_SOURCE_SECRET_ARN` 으로 알려 주므로(`DSQL_MIGRATOR_CDC_MSK_ACCESS` 와 같은 방식), 도구는: 실제로 동작하는 그 ARN을 시크릿 입력란에 미리 채우고; ARN을 입력하기 **전에** 부여된 시크릿이 없다는 사실을 먼저 알리고; 거부되면 IAM이 아니라 실행 가능한 해결책 — 스택 파라미터 설정, 또는 사용자명/비밀번호 사용 — 을 이름으로 가리킵니다. 부여된 시크릿이 요청한 것과 같다면 IAM/KMS 안내를 유지합니다(고객 관리형 키 정책이 통상적 원인). 시크릿 **이름**만 입력했거나 Secrets Manager의 6자 접미사가 없는 ARN도 부여된 ARN과 매칭되므로, 메시지가 자기모순에 빠지지 않습니다. 비관리형 실행(노트북, 직접 구성한 호스트)에는 이 표식이 없어 기존 문구를 그대로 씁니다.

## v0.1.519

### 수정

- **AI DBA가 Debezium 커넥터를 손으로 편집하라고 안내했는데, 이 UI로는 불가능하고 애초에 필요도 없습니다.** 격리된 초과 행에 대해 물으면 `"column.exclude.list": "ecommerce.product_media.content"` 와 "커넥터를 재시작하세요"로 답했습니다 — 운영자가 AWS 콘솔에서만 따를 수 있는 조언이고, 대상은 **이 도구가 소유한 설정**입니다: 마이그레이션의 LOB 제외에서 도구가 `column.exclude.list`를 직접 씁니다. CDC 그라운딩이 그 사실을 몰랐고, **초과-값 격리 클래스 자체를 아예 몰랐습니다**(22P02 / 22001 / 23505 / 23502만 나열하고 1 MiB 한도는 없음) — 그래서 초과 행을 재시도 가능한 것으로 취급했습니다. 이제 그라운딩이 그 클래스와 **영구성**을 명시하고, 커넥터 설정 수동 편집을 **금지**하며, 대신 **앱 내 컨트롤**을 이름으로 가리키고 순서가 있는 런북을 담습니다.

  또한 저장소가 사실로 기록해 두었던 전제를 정정합니다: `ColumnExcludeList`는 커넥터 생성 시에 **고정되지 않습니다**. Start CDC가 매 패스마다 재전송합니다(실제 파라미터 빌더를 실행해 확인 — `submit_update`에 도달하고 이전 값을 가진 스택에 대해 `config_changed`가 True). 즉 새 제외는 **실행 중인** 파이프라인에 cdc 스택 재배포 없이 반영됩니다.

### 추가

- **CDC 데드레터 화면에서 DSQL 1 MiB 값 한도로 버려진 행을 복구할 수 있게 되었습니다.** 읽기 전용이었기 때문에, Full Load에서 그 한도를 만난 운영자는 앱 안에서 고칠 수 있었고("Exclude column & reload") 스트림에서 만난 운영자는 고칠 수 없었습니다 — 위의 AI 조언이 메우고 있던 바로 그 공백입니다. 이제 DLQ 패널이 영향 테이블마다 앰버 카드 하나를 버려진 행의 PK와 함께 보여주고 **Exclude column…** 액션을 제공하며, **앱 내 해법이 있는 유일한 격리 클래스에만** 렌더됩니다(중복 키나 드리프트 거부에 그 해법을 제안하면 거짓입니다).

  피커는 정확히 맞는 컬럼을 미리 체크합니다. 이건 CDC 경로가 Full Load보다 **잘 할 수 있는** 부분입니다: 싱크가 문제 컬럼을 **이름으로** 말합니다(`Value for column 'content' exceeds …`). Full Load는 드라이버 메시지밖에 없고 그건 **타입**을 말하므로 피커가 타입 토큰으로 후보를 추론해야 합니다.

  Full Load처럼 원클릭 "제외 후 재적재"로 만들지 **않았습니다.** 순서가 중요하고 두 단계는 다른 버튼이기 때문입니다: **(1) Stop CDC** — 커넥터가 현재 컬럼 집합으로 이미 오프셋을 커밋했고, 라이브 싱크 중 재적재는 **APPEND 모드로 강제**되어 타깃의 기존 행이 옛 컬럼 집합을 유지합니다; (2) 컬럼 제외; (3) DROP을 선택해 테이블 재적재; (4) Start CDC 다시 — 새 캡처 설정이 재전송됩니다. 대화상자는 원자적으로 할 수 있는 한 단계만 수행하고 나머지를 순서대로 밝히며, 액션 자체는 **기존 가드를 존중합니다** — CDC 스트리밍 중에는 버튼이 이유와 함께 **비활성으로 표시**되고 숨겨지지 않습니다.

## v0.1.518

### 수정

- **CDC DLQ 격리 레코드가 어떤 DML이 버려졌는지 말하지 않아 복구가 불가능했습니다(싱크 플러그인 v43).** CDC는 **상태**를 복제하므로 격리된 INSERT·UPDATE는 소스의 **현재 행**을 다시 읽으면 수렴합니다 — 중간 값은 필요 없습니다. 격리된 DELETE는 반대입니다: 행이 소스에서 사라졌으므로 **어떤 소스 조회로도 타깃에 남아 있다는 사실을 알 수 없고**, 타깃에서 직접 지워야 합니다. 두 조치는 정반대이고, 그것을 구별할 유일한 단서인 작업 종류가 싱크의 지역 변수로만 읽히고 **로깅되지 않았습니다**(운영자가 싱크 로그 2,705줄을 검색해 0건). 이제 격리 사유에 `| op: d` / `c/r` / `u` / `d (tombstone)`이 실리고, 쓰기 전 크기 가드가 파싱된 이벤트를 전달해 **다른 무엇으로도 유추할 수 없는 그 경로**에서도 op이 남습니다(시도 자체가 없었으므로 SQL도 렌더되지 않습니다). **기존 cdc 스택은 Delete + Deploy CDC infrastructure가 필요합니다** — v42의 NULL 키 삭제 가드와 함께 실리므로 **재배포 한 번으로 둘 다** 적용됩니다.
- **레코드의 구조화 필드는 비어 있고 정보는 자유 텍스트에 있었습니다.** CDC 격리 표면은 싱크의 CloudWatch 로그 줄을 파싱하는데, **이미 매칭해 둔 값을 버리고** 있었습니다. 이제 실제 필드로 승격됩니다: 실패한 행의 **PK**(텍스트에 `| pk: id=1`로 있었고, 그래서 고객이 도구 자신의 JSON을 정규식으로 긁어야 했습니다), **op**, 그리고 **Kafka topic/partition/offset** — 격리된 DELETE에는 이것이 원본 레코드에 이르는 유일한 경로입니다(그 행이 아직 존재하는 곳은 DLQ 토픽뿐입니다). `chunk_id`는 **의도적으로 None**입니다: 양방향에서 CDC와 Full Load를 가르는 유일한 판별자라서, 채우면 DLQ 카드가 비고 모든 격리가 Full Load 오류 로그로 쏟아집니다.
- **SQLSTATE가 없는 격리에 안정적 클래스가 생겼습니다.** 쓰기 전 크기 가드는 `DataException`을 던지므로 서버 오류가 없고 보고할 SQLSTATE도 없습니다 — 그래서 `error_code`가 null이었고 UI가 산문을 매칭해야 했습니다. 이제 `OVERSIZED_VALUE`를 보고합니다. **일부러 5자 SQLSTATE 형태가 아닙니다**: 소스 스키마 드리프트 배너가 `error_code`로 분기하므로 합성 코드가 그 SQLSTATE 중 하나로 오인되면 안 됩니다. 실제 SQLSTATE가 있으면 항상 그쪽이 이깁니다.
- **Java 로거 접미사가 운영자용 필드로 새어 나왔습니다.** MSK Connect 워커의 log4j 레이아웃이 모든 줄에 `(dev.dsqlmigrator.connect.DsqlSinkTask:759)`을 붙이고 그 설정은 AWS 관리라 원천에서 끌 수 없습니다. 여기서 제거합니다 — **바로 앞의 PK를 먹지 않으면서**.
- **초과 값 사유가 "재시도하면 된다"로 읽히지 않습니다.** 이제 이 행은 원본 그대로 저장할 수 없고 **재시도되지 않는다**고 명시합니다. 싱크의 분류가 이미 그것을 보장합니다: 일시 오류 분기는 격리에 도달하지 않으므로, `Quarantined record to DLQ` 줄의 존재 자체가 영구 폐기의 증거입니다.

### 변경

- **DLQ 패널이 Validation을 가리키고, 사각지대를 명시합니다.** DLQ 레코드는 "행이 버려졌다"만 말하고, 타깃에 미친 결과를 테이블별·PK별로 보여주는 것은 Validation뿐인데 **두 표면이 전혀 연결돼 있지 않았습니다**(Validation은 DLQ 레코드를 읽지 않고, "알려진 갭" 승인을 채우는 격리 카운트는 Full Load 전용이라 CDC 격리는 절대 그것을 채울 수 없습니다). 이제 어떤 Validation 결과가 어떤 복구를 의미하는지 말합니다 — **누락** 행은 버려진 insert로 재적재가 고치고, **추가** 행은 버려진 delete로 재적재가 **고칠 수 없습니다** — 그리고 사각지대를 밝힙니다: 버려진 UPDATE는 행 수를 바꾸지 않으므로 **기본값인 row-count 모드로는 아예 보이지 않고**, CHECKSUM도 json·부동소수점 컬럼은 건너뜁니다.
...
- **새 필수 CDC 체크가 아무것도 막지 못했습니다: 게이트가 허용목록이었고, 목록이 갱신되지 않았습니다.** `cdc_prerequisite_block_reason`은 의도적으로 `report.can_proceed`를 보지 않지만(그것은 Full Load 전용 사안까지 포함하므로), 그 구분을 **체크 ID 허용목록 3개**로 구현했습니다 — 그래서 차단 체크를 추가할 때 두 곳을 고쳐야 했고, 두 번째 수정이 **두 번** 누락됐습니다. v0.1.516의 `REPLICA_IDENTITY_COVERS_KEY`는 조용히 유실되는 DELETE를 아주 상세히 진단하면서도 **Deploy CDC infrastructure와 Start CDC가 붉은 FAIL 옆에서 활성 상태로 남아 있었고**, 다른 곳의 주석은 그것을 "the authoritative gate … the two can never disagree"라고 단언했습니다. 23개 체크 ID 전수 감사 결과 이것은 **13개 멤버를 가진 계열 결함**이었고, 최악은 `REPLICA_IDENTITY` = `nothing`입니다: 그대로 진행하면 **운영 소스에 쓰기 장애를 장전**합니다(그 테이블의 모든 UPDATE/DELETE가 publisher에서 ERROR). 도구의 유일한 다른 백스톱은 낡은 identity 인덱스, 파티션 리프, 그리고 커넥터 autocreate 경로 전체를 놓칩니다.

  이제 게이트는 **거부목록**입니다: 모든 `required` FAIL이 차단하고, 문서화된 예외는 하나(`TARGET_SCHEMA_READY` — 조치가 화면 밖 Schema Conversion을 가리키고, Deploy 카드는 그 DDL을 적용하는 Full Load와 MSK 생성이 겹치도록 일부러 Prerequisites 옆에 있습니다). 허용목록은 **조용히** 실패하고 거부목록은 **시끄럽게** 실패합니다. 감사로 과도 차단이 우연히 생길 수 없음도 확인했습니다: CDC를 막아선 안 되는 모든 상태는 이미 `required=False`로 채점되며, 그 플래그가 "권고"를 담고 있습니다. `TABLE_PRIMARY_KEY`는 **의도적으로 예외가 아닙니다** — 자체 조치 문구가 CDC를 지목하고, 싱크는 키 없는 테이블의 모든 레코드를 DLQ로 보내며, 패널은 이미 그것에 대해 "CDC: Blocked"를 렌더하면서 두 버튼은 활성이었습니다. 이제 테스트가 **모든 체크 ID를 게이트에 통과시키므로**, 다음 차단 체크는 기억이 아니라 존재만으로 집행됩니다. 비활성 버튼의 툴팁도 실패한 체크·테이블·조치를 이름으로 말합니다.
...
- **재키잉된 테이블의 leading 키 컬럼은 애플리케이션이 절대 UPDATE하지 않는 컬럼이어야 하며, 피커가 이제 그 실제 비용까지 말합니다.** 기존 문구는 "'<col>' must be immutable (DSQL keys cannot change after creation)"였는데 — 이는 ALTER에 관한 **타깃 측 규칙**으로 읽혀서, 정작 중요한 비용이 보이지 않았습니다. Debezium은 키 컬럼 변경을 **이전 키의 DELETE + 새 키의 INSERT**로 복제합니다. 두 레코드는 키가 달라 **서로 다른 토픽 파티션·서로 다른 싱크 태스크**로 가며 둘 사이에 순서 보장이 없고, 두 키 형태가 공존하는 동안 **원래 기본 키에 대한 UNIQUE 인덱스**가 INSERT를 거부합니다(SQLSTATE 23505). 싱크는 23505를 영구 오류로 분류하므로 INSERT는 오프셋이 커밋된 채 DLQ로 가고, 그 뒤 DELETE가 도착하면 **행이 타깃에서 사라집니다**. 이는 **두 소스 엔진 모두** 해당하고 어떤 소스 설정으로도 막을 수 없습니다 — `REPLICA IDENTITY FULL`도 소용없습니다 — 따라서 유일한 보호는 행의 생애 동안 값이 고정된 컬럼을 고르는 것이며, 안내문이 이제 예시와 함께 그것을 말합니다.
- **SQLSTATE 23505가 익명의 poison row가 아니라 분류되어 표시됩니다.** 자체 종류(`unique-conflict`)를 갖고, 소스 스키마 변경이 아니므로 "source schema change" 배너에서는 제외됩니다. 라벨은 **원인을 단정하지 않습니다** — 어떤 unique 인덱스든 23505를 낼 수 있고 외부에서 들어온 중복도 포함되므로 — 대신 본문이 분류 없는 DLQ 항목이 말할 수 없는 것을 말합니다: **행이 타깃에서 누락되었을 수 있다**는 것과 무엇을 확인해야 하는지.
- **Validation이 작동할 수 없는 조치를 더 이상 제시하지 않습니다.** 타깃에는 있고 소스에는 없는 행 — DELETE 미적용의 시그니처 — 이 비-격리 분기 하나로 떨어져, "Re-run Full Load + CDC to backfill the gap … Full Load는 멱등(INSERT ... ON CONFLICT)이라 **missing 행만 채웁니다**"라는 안내를 받았습니다. 바로 그 이유로 **추가 행은 지울 수 없습니다**: 그대로 따르면 전체 재적재를 거쳐 동일한 불일치에 도달합니다. 이제 추가 행에는 전용 안내가 나옵니다 — 건수, 재적재로 해결되지 않는 이유, 유력한 원인(소스 REPLICA IDENTITY가 DEFAULT인 재키잉 테이블; 재키잉된 leading 키 컬럼의 UPDATE), 그리고 실제로 듣는 조치(Full Load를 다시 돌리고 해당 테이블을 **DROP** 선택). missing 행 분기와 영구 격리 분기는 그대로입니다.
...
- **PostgreSQL 재스냅샷 시작이 `snapshot.mode=initial` + `publication.autocreate.mode=disabled`로 구성될 수 있었고, 이 조합은 커넥터를 확정적으로 죽입니다.** `initial`은 "Debezium이 자기 슬롯을 만들고 스냅샷한다"는 뜻이라 퍼블리케이션이 필요하고, `disabled`는 "Debezium은 퍼블리케이션을 만들 수 없다"는 뜻입니다. Aurora PostgreSQL 17.7에서 실측: Debezium 소스 태스크가 첫 접촉에서 `Publication autocreation is disabled, please create one and restart the connector`로 죽었고(배포 13분 경과), CloudFormation이 cdc 스택 전체를 롤백하면서 **6분 전에 RUNNING에 도달했던 sink 커넥터까지 삭제**했습니다. 커넥터 2개(각 2 MCU) + MSK Serverless를 약 14분 과금하고 얻은 것은 롤백뿐입니다.

  두 값은 **별개의 세션 상태**였고, "항상 둘을 같이 설정한다"는 규율로만 일치가 유지됐습니다 — 그 규율은 이미 세 호출부에 적어 두었는데도 지켜지지 않았습니다. 스냅샷 쪽만 설정하는 작성자가 둘 있었습니다: CDC 카드의 Automatic/Manual 라디오, 그리고 세션 복원(autocreate 플래그는 애초에 영속화되지 않아 앱 재시작이나 탭 재열기에서 조용히 "disabled"로 되돌아갔습니다 — 게다가 그 시점에는 두 구제책마저 사라집니다: 탈출 버튼과 두 경로 분기는 사전 점검이 아직 실패 중일 때만 렌더되기 때문입니다). 커넥터 설정 호출부 3곳 중 2곳은 이 값을 **아예 전달하지 않아**, 설정 **미리보기가 실제 배포와 다를** 수 있었습니다.

  작성자별로 고치지 않고 **구조적으로** 고쳤습니다: `publication.autocreate.mode`는 이제 어디에서도 파라미터가 아닙니다. 스냅샷 모드에서 **파생**됩니다(`pg_publication_autocreate_mode`: `initial` → `filtered`, `never` → `disabled`). 별개 상태는 삭제했고, 테스트가 도달 가능한 모든 (워터마크, 시작 모드) 입력을 열거해 치명적 조합 — 그리고 그 거울상인 `never` + `filtered`(요청되지 않은 소스 쓰기로, 이것도 도달 가능했습니다) — 이 **표현 불가능**함을 단정합니다.
- **바로 이 조합을 잡기 위해 존재하는 가드들이 구조적으로 그것을 볼 수 없었습니다.** `pg_replication_objects_blocker`의 `connector_autocreates_publication`은 스스로 "커넥터의 실제 설정의 미러"라고 문서화하지만, **시작 모드에서 재파생한** 값을 받고 있었습니다. 그래서 설정과는 우연히만 일치했습니다: 가드는 "manual"을 "autocreate 함"으로 읽었고, 설정은 아무도 설정하지 않은 별개 플래그를 읽었습니다. 모든 표면 — 사전 점검 채점, Deploy 대화 상자, Start 대화 상자, 워커 백스톱 — 이 통과시켰습니다. 이제 전부 설정 빌더가 쓰는 **같은 순수 함수**를 통해 유효 스냅샷 모드를 읽습니다.
- **사전 점검 재채점이 시작 모드만 봐서 두 번째 경우를 오채점했습니다.** "manual"은 `snapshot.mode=initial`에 도달하는 두 경로 중 하나일 뿐입니다: 워터마크에 슬롯이 없는 CDC-only 시작도 거기에 도달합니다. 없는 슬롯에서 재개할 것처럼 채점되어, 커넥터가 실제로 곧 만들 퍼블리케이션의 부재로 FAIL이 났습니다 — 옆의 (이제 올바른) 게이트와 화면이 모순되는 상태였습니다.
- **앱 내 CDC 오프셋 시드가 갓 생성된 MSK 클러스터에서 실패하고 엉뚱한 곳을 지목했습니다.** MSK Serverless 클러스터는 ACTIVE를 보고하고 부트스트랩 엔드포인트가 해석되더라도, 브로커가 SASL/IAM 핸드셰이크를 완료하기 전입니다. 시드는 kafka-python의 30초 부트스트랩 타임아웃으로 **단 한 번** 시도하고 재시도가 없었습니다: 실측으로 CREATE_COMPLETE 후 62초에 실패했고, 약 11분 뒤 무변경 재시도가 성공했습니다. 메시지는 세 가지 정적 전제(VPC 내부, 9098 인바운드, 데이터플레인 IAM)를 나열했기 때문에, 운영자는 이미 모두 올바른 서브넷·보안 그룹·IAM을 감사했습니다. 이제 첫 Kafka 호출 **전에** 브로커를 기다리고(상한 있음, 최악 약 14분, 멈춘 것처럼 보이지 않도록 진행 로그 출력), 그 대기가 소진된 경우에만 그 전제들을 언급합니다 — 그만큼 기다린 뒤라면 warm-up이 아니라 **경로**가 원인일 가능성이 높다고 명시합니다.

### 변경

- **이 저장소가 기록해 둔 Debezium 관련 진술을 정정했습니다.** `publication.autocreate.mode=filtered`는 기존 퍼블리케이션을 그대로 두지 **않습니다**: 함께 배포되는 플러그인(debezium-connector-postgres-2.7.4)은 FILTERED를 특별 처리해, 퍼블리케이션이 이미 있으면 `ALTER PUBLICATION <name> SET TABLE <table.include.list>`를 실행합니다. 실무적으로는 제한적입니다 — 이름은 항상 도구가 파생하고 새 테이블 목록은 정확히 캡처 대상이므로, 그것을 만든 같은 DB 사용자로 **도구 자신의 퍼블리케이션을 도구 자신의 의도대로** 다시 쓰는 것입니다 — 그래도 소스 쓰기이며, 반대로 적힌 그 주석이 설계의 근거였습니다. 그 주석이 정당화했던 보수적 채점(테이블을 빠뜨린 기존 퍼블리케이션에는 재스냅샷 경로를 제안하지 않음)은 **의도적으로 유지**합니다 — 그 ALTER를 여기서 라이브로 검증하지 않았기 때문입니다.
- CDC 시작 위치 라디오의 낡은 주석이 "사용 가능한 워터마크가 없으면 Automatic을 비활성화한다"고 적고 있었습니다. 그런 코드는 존재한 적이 없습니다(dict로 만든 Quasar 라디오에는 옵션별 비활성화가 없습니다). 유도는 라벨과 그 아래 안내가 합니다.

## v0.1.514

### 수정

- **"Re-snapshot every table"를 골라도 Deploy 대화 상자가 그 결정을 무시해, 이어가기가 다시 막다른 길이 되었습니다.** 그 경로를 고르면 사전 점검이 `Can proceed`로 바뀌고 실패 행이 권고로 강등되는데 — 그다음 Deploy CDC infrastructure가 붉은 패널로 차단하면서 "the connector is configured with publication.autocreate.mode=disabled … the Debezium task would die immediately"라고 주장했습니다. 이 진술은 **그 경로에 대해 거짓**입니다(바로 **커넥터의 DB 사용자**가 퍼블리케이션을 만드는 경로입니다). 그러고는 "start the migration over as Full load + CDC"라고 안내해, 운영자가 방금 내린 선택을 무효로 만들었습니다. 판정 함수는 처음부터 옳았습니다: v0.1.509가 Deploy 대화 상자에 이 게이트를 달면서 프로브를 `[], None`과 `force_initial` 없이 호출했고, 그래서 **알려주지도 않은 결정**을 빈 테이블 목록과 없는 워터마크에 대해 채점했습니다. 호출부가 아니라 **뿌리에서** 고쳤습니다: 이제 공유 게이트 하나가 세 입력을 모두 커넥터 설정 빌더가 쓰는 바로 그 함수들(`_cdc_tables_for_config`, `_cdc_resume_signal`)에서 파생하고, 그 객체를 게이트하는 **모든 표면**(Deploy 대화 상자, Start 대화 상자, 워커 백스톱)이 그것을 거칩니다. `pg_replication_objects_blocker`의 계약은 `build_pg_source_config`의 두 결정을 **정확히 미러링**하는 것이고, 미러가 **한 곳**에 있을 때만 검증할 수 있습니다: 같은 결함이 **두 번째 표면에서 다시 출시된 것이 이번이 두 번째**입니다(v0.1.507이 Start 경로를 고쳤고, v0.1.509가 Deploy에서 되살렸습니다). 이제 테스트가 **AST로** 다른 어떤 함수도 원 프로브를 **참조조차** 하지 않음을 단정합니다 — 호출 문법이 아니라 **참조**를 세는 것은 의도적입니다: 이 버그는 프로브를 `run.io_bound`에 **값으로** 넘겼기 때문에, 호출 문자열 검색으로는 보이지 않았습니다.
- **Start 대화 상자가 퍼블리케이션 커버리지를 캡처 대상이 아니라 소스의 모든 테이블에 대해 채점했습니다.** 위 통합 작업 중에 발견했습니다: 인벤토리 전체를 넘겼기 때문에, 이 마이그레이션이 복제하는 테이블을 정확히 포함하는 퍼블리케이션인데도 같은 DB의 **무관한 이웃 테이블** 때문에 커버리지 누락으로 보고될 수 있었습니다.

### 변경

- **사전 점검 결과 표의 셀이 줄바꿈되어, 가장 긴 조치 안내를 읽을 수 있습니다.** "Detail / remediation"은 압도적으로 가장 넓은 열이고(PostgreSQL 복제 객체 점검 실패는 detail + remediation이 ~800자입니다), Quasar의 QTable은 별도로 지시하지 않으면 모든 셀을 **한 줄**로 유지합니다(`q-table--no-wrap`을 붙이며, 이것이 각 셀에 `white-space: nowrap`을 전달합니다). 실제 브라우저에서 1304px 카드로 측정: 셀 폭 4656px, `.q-table__middle`이 가로 스크롤(scrollWidth 5082) — 그래서 행이 "The connector does n…"로 잘리고 나머지는 가로 스크롤바 뒤에 있었습니다. 운영자에게 가장 필요한 안내가 가장 안 보였던 것입니다. 줄바꿈 후 셀은 1092px, 가로 스크롤바 없음, 행 높이는 48px → 112px입니다: 높이는 페이지를 스크롤하면 되므로 숨겨진 조치 안내와 맞바꿀 만한 대가입니다. v0.1.505가 per-table 상태 카드에 적용한 것과 같은 수정이며, 디자인 시스템에 **두 메커니즘과 각각의 적용 기준**을 기록했습니다.
- **Deploy 대화 상자의 MSK 시드 안내가 한 줄이 되었습니다.** 세 문장 중 둘이 설계 근거였습니다 — 태스크의 서브넷이 아니라 VPC 범위 전체를 허용하는 이유, 그리고 앱이 클러스터에서 할 수 있는 일은 여전히 `kafka-cluster` IAM으로 제한된다는 것. 둘 다 사실이지만 **결정 근거는 아니며**, 과금되는 확인 대화 상자를 정당화로 채우면 **실제로 결정에 쓰이는 패널**(비용 추정, 네트워크 계획)을 찾기 어려워집니다. 이제 범위·VPC·포트·용도만 말합니다.

## v0.1.513

### 변경

- **CDC 사전 점검 아래의 안내가 한 문제를 네 겹으로 말하는 대신 하나의 결정 블록이 되었습니다.** PostgreSQL "Full load only → CDC only" 이어가기에서 점검 아래에 일곱 개가 세로로 쌓였고, **퍼블리케이션이 없다**는 하나의 사실이 내려오면서 서로 다른 심각도를 **네 번** 달았습니다 — 빨간 `Blocked` 배지, Source Configuration의 빨간 "1 failed" 배지, 앰버 "갭 없는 핸드오프는 이제 불가능" 알림, 빨간 "Resolve the failed prerequisite(s)" 문구. 그 사이에 파란 정보 패널 2개가 끼어 있어 "지금 심각한 상태인가, 읽을 거리인가"에 답이 없었습니다. 게다가 **선택지가 둘**이라는 사실에 도달하기까지 산문 11행쯤을 읽어야 했습니다. 이제 앰버 박스 하나가 상황을 **한 번만** 말하고, 두 경로를 **그 박스 안에 동등하게** 담습니다 — 각각 자기 버튼과 "무엇을 남기는가" 한 줄 요약을 갖고, 비용·동작 원리는 "What this does, and what it costs" 아래로 접어 두어 상세가 필요한 사람만 펼칩니다. (이 저장소는 같은 실패 양상을 이미 한 번 기록해 두었습니다 — `stale_error_notice`의 "판정 셋이 동시에, 어느 것이 참인지 사용자가 알 수 없다".)
- **권장 경로가 지시문이 아니라 도구가 수행하는 버튼이 되었습니다.** 기존에는 "위에서 마이그레이션 타입을 바꾸고 Full Load를 다시 돌리세요"라고만 적혀 있었고, 화면의 **유일한** 클릭 가능한 액션은 권장하지 **않는** 경로에 붙어 있었습니다 — 시각적 유도와 권고가 정반대였습니다. 이제 "Start over as Full load + CDC"가 **주 버튼**입니다: 마이그레이션 타입을 바꾸고, 새 타입에 대한 사전 점검(읽기 전용, 수 초)을 다시 돌리고, Full Load를 엽니다. 이동은 **재채점된 보고서가 실제로 진행 가능할 때만** 합니다 — 차단 이유는 운영자가 떠나게 될 그 패널에 있으니까요.
- **실행 가드 문구가 자기 해결책 아래에서 판정을 다시 말하지 않습니다.** 읽는 순서가 역전돼 있었고("이렇게 해결하세요" → "해결해야 합니다"), 이미 판정을 담은 배지 두 개와 중복이었습니다. 경로 선택이 화면에 있을 때는 비어 있는 주 액션 자리의 의미만 말합니다: "Pick one of the two routes above to continue."
- **차단된 사전 점검이 무엇을 막는지 이름을 말합니다.** Full Load, 과금되는 ~5분짜리 CDC 인프라 배포, Start CDC가 모두 "run"할 수 있는 화면에서 "before running"은 모호했습니다. 이제 CDC에서는 "before deploying the CDC infrastructure or starting CDC"(그 실패가 먼저 막는 것이자 **돈이 드는** 것이 배포이므로), 적재에서는 "before running the Full Load"로 읽힙니다.
- **"Full load only" 타일이 PostgreSQL 소스가 이 선택으로 무엇을 포기하는지 밝힙니다.** v0.1.511은 이것을 반대편에서 해결했지만("Full load + CDC" 타일에 CDC 단계를 나중에 시작할 수 있다고 명시), 저렴하고 추가 인프라가 없는 이 타일만 읽는 운영자는 이것이 **갭 없는 CDC 핸드오프를 영구히 포기하는 유일한 선택**임을 여전히 알 수 없었습니다 — 제보자가 정확히 그 이유로 이 타일을 골랐습니다. 이제 결정이 일어나는 자리에서 그것을 말합니다: 적재 중에는 WAL을 붙잡는 것이 없고 복제 슬롯은 과거 위치에 만들 수 없으므로, 나중에 CDC를 붙이려면 모든 테이블을 재스냅샷하거나 "Full load + CDC"로 다시 시작해야 합니다. MySQL은 해당 없음 — CDC-only 시작은 워터마크의 binlog 좌표를 커넥터에 심으므로 binlog가 보존되는 한 핸드오프는 갭이 없습니다.

### 수정

- **사전 점검 패널의 재스냅샷 경로가 결정을 절반만 기록해, 잡이 과금된 뒤 죽었습니다.** 그 경로의 문구는 커넥터가 이 적재에 없던 퍼블리케이션을 만들어 준다고 약속하는데, 버튼(v0.1.510)은 시작 모드("manual", 즉 `snapshot.mode=initial`)만 기록하고 `publication.autocreate.mode=filtered`는 **기록하지 않았습니다**. 절반의 결정이 모든 게이트를 열었습니다: 재채점된 점검이 통과하고, 과금되는 ~5분짜리 인프라 배포가 허용되고, Start CDC 대화상자(자체 프로브가 `force_initial`을 미러링하므로 아무 이의도 제기하지 않습니다)가 커넥터 둘을 만들었고, 그 다음 Debezium 태스크가 "Publication autocreation is disabled"로 죽었습니다. Start CDC 대화상자의 같은 탈출구는 **항상 두 노브를 다** 설정했습니다 — 이제 이쪽도 그렇게 합니다. 권장 경로는 추가로 둘을 **초기화**하므로, 재스냅샷을 골랐다가 마음을 바꾼 운영자가 `snapshot.mode=initial` + `autocreate=filtered`를 **스냅샷 지점에 도구가 만든 슬롯에서 스트리밍하는 것이 전부인 그 실행**으로 끌고 들어가지 않습니다.

## v0.1.512

### 수정

- **복원된 세션이 PostgreSQL 소스에 MySQL 스키마 변환기를 만들었습니다.** Data Migration 화면의 엔진 읽기 두 곳이 소스 엔진을 `source_config`에서 MySQL 폴백으로 가져왔는데, **복원된 세션**(재연결, 태스크 교체)은 엔진을 `source_config` **없이** 기록합니다 — 그래서 정확히 **재연결한 운영자에게만** 폴백이 걸렸고, 그것이 발견되지 않은 이유입니다. 측정된 결과: PostgreSQL `timestamp`가 `TIMESTAMP` 대신 `TIMESTAMPTZ`로 변환되고(타임존 의미가 바뀜), `bit(1)`이 `SMALLINT`가 되며, PostgreSQL 변환 경고가 **전부 사라집니다** — 배열·range의 "Aurora DSQL 미지원" 안내와 1 MiB 값 한도 경고를 포함해서.

  이번 릴리스로 **동작은 바뀌지 않으며**, 그것을 가정하지 않고 확인했습니다: 이 호출부들이 실제로 소비하는 두 값 — CDC 복합 재키잉 컬럼과 외래 키 소유권, 그리고 recreate 공시용 타깃 기본 키 — 이 `bytea` 키·키 없음·복합 키·외래 키 있는 테이블을 포함한 **9가지 기본 키 형태 전부에서** 두 변환기 모두 동일했습니다. 그래도 고칠 가치가 있는 이유는 그 일치가 **우연**이기 때문입니다: 엔진 차이가 재키잉 결정에 닿는 순간 CDC 레코드 키가 틀리고, 싱크가 **잘못된 컬럼으로 upsert**하게 됩니다 — 조용히. 이제 두 곳 모두 다른 모든 PostgreSQL 인식 표면과 같이 `session_source_type`을 써서, 서로 다른 이유로 옳은 두 규칙 대신 **모든 엔진 읽기에 하나의 규칙**이 적용됩니다.

## v0.1.511

### 변경

- **이어가기 안내가 재스냅샷이 아니라 "Full load + CDC로 다시 시작"을 먼저 권합니다.** 두 경로 모두 소스 테이블 전체를 다시 읽으므로 **재읽기는 차이점이 아닙니다**. 다만 적재를 다시 돌리면 (a) 스트리밍 파이프라인이 아니라 도구의 벌크 로더를 쓰고, (b) 적재 **전에** 복제 슬롯을 만들어 스냅샷 지점 이후 전부가 스트리밍되고, (c) 각 타깃 테이블을 먼저 **DROP**할 수 있는데 — 이것이 첫 적재 이후 소스에서 삭제된 행을 지울 **유일한** 방법입니다. 재스냅샷 경로는 그 행들을 타깃에 영구히 남깁니다. 같은 비용에 결과가 엄격히 더 좋으므로 그쪽을 권장으로 올렸고, 재스냅샷은 "적재를 다시 돌릴 수 없을 때"의 2순위입니다.
- **"CDC only" 타일이 PostgreSQL에서 불가능한 두 가지를 약속하지 않습니다.** 기존 문구는 "start from a prior watermark or an external start position"이었는데, 앞은 그 워터마크가 Full-load-+-CDC가 만든 **슬롯을 담고 있을 때만** 참이고, 뒤는 **아예 존재하지 않습니다** — `_cdc_resume_signal`이 PostgreSQL에서는 운영자의 시작 override를 **버립니다**. Debezium PostgreSQL은 복제 슬롯의 커밋된 위치에서만 재개하고 임의 WAL LSN을 시작점으로 받을 수 없기 때문입니다. 타일을 **비활성화하지 않은 것은 의도적입니다**: 이전 실행이 슬롯을 기록했다면 CDC-only는 유효하고 갭이 없으며, 워터마크를 포함한 잡 상태는 영속화되어 시작 시 다시 읽히므로 **나중 세션에서도 보입니다** — 그리고 비활성화하면 재스냅샷 이어가기까지 사라집니다. "Full load + CDC" 타일에는 그것만의 강점을 명시했습니다: 슬롯이 적재 전에 만들어지고, CDC 단계는 나중에 시작할 수 있습니다. 이 문구는 Data Migration 스텝 **배너로도 재렌더링**되므로, 거짓 절이 타일이 아니라 그 스텝의 **모든 화면 위에** 떠 있었습니다.

### 수정

- **타일을 "Full load + CDC"로 바꾼 뒤 CDC 인프라를 배포해도, 적재가 프로비저닝하지 않은 클러스터에 과금되지 않습니다.** 적재가 끝난 Full-load-only 뒤에 타입을 바꾸면 존재 사전 점검이 SKIP으로 재등급되고("Full Load가 이번 실행에서 만든다"는 detail), 게이트가 객체가 아직 없는데도 Deploy를 열어줬습니다. MSK Serverless가 생성되고 Start CDC가 거부했습니다. 이제 비용 게이트가 타일이 아니라 **워터마크**를 봅니다 — 새 결합 실행은 워터마크가 없으므로 권장 흐름(적재 전 인프라 배포)은 그대로, 적재가 프로비저닝한 실행은 슬롯을 갖고 있으며, **슬롯 없이 끝난 적재**가 바로 막아야 할 상태입니다. 이번 릴리스가 권하는 경로에 있던 함정이라 중요합니다.
- **복원된 세션이 PostgreSQL 운영자에게 MySQL 타일 의미를 보여줬습니다.** 마이그레이션 타입 선택기가 엔진을 `source_config`에서 MySQL 폴백으로 읽었고, 재연결된 세션은 엔진을 `source_config` **없이** 기록합니다 — 그래서 위의 모든 엔진별 문구가 **정확히 재연결한 운영자에게만** 무력화됐을 것입니다. 이제 다른 모든 PostgreSQL 인식 표면과 같이 `session_source_type`을 씁니다.

## v0.1.510

### 수정

- **PostgreSQL "Full load only" 이후 CDC로 이어가는 것이 아예 불가능했습니다 — 무손실 경로가 죽은 상태로 출하되고, 그 뒤 거기로 가는 버튼 둘이 비활성화됐습니다.** v0.1.507이 "Re-snapshot every table instead"를 넣어 퍼블리케이션 없는 CDC-only 시작도 무손실로 진행할 수 있게 했지만, Start-CDC 워커 백스톱이 아무것도 만들기 직전에 같은 blocker를 호출하고 그 blocker가 **의도와 무관하게** 퍼블리케이션 부재를 거부했습니다. 그래서 버튼이 Start를 풀어주고 잡이 "Start CDC blocked"로 raise했습니다. 그 뒤 v0.1.509가 실패한 사전 점검에서 Deploy를 (올바르게) 비활성화했고, 재스냅샷 결정을 기록할 수 있는 유일한 표면은 그 버튼 **뒤에 있는** 대화상자였습니다 — 경로가 하나도 남지 않았습니다. 이제 blocker가 커넥터의 두 의도를 명시적으로 받아 `build_pg_source_config`를 그대로 반영합니다: `publication.autocreate.mode=filtered`는 퍼블리케이션 부재를, `snapshot.mode=initial`은 슬롯 부재를 면제합니다. **존재하지만** 테이블을 빼먹었거나 publish 목록이 좁은 퍼블리케이션은 계속 차단합니다 — Debezium의 `filtered`는 기존 퍼블리케이션을 변경하지 않습니다.
- **완료되지 않은 Full Load의 워터마크가 갭 없는 재개 지점으로 취급돼, 적재되지 않은 테이블을 조용히 누락시켰습니다.** 아무도 보고하지 않았고 이 사슬에서 가장 심각한 결함입니다. 워터마크는 **첫 테이블 적재 전에** 붙으므로, FAILED/부분 실행도 성공한 실행과 바이트 단위로 동일한 워터마크(같은 LSN, 같은 슬롯 이름)를 남기고, CDC 시작 경로의 어디도 잡의 status를 읽지 않았습니다. 그래서 CDC 시작이 `snapshot.mode=never`를 골라 적재되지 않은 테이블은 베이스라인도 백필도 못 받고 타깃에서 **영구히 누락**됐습니다. 타입을 "CDC only"로 바꾸면 "실패한 테이블을 먼저 재시도하라"는 안내마저 사라집니다(그 서브스텝이 없어지므로). 이제 `DONE`이 아닌 잡에서는 슬롯을 억제해 전체 재스냅샷(무손실)으로 보내고, **자기 치유**됩니다 — 재시도가 원래 워터마크를 이어받으므로 적재가 완료되면 갭 없는 재개가 돌아옵니다.
- **무효화된 복제 슬롯이 usable로 등급**돼, 복구 가능한 재스냅샷이 죽은 Debezium 태스크로 바뀌었습니다. 이제 두 프로브 모두 `wal_status='lost'`를 거부합니다. PostgreSQL 16.15에서 라이브 확인: `max_slot_wal_keep_size=0`과 충분한 변경으로 슬롯이 `wal_status=lost`에 도달하는 동안 소스는 계속 커밋했습니다 — 즉 초과된 슬롯은 **무효화된 선택지**이고 소스 장애가 아닙니다.

### 추가

- **이어가기 수단이 실패한 사전 점검 항목 옆에 제공됩니다** — 비활성화된 버튼 뒤의 대화상자가 아니라. "Re-snapshot every table"이 결정을 기록하고 (읽기 전용, 수 초의) 검사를 다시 돌려서, **하나의 재등급된 리포트가 모든 게이트를 한 번에** 해제합니다. 활성화된 주 버튼 옆에 빨간 FAIL이 남지 않습니다. 재스냅샷이 실제로 고치는 실패에만 제공하며, 메시지 문구 매칭이 아니라 명시적 플래그로 판별합니다. 문구가 실제 비용 두 가지를 모두 밝힙니다: 소스 테이블을 다시 읽는다는 것, 그리고 적재와 CDC 시작 사이에 **삭제된** 행은 스냅샷에도 스트림에도 없어 타깃에 남고 Validation이 extra row로 보고한다는 것.

이제 PostgreSQL에서 "Full Load를 끝내고 CDC로 이어가기"가 두 경로로 동작하고, 그 차이가 발견 대상이 아니라 **명시**됩니다. 재읽기 없는 갭 없음은 적재 **전에** 결정해야 하고 — 논리 복제 슬롯은 과거 위치에 만들거나 되감을 수 없습니다 — 도구는 그 결정을 이미 "Full load + CDC" 타입으로 구현하고 있으며 그 CDC 단계는 나중에 시작할 수 있습니다. Full-load-only 실행 뒤 남는 무손실 경로는 재스냅샷입니다. MySQL은 모든 모드에서 영향 없습니다(binlog는 소비자 없이 이력을 보존).
...
- **CDC의 퍼블리케이션과 복제 슬롯이 실제로 존재하는지 검사하는 사전 점검 항목.** "Source user can create the CDC publication"은 **권한**을 검사하므로 객체가 없어도 초록으로 통과했습니다 — 그래서 "사전 점검 통과 후 확정된 배포 실패"가 나왔습니다. 둘은 서로 다른 질문이고 해결책도 다릅니다(권한 부여 vs. 적재 실행/재스냅샷). 이제 나란히 별도 행입니다. 존재만 보지 않고 **커버리지도 등급**합니다: 선택 테이블을 빼먹은 퍼블리케이션, INSERT/UPDATE/DELETE를 좁힌 퍼블리케이션은 **FAIL** — 각각 커넥터가 RUNNING인 채 해당 테이블 또는 해당 변경 종류를 전혀 복제하지 않게 됩니다. **슬롯** 부재는 WARN입니다. 그런 시작은 재스냅샷하므로 정확성이 아니라 재읽기 비용만 듭니다. 읽지 못한 사실은 UNKNOWN으로 남고 아무것도 차단하지 않습니다.

  이 행은 마이그레이션 타입이 아니라 **프로비저닝 주체 자신의 판정**으로 게이팅합니다: "Full load + CDC"에서는 객체가 **없는 것이 정상**(Full Load가 만듭니다)이라 그 모드에서는 SKIP이고, 갭 없는 유일한 경로를 절대 막지 않습니다.

### 변경

- **"CDC only" 실행에서 소스에 스트리밍할 것이 없으면 CDC 인프라 배포를 거부합니다** — 약 5분짜리 과금 MSK Serverless 생성 **뒤**가 아니라 **전에**. 의도적으로 비대칭입니다: "Full load + CDC"는 그대로 둡니다. 적재 전에 인프라를 배포하는 것이 도구 자신이 권하는 흐름이고, 그 시점에 객체가 없는 것이 정상이기 때문입니다. 아무도 가둬두지 않습니다 — Start 쪽 검사는 이 판정을 기억하지 않고 **다시 조회**하므로, 그 사이에 프로비저닝하면 그냥 해제됩니다.
- **Full-load-only PostgreSQL 실행이 도구 자신의 목소리로 깨진 경로를 권하지 않습니다.** 적재 후 안내가 "CDC only"로 설정하라며 "이 Full Load의 워터마크에서 이미 적재된 타깃으로 스트리밍(재스냅샷 없음)"이라고 했는데 — PostgreSQL에서는 **양쪽 다 거짓** — 원클릭 링크까지 제공했습니다. 이제 슬롯이 만들어지지 않았음, 갭 없는 핸드오프는 슬롯이 적재 **전에** 있어야 함, 그리고 정직한 두 경로의 비용을 설명합니다. MySQL 문구는 그대로이고 여전히 참입니다.
- **"CDC only" 타일이 퍼블리케이션을 마련해 줄 것처럼 암시하지 않습니다.** PostgreSQL 요구사항이 이제 퍼블리케이션과 슬롯이 **이미 존재해야** 하고 이 모드는 만들지 않는다고 말하며, "Full load + CDC" 타일은 스냅샷 지점에 도구가 만든다고 말합니다. Full-load-only 사전 점검 **프리뷰**에도 그 내용을 넣었습니다 — 그 리포트가 갭 없는 선택이 가능한 마지막 순간입니다.
- **FAILED 커넥터의 문제 해결 줄이 CloudWatch 로그 그룹 이름을 알려줍니다** — UI 어디에도 렌더링되지 않던 링크를 약속하는 대신에.
- README와 cdc-stack 템플릿이 갭 없는 핸드오프를 소스 무관하게 설명하지 않습니다. 메커니즘은 엔진마다 다르고, 소스 무관 문구가 틀렸던 부분이 정확히 PostgreSQL 쪽입니다.

## v0.1.507

### 수정

- **PostgreSQL에서 "Full load only" 이후 "CDC only"로 시작하면 Debezium 태스크가 즉사했고, 도구는 인프라 비용을 먼저 치르게 했습니다.** CDC 퍼블리케이션과 복제 슬롯을 만드는 것은 단 하나 — Full Load의 프로비저닝 단계 — 이고, 그 실행이 CDC 계획의 일부일 때만입니다. Full-load-only 실행은 둘 다 만들지 않으므로, 커넥터가 존재하지 않는 객체를 가리킨 채 배포되고 소스 태스크가 "Publication autocreation is disabled"로 죽었습니다. 도구는 이미 이 상황을 감지했지만 **경고만** 했기 때문에, 과금되는 MSK Serverless 클러스터와 커넥터 2개가 먼저 만들어졌습니다. 이제 Start CDC가 소스를 **조회**해(권한이 필요 없는 카탈로그 읽기) 지출 전에 차단합니다 — Start 대화상자에서, 그리고 아무것도 만들기 직전 워커 스레드에서 한 번 더. 두 번째 검사는 대화상자 없이 잡을 제출하는 "Retry CDC"를 커버합니다. "카탈로그가 없다고 말할 때"만 차단하고, "물어볼 수 없었을 때"는 차단하지 않습니다.
- **그 뒤에 있던 더 나쁜, 조용한 버그: 도구가 만든 적 없는 복제 슬롯에서 재개하라고 설정했습니다.** `snapshot.mode`를 워터마크의 WAL LSN **만**으로 골랐고, Full-load-only 워터마크는 LSN은 있고 슬롯은 없습니다 — 그래서 CDC-only 시작이 `snapshot.mode=never`로, 즉 위치를 알 수 없는 슬롯에서 재개하도록 구성됐습니다. 지금의 요란한 태스크 사망이 그것을 막는 **유일한** 장치였습니다: 객체가 우연히 존재했다면(수동 생성, 또는 이전 세션의 잔존) 커넥터가 시작되어 적재 이후의 모든 변경을 조용히 건너뛰었을 것이고, 기본 ROW_COUNT 검증으로는 그것을 탐지할 수 없습니다. 이제 좌표가 아니라 **기록된 슬롯**으로 모드를 고릅니다. 기록된 슬롯이 없으면 `initial`입니다.
- **시작 지점 카드가 존재하지 않는 슬롯으로 갭 없음을 약속했습니다.** UI에서도 같은 근본 원인 — LSN만 검사했기 때문에 Full-load-only 워터마크가 "Automatic — gapless from the replication slot (recommended)", 초록 Ready 배지, 초록 체크 "Start point set"을 렌더링했습니다. 이제 기록된 슬롯도 요구합니다. (바로 옆에 이미 기록돼 있는 MySQL GTID-only 사후분석과 같은 버그 형태입니다.)
- **PostgreSQL 커넥터 실패를 CloudWatch에 남기지 않고 진단합니다.** 기존 로그 시그니처가 전부 MySQL/MSK 계열이어서, PostgreSQL 태스크 사망은 내용 없는 "<connector> entered FAILED state."만 냈고 운영자가 MSK Connect 로그 그룹을 직접 열어야 했습니다. PostgreSQL 시그니처 2개를 추가하고, FAILED뿐 아니라 **타임아웃**에도 워커 로그를 참조합니다 — MSK Connect는 태스크 수준 상태도, 마지막 태스크 예외도 노출하지 않으므로, 기동 시 유일한 태스크가 죽은 커넥터는 그렇지 않으면 예산이 소진될 때까지 CREATING에 머물고 원인은 도구 어디에도 보이지 않습니다.

### 변경

- **객체가 없을 때, 차단은 막다른 길 대신 원클릭 탈출구를 제공합니다.** "Re-snapshot every table instead"를 누르면 Debezium이 자기 슬롯을 만들고 선택된 테이블을 스냅샷한 뒤 그 슬롯에서 스트리밍합니다 — 스냅샷과 스트림 사이에 창이 없으므로 잃는 것이 없고, 타깃 적재는 멱등이므로 행을 다시 읽어도 안전합니다. 퍼블리케이션도 없으면 **커넥터의** DB 사용자가 캡처 대상 테이블만 담은 퍼블리케이션을 만듭니다. 도구 자신은 여전히 소스에 쓰지 않습니다. 비용은 소스 테이블을 다시 읽는 것입니다.

  "갭을 받아들이고 지금부터 스트리밍" 옵션은 **의도적으로 없습니다.** 창은 Full Load **시작**부터 열리므로 적재 전체와 모든 고민 시간을 포함하고, 도구는 몇 행이 몇 테이블에서 영향받는지 말해줄 수 없으며, 기본 검증 모드는 그 결과를 clean으로 인증합니다. 평가할 수 없는 동의는 동의가 아니라 그냥 클릭입니다.

PostgreSQL 소스에서 갭 없음은 적재 **전에** 내리는 결정입니다. "Full load + CDC"는 스냅샷 지점에 슬롯을 만들어 갭도 재읽기도 없습니다. Full-load-only 실행 뒤에는 그 선택지가 사라집니다 — 논리 복제 슬롯은 과거 위치에서 만들 수도, 되돌릴 수도 없습니다(PostgreSQL 16·18에서 실측: 함수에 LSN 인자가 없고, CREATE_REPLICATION_SLOT은 거부하며, slot_advance는 전진만 하고, START_REPLICATION은 조용히 clamp합니다) — 그래서 남은 갭 없는 경로는 재스냅샷입니다. MySQL은 모든 모드에서 영향이 없습니다. binlog는 소비자 없이도 이력을 보존하므로 프로비저닝이 필요 없습니다.
...
- **Per-table migration status 테이블에 더 이상 카드 안쪽 가로 스크롤바가 생기지 않습니다.** Quasar가 모든 테이블 헤더와 셀에 `white-space: nowrap`을 지정하기 때문에, 11개 컬럼의 여러 단어 헤더("Source rows (est.)")와 긴 `schema.table` 이름이 테이블을 카드보다 넓게 고정시켜 내부 영역이 스크롤되기 시작했습니다. 이제 헤더와 테이블 이름 컬럼은 줄바꿈될 수 있습니다. 두 가지가 모두 필요했습니다 — 헤더 줄바꿈을 허용하면 폭의 대부분을 되찾지만, `nowrap`이 살아 있는 동안 이름 컬럼의 `word-break`는 아무 일도 하지 않습니다(줄바꿈이 금지된 곳에서는 break 지점을 쓸 수 없습니다). 그 한 가지를 빼먹으면 이름 컬럼이 전체 폭에 고정된 채 시험한 모든 폭에서 스크롤바가 남았습니다. 숫자 컬럼은 `nowrap`을 유지합니다. 천 단위 구분 숫자는 애초에 break 지점이 없고, 쪼개지면 읽기 나쁩니다. 앱이 실제로 렌더링하는 columns·rows·셀 slot으로 실제 브라우저에서 측정했습니다: 1100px 카드는 가로 오버플로 59px → 0, 1000px는 159px → 0, 850px는 309px → 0이며, 어느 폭에서도 잘리는 셀은 없습니다. 약 820px 아래에서는 숫자 컬럼이 줄바꿈으로 줄일 수 없는 하한에 닿는데, 그 지점에서는 데이터를 숨기는 대신 스크롤바를 두는 편이 낫습니다.

## v0.1.504

### 수정

- **Fargate 배포에서 PostgreSQL CDC를 여전히 시작할 수 없었습니다 — 앱이 자기 IP를 잘못 읽었습니다.** v0.1.503은 cdc-stack이 MSK 9098에서 허용해야 할 네트워크를 알아내려고, UDP 소켓을 링크로컬 주소로 `connect()`한 뒤 커널이 바인딩한 주소를 읽었습니다. `awsvpc` ECS 태스크에서 그것은 **틀린 인터페이스**입니다 — 태스크에는 ECS 메타데이터/자격증명 엔드포인트용 별도 링크로컬 인터페이스가 있고, 그 주소가 **거기서** on-link이기 때문에 오히려 그 인터페이스가 선택됩니다. 그래서 앱은 ENI 사설 IP가 아니라 `169.254.172.2`를 보고했습니다. "모르겠다"가 아니라 **형식만 맞는 틀린 답**을 돌려준 것입니다. 그 결과 오탐이 하나가 아니라 둘이었습니다: 배포 대화상자가 "이 앱의 네트워크 인터페이스를 vpc-…에서 찾을 수 없다"며 차단했는데 같은 대화상자의 Network 패널은 바로 그 VPC를 정확히 설명하고 있었고, Start CDC **역시** 거부했습니다 — 그래서 `DSQL_MIGRATOR_CDC_HOST_SUBNET_CIDR`을 설정해도 우회되지 않았습니다. 즉 Fargate에서는 PostgreSQL CDC를 시작할 수 있는 지원 구성이 **하나도** 없었습니다. 이제 앱은 ECS에서 실행 중이면 ECS 태스크 메타데이터 엔드포인트에 자기 ENI 주소를 물어보고, 더 중요하게는 **VPC 네트워크 인터페이스가 절대 가질 수 없는 주소**(링크로컬·루프백·멀티캐스트·예약)를 그대로 쓰지 않고 **거부**합니다. 모르는 답은 이제 정직하게 "모름"입니다. Start CDC는 추가로, 명시적으로 설정된 `DSQL_MIGRATOR_CDC_HOST_SUBNET_CIDR`을 자체 주소 검사보다 우선합니다 — 그 값을 설정한 운영자는 이 호스트의 네트워크를 이미 확인해 준 것이기 때문입니다.

### 변경

- **네트워크를 알아낼 수 없으면 배포를 차단하는 대신 경고하고, 어느 단계가 실패했는지 말합니다.** 서로 다른 네 가지 원인 — 자기 주소를 못 알아냄, `ec2:DescribeNetworkInterfaces` 거부/스로틀, 입력한 VPC에 그 주소를 가진 인터페이스가 없음, 주소가 VPC CIDR 블록 밖 — 이 모두 **동일한 한 문구**로 표시됐고, 그 유일한 조치 안내("이 앱이 실행되는 VPC에 CDC 인프라를 배포하세요")는 이미 그 VPC 안에 있는 앱에는 순환 논리였습니다. 이제 각 원인이 자기만의 문구와 자기만의 해결책을 갖고, 배포는 더 이상 막히지 않습니다: cdc-stack의 9098 ingress는 기본값이 빈 CIDR 규칙이므로, 그 없이 배포한 뒤 cdc-stack의 `ConnectorSecurityGroup`에 TCP 9098 인바운드 규칙을 직접 추가하는 것이 **동작하는 경로**입니다 — 반면 차단은 아무 경로도 남기지 않았습니다. 배포의 **템플릿 자체**가 MSK에 도달할 수 없다고 선언하는 경우는 계속 차단합니다. 보안 그룹 규칙으로는 없는 egress 규칙이나 없는 태스크 역할 정책을 만들 수 없기 때문입니다.
- **네트워크 확인 과정이 더 이상 침묵하지 않습니다.** `core/ec2_metadata.py`에는 로거가 아예 없었고 세 군데에서 원인을 조용히 삼켜서, 이 문제를 진단하려면 일회성 Fargate 태스크를 띄워야 했습니다. 이제 확인된 주소, 버려진 주소, 실패한 AWS 조회를 모두 로그로 남깁니다 — 포기하는 경우는 `WARNING`이라, 배포된 기본 `INFO` 레벨에서도 보입니다.

앱 코드만 변경: 기존 스택은 이미지(`ContainerImageUri`)만 갱신하면 되고, 전체 템플릿 갱신은 필요 없습니다.
